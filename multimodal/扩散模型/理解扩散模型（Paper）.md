...
\end{align}$$
在这种情况下，我们学习了一个中间瓶颈分布 $q_{\phi}(z|x)$，可以将其视为一个编码器；它将输入转换为潜在变量的分布。同时，我们学习了一个确定性函数 $p_{\theta}(x|z)$，用于将给定的潜在向量 $z$ 转换为观测值 $x$，这可以被解释为一个解码器。

方程 19 中的两项都有直观的描述：第一项衡量了从我们的变分分布中解码器的重建可能性；这确保了学习到的分布能够建模有效的潜在变量，从而可以从原始数据中重新生成。第二项衡量了学习到的变分分布与潜在变量的先验信念之间的相似程度。最小化这一项可以鼓励编码器真正学习一个分布，而不是坍缩成一个狄拉克 $δ$ 函数。因此，最大化 ELBO 等同于最大化其第一项并最小化其第二项。

变分自编码器（VAE）的一个关键特征在于如何联合优化参数 $\phi$ 和 $\theta$ 的 ELBO。VAE 的编码器通常被设计为建模具有对角协方差矩阵的多元高斯分布，而先验分布则常被选为标准多元高斯分布：$$\begin{align}
    q_{\phi}(z|x) &= \mathcal{N}(z; \mu_{\phi}(x), \sigma_{\phi}^2(x)\textbf{I})\\
    p(z) &= \mathcal{N}(z; 0, \textbf{I})
\end{align}$$
然后，*ELBO 的 KL 散度项可以解析计算，而重构项可以使用蒙特卡洛估计来近似*。我们的目标可以重写为：$$\begin{align}
    {\arg\max_{\phi, \theta} \mathbb{E}_{q_{\phi}(z|x)}\left[\log p_{\theta}(x|z)\right] - D_{\mathrm{KL}}({q_{\phi}(z|x)}\|{p(z)} )\approx \arg\max_{\phi,\theta} \sum_{l=1}^{L}\log p_{\theta}(x|z^{(l)}) - D_{\mathrm{KL}}({q_{\phi}(z|x)}\|{p(z)})}
\end{align}$$
其中潜在变量 $\{z^{(l)}\}_{l=1}^L$ 是从 $q_{\phi}(z|x)$ 中采样得到的，对应于数据集中的每个观测值 $x$。然而，这种默认设置存在一个问题：计算损失时所依赖的每个 $z^{(l)}$ 都是通过随机采样过程生成的，而这一过程通常是不可微的。幸运的是，当 $q_{\phi}(z|x)$ 被设计为模拟某些分布（包括多元高斯分布）时，可以通过 *重参数化技巧* 来解决这个问题。

重参数化技巧将一个随机变量重写为一个噪声变量的确定性函数；这使得可以通过梯度下降优化非随机项。例如，来自具有任意均值 $\mu$ 和方差 $\sigma^2$ 的正态分布 $x \sim \mathcal{N}(x;\mu, \sigma^2)$ 的样本可以重写为：$$\begin{align*}
    x &= \mu + \sigma\epsilon \quad \text{with } \epsilon \sim \mathcal{N}(\epsilon; 0, I)
\end{align*}$$
换句话说，任意高斯分布都可以解释为标准高斯分布（其中 $\epsilon$ 是一个样本），通过加法将其均值从零移动到目标均值 $\mu$，并通过目标方差 $\sigma^2$ 拉伸其方差。因此，通过重参数化技巧，从任意高斯分布中进行采样可以通过从标准高斯分布中采样，将结果按目标标准差进行缩放，并按目标均值进行平移来实现。

在变分自编码器（VAE）中，每个 $z$ 都是输入 $x$ 和辅助噪声变量 $\epsilon$ 的确定性函数计算得出的：$$\begin{align*}
    z &= \mu_{\phi}(x) + \sigma_{\phi}(x)\odot\epsilon \quad \text{with } \epsilon \sim \mathcal{N}(\epsilon;0, \textbf{I})
\end{align*}$$
训练完变分自编码器（VAE）后，可以通过直接从潜在空间 $p(z)$ 采样，再通过解码器运行来生成新数据。当潜在变量 $z$ 的维度低于输入 $x$ 时，变分自编码器尤为有趣，因为此时我们可能正在学习紧凑而有用的表征。此外，当学习到具有语义意义的潜在空间时，可以在将潜在向量传递给解码器之前对其进行编辑，从而更精确地控制生成的数据。

### 2.3 分层变分自编码器

分层变分自编码器（HVAE）是 VAE 的泛化形式，其潜变量可扩展为多层次结构。在该框架下，潜变量本身被解释为由更高层级、更抽象的潜变量生成。直观而言，正如我们将三维观测物体视为由更高层抽象潜变量生成，柏拉图洞穴中的人们也将三维物体视为生成其二维观测的潜变量。因此，从洞穴居民的视角来看，他们的观测可被视为由深度为二（或更深）的潜变量层次结构所建模。

在一般的具有 $T$ 个层级的分层变分自编码器（HVAE）中，每个潜在变量都可以依赖于所有先前的潜在变量，而在这项工作中，我们专注于一种特殊情况，我们称之为马尔可夫分层变分自编码器（MHVAE）。在 MHVAE 中，生成过程是一个马尔可夫链；也就是说，层级结构中的每一次转移都是马尔可夫性质的，其中解码每个潜在变量 $z_t$ 仅依赖于前一个潜在变量 $z_{t+1}$。直观且形象地来看，这可以简单地理解为将多个变分自编码器（VAE）堆叠在一起，如图 2 所示；另一个适合描述该模型的术语是递归变分自编码器（Recursive VAE）。从数学角度，我们将马尔可夫层级变分自编码器（HVAE）的联合分布和后验分布表示为：$$\begin{align}
    p(x, z_{1:T}) &= p(z_T)p_{\theta}(x|z_1)\prod_{t=2}^{T}p_{\theta}(z_{t-1}|z_{t}) \\
    q_{\phi}(z_{1:T}|x) &= q_{\phi}(z_1|x)\prod_{t=2}^{T}q_{\phi}(z_{t}|z_{t-1})
\end{align}$$
然后，我们可以轻松地将 ELBO 扩展为：$$\begin{align}
\log p(x) &= \log \int p(x, z_{1:T}) dz_{1:T}         && \text{(Apply Equation 1)}\\
&= \log \int \frac{p(x, z_{1:T})q_{\phi}(z_{1:T}|x)}{q_{\phi}(z_{1:T}|x)} dz_{1:T}         && \text{(Multiply by 1 = $\frac{q_{\phi}(z_{1:T}|x)}{q_{\phi}(z_{1:T}|x)}$)}\\
&= \log \mathbb{E}_{q_{\phi}(z_{1:T}|x)}\left[\frac{p(x, z_{1:T})}{q_{\phi}(z_{1:T}|x)}\right]         && \text{(Definition of Expectation)}\\
&\geq \mathbb{E}_{q_{\phi}(z_{1:T}|x)}\left[\log \frac{p(x, z_{1:T})}{q_{\phi}(z_{1:T}|x)}\right]         && \text{(Apply Jensen's Inequality)}
\end{align}$$
[图 2]：一个具有 $T$ 层潜在变量的马尔可夫分层变分自编码器。其生成过程被建模为马尔可夫链，其中每个潜在变量 $z_t$ 仅由前一个潜在变量 $z_{t+1}$ 生成。


然后，我们可以将联合分布（方程 20）和后验分布（方程 21）代入方程 25，从而得到另一种形式：
