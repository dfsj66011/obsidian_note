...
GPT-4o 通过在自回归解码流程中实现原生图像生成功能，在多模态生成建模领域实现了重大突破。与ChatGPT等依赖离散模态专用API（如外部调用DALL·E模型生成图像）的先前系统不同，GPT-4o将图像合成直接整合到其Transformer主干网络中。该模型通过将图像表示为连续值潜在补丁序列，采用基于扩散的去噪方法进行解码，并在单一输出序列中与文本标记自然交织，从而实现了这一创新。
...
- Latent patch vectors represent image data, not as pixel arrays, but as compressed, continuous latent vectors in ℝd, generated using a VAE or Latent Diffusion model.
...
- Each token (whether text or image) is embedded into a shared hidden representation space ℝd so that all subsequent transformer operations can proceed uniformly.
...
- n: number of patches (e.g., for a 16 × 16 grid, n=256)
...
- At training and inference time, each image is converted into a sequence of latent vectors via the encoder:
    ...
    z=VAEenc(x),with z∈ℝn×d
    
    - where:
        - x∈ℝH×W×3: original RGB image (e.g., 256×256).
...
- After generation, the latent representation is decoded back into pixel space using the decoder:
    ...
    - where:
        ...
        - VAEdec: decoder (typically a U-Net or CNN-based upsampler).
...
- To implement the hybrid mask effectively:
    ...
    - This mask is injected into the standard self-attention mechanism before softmax: Attention(Q,K,V)=softmax(QKTdk√+M)V
...
- GPT-4o’s total loss is computed as a simple linear combination of the language modeling loss and diffusion loss:
    ...
    - where:
        - LM: language modeling loss for discrete tokens.
        - DDPM: denoising score-matching loss for image patches.
        - λ: weighting coefficient to balance the two losses (often tuned via validation or set to 1.0).
...
- The LM loss used for text is standard autoregressive next-token prediction:
    ...
    - where:
        ...
        - yi: the token to predict at position i.
        - y<i: the prefix up to, but not including, i.
        - Pθ: the model’s predicted probability distribution, parameterized by θ.
        - 𝔼y: the expectation over the training corpus.
...
- The forward process gradually adds Gaussian noise to a clean latent vector:
    ...
    - where:
        - x0: clean latent image patch.
        - xt: noisy version of x0 at timestep t.
        - α¯t=∏ts=1(1−βs): cumulative product of the noise schedule.
        - ϵ∼(0,I): sampled Gaussian noise.
- This process is implemented efficiently using the closed-form reparameterization trick that allows sampling xt in a single step from x0.
...
- The model learns to predict the noise added to each patch via a regression loss:
    ...
    DDPM=𝔼x0,t,ϵ[‖ϵ−ϵθ(xt,t,c)‖2]
    
    - where:
        - xt: the noised patch latent at timestep t.
        - ϵ: the actual Gaussian noise added to x0.
        - ϵθ(xt,t,c): model’s prediction of the noise, conditioned on:
            - t: diffusion step.
...
- Transfusion and Rolling Diffusion both use **cosine noise schedules** or **linear beta schedules** to determine βt, which controls how much noise is added at each step.
...
- During diffusion training:
    ...
    - The model predicts noise vectors ϵ, rather than predicting the patch directly.
- Additional training details:
    ...
    - Teacher forcing is used: the target for each patch is the actual noise ϵ added during the forward step.
...
- However, hyperparameter tuning (especially for λ) remains critical, as over-weighting the image loss can reduce text fluency, and vice versa.
...
- The traditional DDPM sampling procedure expects to denoise an entire latent grid z∈ℝn×d in parallel across all patches at every timestep t∈{T,T−1,...,1}. While effective for static image generation, this method is incompatible with:
...
- In Rolling Diffusion, the noise schedule is made spatially dependent. Instead of a global timestep t being applied uniformly across all patches, a position-adjusted local timestep tk is computed for each patch k:
    ...
    - where:
        ...
        - tk: adjusted timestep for patch k — later patches receive more noise.
- This creates a rolling schedule, where denoising sweeps over the image spatially, producing cleaner outputs as we progress.
    
- The following figure ([source](https://arxiv.org/pdf/2402.09470)) visualizes the global rolling diffusion process and its local time reparameterizatiod in Rolling Diffusion. The global diffusion denoising time t (vertical axis) is mapped to a local time tk for a frame k (horizontal axis). The local time is then used to compute the diffusion parameters αtk and σtk . On the right, we show how the same local schedule can be applied to each sequence of frames based on the frame index w. The nontrivial part of sampling the generative process only occurs in the sliding window as it gets shifted over the sequence.
...
- The reverse step in Rolling Diffusion is a modification of the standard DDPM reverse equation, adapted to patch-wise sequential generation. For a given patch xkt, the denoised value is computed as:
    
    xktk−1=1αtk‾‾‾√(xktk−1−αtk1−α¯tk‾‾‾‾‾‾‾√ϵθ(xktk,tk))+σtkz
    
    - where:
        - xktk: current noisy latent patch at local time tk.
        - αtk,α¯tk: noise schedule parameters for local time tk.
        - ϵθ(⋅): model’s predicted noise for patch k at that timestep.
        - σtk: scale of Gaussian noise reintroduced at that step.
        - z∼(0,I): noise sampled per step.
...
- During training, a corresponding localized loss function is used. The model learns to predict noise for only the patches in the current window:
    ...
    win,θ=∑k∈win(t)a(tk)‖‖xk0−x̂ k0‖‖2
    
    - where:
        - win(t): the window of patches being denoised at time t.
        - a(tk): time-dependent weighting function (e.g., based on signal-to-noise ratio).
        - xk0: the original clean patch.
        - x̂ k0: reconstruction from model prediction at local time tk.
...
- These are combined with a scalar λ to control balance. In Transfusion, this was statically set, but in practice could be:
