...
所以我们将其作为输入, 并像这样从这个分布中采样:x 等于均值加上标准差乘以噪声. 最后, 还有另一个步骤, 我们需要用一个常数来缩放输出. 这个常数我在原始仓库中找到了, 所以我只是在这里写下来, 没有解释为什么, 因为我实际上也不知道. 它只是 个他们在最后使用的缩放常数. 我不知道这是否出于历史原因, 因为他们使用了一些之前有这个常数的模型 或者他们出于某种特定原因引入了它, 但我在原始仓库中看到了这个常数, 实际上, 如果你检查稳定扩散模型的原始参数, 也有这个常数. 所以我也在输出上用这个常数进行缩放, 然后我们返回 x.
