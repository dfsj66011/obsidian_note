...
- [Reinforcement Learning for Agents](https://aman.ai/primers/ai/agents/#reinforcement-learning-for-agents)
...
AI 代理是自主系统，它将自主框架的决策和行动导向能力与 LLMs 的自然语言处理和理解优势相结合。LLM 在代理中充当“大脑”，负责解释语言、生成响应和规划任务，而代理框架则使这些任务能够在特定环境中执行。两者结合使代理能够参与目标导向的工作流程，其中 LLMs 提供战略洞察、问题解决和适应性，以在最少人为干预的情况下实现成果。

AI 代理将 LLMs 作为核心推理引擎，实现实时决策、任务优先级排序和动态适应。实践中，AI 代理通过循环流程运作：LLM 分析输入信息→制定可执行计划→与 API、网络工具或嵌入式传感器等模块化系统协同执行具体步骤。在此过程中，LLM 能持续保持上下文关联并进行迭代，根据环境反馈或前序步骤结果调整行动。这种集成系统使 AI 代理能以日益精密的水平处理复杂的多阶段任务，推动金融、软件工程和科学发现等领域的创新。


## 二、“代理式 AI 时刻”

许多人都在 ChatGPT 发布时经历了一个关键的 “AI 时刻”——那时系统的能力超出了他们的预期。这种现象通常被称为 “ChatGPT 时刻”，它概括了人工智能表现超出预期限制的互动场景，展示了非凡的能力、创造力或问题解决技巧。

类似于 “ChatGPT 时刻”，智能体也迎来了“自主 AI 时刻”——即 AI 系统展现出意料之外的自主性和应变能力的实例。一个著名案例 ([source](https://www.deeplearning.ai/the-batch/how-agents-can-improve-llm-performance/)) 涉及一款为在线研究开发的 AI 智能体。在实时演示中，该智能体使用主网络搜索工具时遭遇访问频次限制错误。它并未因此失败，而是流畅地切换至备用工具（维基百科搜索功能）有效完成了任务。这一未经预设的应变行为，彰显了智能体独立适应突发状况的能力——这种标志性的自主规划与动态问题解决特质，正凸显出 AI 智能体在复杂现实场景中的应用潜力。

## 三、代理工作流

目前 LLMs 的应用大多采用零样本模式，即模型逐个 token 生成响应，而不对初始输出进行重新审视或优化。这种方式类似于要求某人一次性完成一篇文章而不做任何修改。尽管 LLMs 在这些限制下表现非常出色，但一种主动的、迭代的工作流程往往能产生更高质量的结果。
...
这种结构化、类似人类的优化过程使 AI 代理能够相比单次处理方式，产生更稳健且细致的输出。

## 四、工作流 vs. 代理

“代理”一词可以有多种解释。一些人将代理定义为完全自主的系统，能够长期独立运行，并使用各种工具完成复杂任务。另一些人则认为代理是遵循预设工作流程的规定性系统。

根据 [Anthropic](https://www.anthropic.com/engineering/building-effective-agents) 的分类，两者都属于代理系统，但要区分两种关键架构：

- **工作流**：通过预定义的代码路径协调 LLM 和工具以完成任务的一套系统。
- **智能体**：LLM 动态指导自身流程的系统，能够自主决定如何使用工具和管理任务

通过运用这些主动式方法，人工智能系统能够超越一次性响应，以迭代和智能的方式不断优化其输出结果。

## 五、Agent 框架

代理框架为组织 AI 代理的核心组件提供了结构化、模块化的设计。通过整合关键组件，每个组件都有明确的角色，共同实现无缝任务执行，从而实现高效、自适应的交互。

![|400](https://aman.ai/primers/ai/assets/agents/coreAI.jpg)

上图[(source)](https://developer.nvidia.com/blog/introduction-to-llm-agents/) 展示了一个典型的端到端智能体流水线架构。接下来，我们将详细探讨每个组件，以理解 AI 智能体的技术运作原理。

### 5.1 Agent Core (LLM)

在智能代理的核心部分，Agent Core 作为主要决策引擎运作，其中运用了 OpenAI 的 GPT-4 来处理高级推理和动态任务管理。该组件包括：

* 决策引擎：分析输入数据、记忆和目标，生成合适的响应。
* 目标管理系统：根据任务进展持续更新代理的目标。
* 集成总线：管理内存、工具和规划模块之间的信息流，确保数据交换的连贯性。

Agent Core 利用 LLM 的能力完成任务，根据需要生成新任务，并根据不断变化的任务上下文动态调整优先级。

### 5.2 记忆模块

记忆是框架的基本组成部分，向量数据库（如 Pinecone、Weaviate、Chroma 等）为任务相关数据提供了强大的存储和检索机制。内存模块通过以下方式增强了代理的情境意识和任务相关性：

* 短期记忆（STM）：管理临时数据以满足即时任务需求，存储在易失性结构（如堆栈或队列）中，以支持快速访问和频繁清除。
...
- For a detailed discourse on Reinforcement Learning for Agents please refer our [Reinforcement Learning for Agents](https://aman.ai/primers/ai/RL-for-agents) primer.
...
![](https://aman.ai/primers/ai/assets/agents/agent_ecosystem_architecture.jpg)
