...
2. **Compute Gini Impurity for the Parent Node:**
    ...
    Gini=1−(p2A+p2B)=1−(0.62+0.42)=1−(0.36+0.16)=0.48Gini=1−(pA2+pB2)=1−(0.62+0.42)=1−(0.36+0.16)=0.48
...
- Once the new tree is trained, the model is updated by adding a scaled version of this tree’s predictions to the current model’s predictions. The scaling factor (often denoted by a learning rate, ηη) controls the contribution of each tree to the final model, helping to prevent overfitting:
...
- Bias refers to the error introduced by approximating a real-world problem with a simplified model. High bias typically results from underfitting. Bias2=(𝔼[f̂ (x)]−f(x))2Bias2=(E[f^(x)]−f(x))2 where f̂ (x)f^(x) is the predicted output and f(x)f(x) is the true output.
    
- Variance refers to the error introduced by the sensitivity of the model to small fluctuations in the training data. High variance is associated with overfitting. Variance=𝔼[(f̂ (x)−𝔼[f̂ (x)])2]Variance=E[(f^(x)−E[f^(x)])2]
...
- At iteration mm, the weak learner hm(x)hm(x) is trained to minimize the weighted error: ϵm=∑ni=1w(m)i𝕀(yi≠hm(xi))∑ni=1w(m)iϵm=∑i=1nwi(m)I(yi≠hm(xi))∑i=1nwi(m) where w(m)iwi(m) are the weights on samples, 𝕀I is the indicator function, yiyi is the true label, and hm(xi)hm(xi) is the weak learner’s prediction.
    
- The weight of each weak learner is computed as: αm=12ln(1−ϵmϵm)αm=12ln⁡(1−ϵmϵm)
    
- Misclassified samples have their weights updated to: w(m+1)i=w(m)iexp(αm𝕀(yi≠hm(xi)))wi(m+1)=wi(m)exp⁡(αmI(yi≠hm(xi)))

##### High Bias in AdaBoost

- AdaBoost can have higher bias than gradient boosting in certain scenarios, particularly because:
    - **Weak Learners (Shallow Decision Stumps):** AdaBoost often uses very simple learners like decision stumps (one-level decision trees). While these are easy to interpret and computationally efficient, they are inherently biased due to their simplicity, limiting the complexity of the ensemble. Weak learners like decision stumps (hm(x)hm(x)) can lead to underfitting because their decision boundaries are too simple: hm(x)=𝕀(xj≥t)hm(x)=I(xj≥t)
        - where xjxj is a feature and tt is a threshold. These stumps inherently introduce bias as they cannot model complex relationships.
    - **Sensitivity to Noise:** AdaBoost can struggle with noisy data, as it focuses increasingly on harder-to-classify samples. This might lead to a situation where the model does not fit the underlying patterns well, contributing to higher bias in such cases. H(x)=sign(∑Mm=1αmhm(x))H(x)=sign(∑m=1Mαmhm(x))
        - If hm(x)hm(x) are too simple, the ensemble fails to approximate f(x)f(x) accurately, leading to higher bias.
...
- Gradient boosting is a more generalized approach to boosting:
    - Gradient boosting minimizes a differentiable loss function L(y,ŷ )L(y,y^) (e.g., mean squared error for regression or log loss for classification) using gradient descent. At each iteration, a new weak learner is trained to reduce the residuals (negative gradient of the loss): r(m)i=−∂L(yi,ŷ (m−1))∂ŷ (m−1)ri(m)=−∂L(yi,y^(m−1))∂y^(m−1)
    - Each weak learner hm(x)hm(x) is trained to reduce the residual errors (gradient of the loss function) from the previous iteration, and the model is updated as: ŷ (m)(x)=ŷ (m−1)(x)+ηhm(x)y^(m)(x)=y^(m−1)(x)+ηhm(x)
...
- Gradient boosting typically has lower bias than AdaBoost because:
    - **Flexible Weak Learners:** It often uses more complex trees (e.g., trees with multiple levels, i.e., deeper decision trees (hm(x)hm(x))), which can capture intricate patterns in the data, reducing bias. hm(x)=∑Tj=1𝕀(x∈Rj)⋅vjhm(x)=∑j=1TI(x∈Rj)⋅vj
        - where RjRj are leaf regions and vjvj are predictions for each region. Deeper trees (T>1T>1) can better capture intricate patterns.
    - **Optimized Objective Function:** Gradient boosting directly optimizes a loss function, leading to a more systematic reduction in prediction error. Put simply, loss optimization directly reduces error: minh∑ni=1(r(m)i−h(xi))2minh∑i=1n(ri(m)−h(xi))2
...
- For decision stumps (T=1T=1):
    - AdaBoost ensembles are limited in their expressive power. The final model H(x)=sign(∑Mm=1αm⋅𝕀(xj≥t))H(x)=sign(∑m=1Mαm⋅I(xj≥t)) cannot fit complex patterns, leading to higher bias.

##### Gradient Boosting Flexibility

- For gradient boosting with deeper trees (T>1T>1) The final model ŷ (x)=∑Mm=1η⋅∑Tj=1𝕀(x∈Rj)⋅vjy^(x)=∑m=1Mη⋅∑j=1TI(x∈Rj)⋅vj is capable of capturing complex dependencies, reducing bias.

#### Summary of Bias Differences

|**Aspect**|**AdaBoost**|**Gradient Boosting**|
|---|---|---|
|Weak Learner Complexity|Often simple (e.g., stumps, i.e., T=1T=1).|Flexible; can use more complex trees (T>1T>1).|
...
Variance=𝔼[(f̂ (x)−𝔼[f̂ (x)])2]Variance=E[(f^(x)−E[f^(x)])2]

- When a boosted model overfits:
    1. Predictions f̂ (x)f^(x) vary significantly if the training data changes.
...
- Boosting frameworks (e.g., XGBoost, LightGBM) include various regularization mechanisms to prevent overfitting:
    - **Learning Rate (ηη):** Reduces the contribution of each weak learner: ŷ (m)(x)=ŷ (m−1)(x)+ηhm(x)y^(m)(x)=y^(m−1)(x)+ηhm(x)
...
1. **Initialize the Model**:
    - Start with an initial prediction, often the mean of the target variable for regression, or log odds for classification. F0(x)=argminc∑iL(yi,c)F0(x)=arg⁡minc∑iL(yi,c)
2. **Compute the Negative Gradient (Residuals)**:
    - At each iteration tt, calculate the negative gradient of the loss function with respect to the model’s predictions: r(t)i=−∂L(yi,F(xi))∂F(xi)∣∣∣F(xi)=Ft(xi)ri(t)=−∂L(yi,F(xi))∂F(xi)|F(xi)=Ft(xi)
...
3. **Fit a Weak Learner (Decision Tree)**:
    - Train a weak learner (e.g., a small decision tree) to predict the residuals. The tree’s predictions ht(x)ht(x) approximate the negative gradient: ht(x)≈r(t)iht(x)≈ri(t)
4. **Update the Model**:
    - Add the predictions of the new tree to the existing model, scaled by a learning rate (ηη): Ft+1(x)=Ft(x)+η⋅ht(x)Ft+1(x)=Ft(x)+η⋅ht(x)
5. **Repeat**:
...
3. **No Direct Parameter Updates**:
    - Unlike traditional gradient descent, there are no parameter updates like θt+1=θt−α⋅∇L(θt)θt+1=θt−α⋅∇L(θt). Instead, the “update” is the addition of a new tree that minimizes the residuals.
...
- **Purpose**: In GBDTs, the learning rate (ηη) scales the contribution of each weak learner (tree) to the overall model.
...
- **How It Works**:
    ...
    - The predictions from this weak learner are scaled by the learning rate before being added to the model: Ft+1(x)=Ft(x)+η⋅ht(x)Ft+1(x)=Ft(x)+η⋅ht(x) where:
...
- **How It Works**:
    - At each iteration, the model’s parameters ((\theta)) are updated in the direction of the negative gradient of the loss function: θt+1=θt−α⋅∇L(θt)θt+1=θt−α⋅∇L(θt) where:
        - (\alpha): Learning rate.
...
- **Initialization**:
    - **Equal Weights**: All data points start with equal weights. wi=1N,for i=1,2,…,Nwi=1N,for i=1,2,…,N where (N) is the total number of data points.
...
- **Initialization**:
    - **Global Prediction**: The model begins with a constant prediction, often the mean (for regression) or the log odds (for binary classification). For example:
        - **Regression:** ŷ (0)i=mean(y)y^i(0)=mean(y)
        - **Classification:** ŷ (0)i=logit(mean(y))y^i(0)=logit(mean(y))
    - **Residual Calculation**: The first tree is trained on the **residuals** (errors) between the initial prediction and the actual targets: ri=yi−ŷ (0)iri=yi−y^i(0)
...
- In GBDTs, a small learning rate ηη regularizes the model by:
...
- In GBDTs, the goal is to minimize a loss function L(y,ŷ )L(y,y^), where:
    - yy is the true target,
    - ŷ y^ is the model’s prediction.
- At each boosting iteration tt, a new tree ft(x)ft(x) is added to the ensemble to reduce the residual errors of the previous predictions. The prediction at iteration tt is:
    
    ŷ t=ŷ t−1+ηft(x)y^t=y^t−1+ηft(x)
    
    - where:
        - ŷ t−1y^t−1 is the prediction from the previous t−1t−1 iterations,
        - ft(x)ft(x) is the output of the new tree at step tt,
        - ηη is the **learning rate**, a scalar controlling the contribution of ft(x)ft(x).

#### Effect of Small Learning Rate

- A small ηη reduces the impact of each tree ft(x)ft(x), ensuring that the model progresses gradually toward the optimal solution. This has regularization effects in several ways:

##### Slower Overfitting to Training Data

- If ηη is too large, the model can quickly fit to noise in the training data, especially if the trees ft(x)ft(x) are deep and have high capacity. A small ηη ensures the updates are smaller, preventing overly aggressive fitting.
    
- For example, consider that the residuals rtrt are defined as follows: rt=−∂L(y,ŷ t−1)∂ŷ t−1rt=−∂L(y,y^t−1)∂y^t−1
- In this case, the tree ft(x)ft(x) fits these residuals. The learning rate ηη scales the residual adjustment: ŷ t=ŷ t−1+ηrty^t=y^t−1+ηrt
- A small ηη effectively reduces the impact of residual errors caused by noise, acting as a regularizer.

##### Promotes Ensemble Diversity

- A small ηη requires more boosting iterations (more trees) to achieve the same level of loss reduction. Each individual tree ft(x)ft(x) contributes less to the final prediction, making the ensemble more diverse. This diversity can prevent overfitting, as no single tree dominates the model.

##### Encourages Smooth Loss Optimization

- In GBDTs, the optimization of the loss function is iterative. A small ηη ensures that the loss is minimized more smoothly, avoiding abrupt changes that could lead to sharp decision boundaries (analogous to sharp minima in neural networks).
    
- Let LtLt denote the loss after tt-th iteration:
    

Lt=Lt−1+η⋅∑i=1n∂L(yi,ŷ t−1)∂ŷ t−1Lt=Lt−1+η⋅∑i=1n∂L(yi,y^t−1)∂y^t−1

- With a small ηη, the updates to LtLt are smaller, ensuring smoother convergence and reducing the likelihood of overfitting.

#### Illustrative Example

- Suppose the goal is to minimize mean squared error (MSE):

L(y,ŷ )=1n∑i=1n(yi−ŷ i)2L(y,y^)=1n∑i=1n(yi−y^i)2

- The gradient for each sample ii is:

∂L(yi,ŷ t−1)∂ŷ t−1=−2(yi−ŷ t−1)∂L(yi,y^t−1)∂y^t−1=−2(yi−y^t−1)

- At each iteration tt, the residuals rt=−2(yi−ŷ t−1)rt=−2(yi−y^t−1) are used to fit ft(x)ft(x), and the predictions are updated as:

ŷ t=ŷ t−1+ηft(x)y^t=y^t−1+ηft(x)

- A small ηη reduces the step size toward minimizing the residuals, effectively regularizing the updates.

#### Trade-offs

- **Small ηη:** Slower convergence, but better generalization and reduced overfitting.
- **Large ηη:** Faster convergence, but higher risk of overfitting, especially if the model capacity is high.
- To balance this, practitioners often use a small learning rate with a larger number of boosting iterations TT.
...
- Each weak learner ht(x)ht(x) produces a prediction, and its weight αtαt reflects its importance in the ensemble. The final prediction H(x)H(x) is a weighted vote of all learners:
    
- For classification:
    
    H(x)=sign(∑Tt=1αtht(x))H(x)=sign(∑t=1Tαtht(x))
    
    - where:
        - ht(x)∈{−1,1}ht(x)∈{−1,1}: Prediction of the tt-th weak learner.
        - αtαt: Weight of the tt-th weak learner, calculated based on its error rate.

##### Weight Calculation

- The weight αtαt for each learner is calculated based on its error rate εtεt:
    
    αt=12ln(1−εtεt)αt=12ln⁡(1−εtεt)
    
    - where:
        - εtεt: Error rate of the tt-th weak learner, defined as: εt=∑ni=1wi⋅𝟙(ht(xi)≠yi)∑ni=1wiεt=∑i=1nwi⋅1(ht(xi)≠yi)∑i=1nwi
        - wiwi: Weight of the ii-th training sample.
        - 𝟙(ht(xi)≠yi)1(ht(xi)≠yi): Indicator function, 1 if ht(xi)≠yiht(xi)≠yi, otherwise 0.

##### Example: Binary Classification

###### Dataset

|**Sample**|**<br><br>xx<br><br>**|**<br><br>yy<br><br>(True Label)**|
|---|---|---|
|1|A|11|
|2|B|−1−1|
|3|C|11|
...
- h1(x)h1(x): Learner 1 predicts [1,−1,−1,−1][1,−1,−1,−1]
- h2(x)h2(x): Learner 2 predicts [1,−1,1,1][1,−1,1,1]

###### Step 1: Compute Error Rates

- Assume initial weights wi=1n=0.25wi=1n=0.25 for all samples:
    - For h1(x)h1(x), errors occur on samples 3 and 4: ε1=0.25+0.25=0.5ε1=0.25+0.25=0.5
    - For h2(x)h2(x), errors occur on sample 4: ε2=0.25ε2=0.25

###### Step 2: Compute Weights for Learners

α1=12ln(1−0.50.5)=0α1=12ln⁡(1−0.50.5)=0

α2=12ln(1−0.250.25)=0.69α2=12ln⁡(1−0.250.25)=0.69

###### Step 3: Final Weighted Prediction

- Each learner contributes to the final vote:

H(x)=sign(α1h1(x)+α2h2(x))H(x)=sign(α1h1(x)+α2h2(x))

- Predictions:
    - h1(x)h1(x): [1, -1, -1, -1], weighted by α1=0α1=0.
    - h2(x)h2(x): [1, -1, 1, 1], weighted by α2=0.69α2=0.69.
- Final weighted vote:

H(x)=sign(0⋅h1(x)+0.69⋅h2(x))=sign([1,−1,1,1])H(x)=sign(0⋅h1(x)+0.69⋅h2(x))=sign([1,−1,1,1])

- Final prediction:

H(x)=[1,−1,1,1]H(x)=[1,−1,1,1]
...
- In regression, weighted voting is a weighted sum of the predictions from all weak learners:
    ...
    H(x)=∑Tt=1αtht(x)H(x)=∑t=1Tαtht(x)
    
    - where:
        - ht(x)ht(x) is the prediction of the tt-th weak learner.
        - αtαt is the weight of the tt-th learner, reflecting its accuracy.

##### Weight Calculation

- Weights αtαt can be based on the residual errors or another measure of the learner’s effectiveness. For example:
    
    αt=ln(1errort)αt=ln⁡(1errort)
    
    - where errorterrort is the mean squared error (MSE) of the learner.

##### Example: Regression

###### Dataset

|**Sample**|**<br><br>xx<br><br>**|**<br><br>yy<br><br>(True Value)**|
|---|---|---|
|1|A|3|
|2|B|7|
|3|C|2|
...
- h1(x)h1(x): Learner 1 predicts [3,6,2,5][3,6,2,5], MSE1=0.5MSE1=0.5
- h2(x)h2(x): Learner 2 predicts [2.5,7,1.5,6][2.5,7,1.5,6], MSE2=0.25MSE2=0.25

###### Step 1: Compute Weights

α1=ln(10.5)=ln(2)≈0.693α1=ln⁡(10.5)=ln⁡(2)≈0.693

α2=ln(10.25)=ln(4)≈1.386α2=ln⁡(10.25)=ln⁡(4)≈1.386

###### Step 2: Final Weighted Prediction

- Combine predictions from the learners using their weights:

H(x)=α1h1(x)+α2h2(x)α1+α2H(x)=α1h1(x)+α2h2(x)α1+α2

- Predictions:
    - h1(x)h1(x): [3, 6, 2, 5], weighted by α1=0.693α1=0.693.
    - h2(x)h2(x): [2.5, 7, 1.5, 6], weighted by α2=1.386α2=1.386.
- Final weighted prediction for each xx:

H(x)=(0.693⋅[3,6,2,5])+(1.386⋅[2.5,7,1.5,6])0.693+1.386H(x)=(0.693⋅[3,6,2,5])+(1.386⋅[2.5,7,1.5,6])0.693+1.386

H(x)=[2.079,4.158,1.386,3.465]+[3.465,9.702,2.079,8.316]2.079H(x)=[2.079,4.158,1.386,3.465]+[3.465,9.702,2.079,8.316]2.079

H(x)=[2.73,7.01,1.76,6.01]H(x)=[2.73,7.01,1.76,6.01]
