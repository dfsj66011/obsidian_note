...
- 适配器层（通常称为“适配器”）仅在预训练模型中添加极少的额外参数。这些适配器被插入到网络的现有层之间。
- 适配器（Adapters）是一种参数高效微调（PEFT）技术，研究表明，与仅微调顶层相比，它能实现相近的性能，同时所需参数量减少两个数量级。
- 基于适配器的调优只需在预训练网络的各层之间插入称为“适配器模块”的新模块。
- 下图（来源）阐释了 Transformer 模块的这一概念。

![](https://aman.ai/primers/ai/assets/parameter-efficient-fine-tuning/adapterraschka.png)

- 在微调过程中，仅更新这些适配器层的参数，而原始模型参数保持固定。这会产生一个额外参数较少且针对特定任务的模型。
