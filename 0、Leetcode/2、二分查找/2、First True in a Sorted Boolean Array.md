...
你有一叠按固定顺序排列的报纸。每份报纸都有一个阅读时间。你必须将这叠报纸分成连续的几部分，并将每一部分分配给一个工人。工人们会并行阅读他们被分配到的报纸。

限制条件：不能重新排列报纸的顺序。如果将位置 1、2、3 的报纸分配给工人 A，就不能再将报纸 2 分配给工人 B。每个工人必须从原始堆栈中分配一个连续的报纸块。

找出阅读所有报纸所需的最短时间。由于工人们是并行阅读的，总时间等于最慢的工人所花费的时间。

例如，报纸列表为 `[7,2,5,10,8]`，有 2 名工人时，你可以将 `[7,2,5]` 分配给工人 A（总计 14 分钟），`[10,8]` 分配给工人 B（总计 18 分钟）。工人 B 最后完成，所以答案是 18 分钟。


### 解题思路：

1. **识别单调性:** 假设我们有一个最大阅读时间 `X`。如果我们可以用 `k` 个工人阅读所有报纸，并且每个工人的阅读量都不超过 `X`，那么我们就可以用 `k` 个工人完成阅读量不超过 `X` 的任务。

2.  **定义搜索空间:**
    *  最小可能的最大阅读量 (low)：单份报纸的阅读时间。因为每个工人至少要阅读一份报纸。
    *  最大可能的最大工作量 (high): 所有报纸的总阅读时间，只有他自己一个人

3.  辅助函数：接收一个假设的最大阅读量 `X`，判断是否可能用 `k` 个工人完成阅读，工作方式是：遍历报纸，尽可能地让每个工人承担不超过 `X` 的最大的工作量，并计算所需的工人数量。

4.  二分搜索过程：在 `[low, high]` 范围内进行二分搜索。

```python
def check_k(arr: List[int], x: int, k: int) -> bool:  
    num_worker = 1               # 至少需要一个
    acc = 0                      # 累计从 0 开始
    for i in range(len(arr)):  
        if acc + arr[i] <= x:   # 累计小于目标，继续累计
            acc += arr[i]  
        else:  
            num_worker += 1     # 超过目标，重新累计，工人数量 +1
            acc = arr[i]  
  
    return True if num_worker <= k else False  
...
def find_first_occurrence(arr: list[int], k: int) -> int:  
    ...
    while low <= high:  
        mid = low + (high - low) // 2  
        if check_k(arr, mid, k):          # 说明当前工作量可以，但可能偏大
            result = mid           
            high = mid - 1                # 尝试找个更小的
        else:  
            low = mid + 1                 # 当前工作量不行，尝试让每个人再多读一些
