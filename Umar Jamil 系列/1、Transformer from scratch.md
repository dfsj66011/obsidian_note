...
WSL: Ubuntu > TIME LNE a new matrix which is called q prime in my slides i just call it query here 3. 10. 6 (trans
ENG
5/22/2023
7:04 PM
...
WSL: Ubuntu
> TIMELNE
Ln 84. Col9 Spaces 4 UTF-8 LF ( Python3. 10. 6(trans fo
ENG
7:04 PM
5/22/2023
V, mask ):
> OUT U NE 我们对键和值做同样的事情,
> WSL : Ubuntu > TIME LNE We do the same with the keys and the same with the values. s u Q wto n xio6rtrars
ENG
5/22/2023
7:04 PM
def forward (self, q, k, v, mask ):
...
> OUTUNE
> WSL: Ubuntu > TIME LNE
Ln 85, Col15 Spaces:4 UTF-8 LF {↓ Python 3. 10. 6(transfc
ENG
5/22/2023
7:04 PM
def forward (self, q, k, v, mask):
query = self. w_q(q)
key=self. w_k( 让我也写下维度.
> OUT UNE value = self. w
WSL : Ubuntu > TIME LNE Let me also write the dimensions.
Ln84, Col26 Spaces4 UTF-8 LFPython 3. 10. 6(transformer :co
NG
5/22/2023
7:04 PM
82
83
sk): 所以我们从批量序列长度到d模型. 通过这个乘法,
> OUTUNE
WSL: Ubuntu > TIME LNE So we are going from Pytho 3. 10. 6 (tra
7:04 PM
P
5/22/2023
82
83 所以我们从批量序列长度到d模型. 通过这个乘法,
q(g)#( Batc H
> OUTUNE
> WSL: Ubuntu > TIME UNE batch sequence length to d model with this multiplication we are going to another ENG
5/22/2023
7:04 PM
82
w_q(g)#( Batch, Seq_ Len, d_mod
> OUTUNE 我们将得到另一个矩阵, 它是批量序列长度和d模型
> WSL : Ubuntu > TIME LNE batch sequence length to d model with this multiplication we are going to another 7:04 PM
ENG
5/22/2023
82
83
w_q(g)#( Batch, Seq_ Len, d_
> OUTUNE 我们将得到另一个矩阵, 它是批量序列长度和d模型
> WSL : Ubuntu > TIME LNE matrix which is batch sequence length and d model and you can see. that from the 7:04 PM
5/22/2023
seq sequence length = size of the embedding vector yti Head ( Q, K, V)= Concat(head.. headn)wo
number of heads 你可以在幻灯片中看到
=d model /h slides so when we
seq sequence length =size of theembedd ing 所以当我们做序列乘以d模型再乘以d模型, 我们
.. headn)wo
number ofheads
dk-d
=dmodel /h slides so when we
seq sequence length =size of the embedding v 所以当我们做序列乘以d模型再乘以d模型, 我们
.. head n)wo
number of heads dk-d
uence by dmodel multiplied by d model byd model we get a new matrix which has
模型;这对它们三个都是一样的,
self.
q(g) #( Batch, S eq_ Len, d_model
> OUTUNE
86
85
> WSL: Ubuntu > TIME LNE
...
> TIMELNE
all three of the m
Ln 83, Col 78 (8 selected)
ces4 UTF-8 LF Python 3. 10. 6(trans
ENG
7:04 PM
5/22/2023
def forward (self, q, k, v, mask):
query = self. w_q(q) #( Batch, Seq _ Len, d_model )
value = self. w_v(v)
key=self. w_k(k)
> OUTUNE
> WSL: Ubuntu > TIME LNE
8
Ln 83, Col31 (55 selected ) Spaces 4 UTF-8 LF{ Python3. 10. 6(transformer :conda)
7:04 PM
5/22/2023
82
83
forward (self, q, k, v, mask):
> OUTUNE 现在我们想做的是, 我们想将这个查询、键和值分成更小的
> TIMELNE
7:04 PM
forward (self, q, k, v, mask):
83
> OUTUNE 矩阵, 以便我们可以将每个小矩阵分配给不同的头. 让我们
wsu un ooa. now what we want to do is to we want to divide this query key and value into smaller > TIMELINE 人
ENG
5/22/2023
7:04 PM
82
forward(self, q, k, v, mask):
83
> OUT U NE 矩阵, 以便我们可以将每个小矩阵分配给不同的头. 让我们
> WSL : Ubuntu > TIME LNE matrices so that we can get give each small matrix to a different head so let's do it
/22/2023
7:05 PM
def forward (self, q, k, v, mask):
query = self. w_q(q)#( Batch, Seq _ Len, d_model )--> ( Batch, Seq _ Len, d_model )
85
86
del)
> OUTUNE
87
query|
> WSL: Ubuntu > TIME UNE matrices so that we can get give each small matrix to a different head so let's do it
7:05 PM
5/22/2023
82 forward (self, q, k, v, mask):
83
84
elf. v
> OUTUNE 我们将使用
pytorch 的view 方法进行分割, 这意味着我们保持
> WSL: Ubuntu
> TIMEUNE
wewil
7:05 PM
...
84
> OUT UNE 我们将使用
py torch 的view 方法进行分割, 这意味着我们保持
> WSL: Ubuntu
> TIMEUNE
1 A0
divide into using the view method of py torch which means that we keep the batch.
7:05 PM
ENG
/22/2023
82
forward(self, q, k, V, mask):
83
> OUT U NE 批量维度, 因为我们不想分割句子. 我们想将嵌入分割成
> WSL : Ubuntu > TIME UNE dimension because we don't want to split the sentence we want to split the embedding /22/2023
7:05 PM
28
def forward(self, q, k, v, mask):
query = self. w _q(q) #( Batch,
->( Batch, Seq _ Len, d_model )
( Batch, Seq_ Len, d_model )
86 各个部分
( Batch, Seq_ Len, d_model)
> OUT UNE 87
query=query. view(que
> WSL: Ubuntu > TIME LNE
5/22/2023
7:05 PM
28
lef
forward(self, q, k, v, mask):
query = self. w _q(q) #( Batch, Seq _ Len, d_model ) -->( Batch, Seq _ Len, d_model )
> OUTUNE
86
> TIMELINE
87
query = query. view (query. shape [e]]
> WSL : Ubuntu Ln 87, Col 42
...
forward(self, q, k, v, mask):
83
query =self. w_q(q)#( Batch, Seq_ Len,
> OUTUNE 我们也想保持第二个维度, 即序列, 因为我们不想分割它
> WSL: Ubuntu > TIMELINE we also want to keep the second dimension which is the sequence because we don't want /22/2023
7:05 PM
28
forward(self, q, k, v, mask): 还有第三个维度, 即 D模型
odel
del
> OUTUNE
86
> WSL: Ubuntu > TIMELINE we also want to keep the second dimension which is the sequence because we. don't. want.
5/22/2023
7:05 PM
82
forward(self, q, k, v, mask):
83
> OUTUNE 我们想将其分割成两个更小的维度, 即 H乘以 DK, 所以
WSL: Ubuntu
> TIMELNE
to split it and the third dimension so the D model we want to split it into two octra
7:05 PM
5/22/2023
forward (self, q, k, v, mask ):
query =self. w_q(q)#( Batch, Seq_ L e
> OUTUNE 我们想将其分割成两个更小的维度, 即 H乘以 DK, 所以
WSL: Ubuntu > TIME LNE
smaller
7:05 PM
...
82
forward(self, q, k, v, mask):
> OUTUNE 我们想将其分割成两个更小的维度, 目 即 H乘以 DK, 所以
> WSL: Ubuntu
> TIMELNE
dimensions which is H by DK so self. H self. DK as you remember DK. is. basically D. model.
7:05 PM
82
forward(self, q, k, v, mask):
83
...
e
elf. w_k(k)
self. H, self. DK. 如你所记, DK基本上是 D模型除以 H.
> OUTUNE > TIMELINE /22/2023
7:05 PM
83
82
forward(self, q, k, v, mask):
sel f. H, self. DK. 如你所记, DK基本上是 D模型除以 H.
84
self. w_k(k)
> OUTUNE
WSL: Ubuntu > TIME UNE divided by H so this multiplied by this gives you. D model ur & u
pto n
4. 10. 6 (tra
7:05 PM
ENG
...
forward(self, q, k, v, mask):
83
query =self. w _q(q)#( Batch, Seq_ L en, d_model) 所以这个乘以这个, 给你 D模型, 然后我们转置.
> OUTUNE
WSL: Ubuntu > TIME UNE divided by H so this multiplied by this gives you Dmodel
7:05 PM
5/22/2023
228
def forward(self, q, k, v, mask):
query = self. w_q(q)#( Batch, Seq_ Len)
key= self. w_k(k)#( Batch >( Batch, Seq _ Len, d_model )
86
value =self. w_v( 为什么要转置
Seq_ Len, d_model )
> OUTUNE
87
, self. d_k). trhnspd
WSL: Ubuntu > TIME LNE and then we transpose why do we transpose?
...
82
forward(self, q, k, v, mask):
83 因为我们更喜欢有边缘维度. 而不是作为第三个维度, 我们
> OUT U NE
> WSL : Ubuntu > TIME UNE because we prefer to have the edge dimension instead of being the third dimension we 人
5/22/2023
7:05 PM
83
82
forward(self, q, k, v, mask): 希望它成为第二个维度, 这样每个头都能看到整个句子.
> OUT U NE
WSL Ubuntu > TIMELINE want it to be the second dimension and this way each head will see all the sentence ENG
5/22/2023
7:06 PM
91
> OUTUNE 所以我们会看到这个维度. 所以序列长度乘以dk一一让我
WSL: Ubuntu > TIMELINE
so we'
Ln87, Col55 (11 selected ) Spaces 4 UTF-8 LF { Python 3. 10. 6 C
ENG
5/22/2023
7:06 PM
91
> OUTUNE 所以我们会看到这个维度. 所以序列长度乘以dk一一让我
WSL: Ubuntu00
> TIMELINE see this dimension so the sequence length by dk let me also write the comment here 7:06 PM 口
ENG
> OUT U NE
WSL: Ubuntu00
> TIMELNE
In87, Col9 Spaces :4 UTF-8 LF( Python3. 10. 6(transformer :conda)
5/22/2023
7:06 PM
91
> OUTUNE 边缘, 衰减, 然后通过使用转置, 我们将到批量边缘序列
WSL: Ubuntu00
> TIME UNE so we are going from batch sequence length de modal to batch sequence length edge
ENG
5/22/2023
7:06 PM
91
...
长度和衰减.
> OUT U NE
WSL : Ubuntu > TIMELINE and decay this is really important because we want es4 UTF-8 LF( Python3. 10. 6(transformer :conda)
5/22/2023
7:06 PM
91
> OUTUNE 这非常重要, 因为我们希望, 我们希望每个头都能观察到
WSL: Ubuntu > TIMELINE and decay this is really important because we want s ur: u I ption 口 人
/22/2023
7:06 PM
91
> OUTUNE 这非常重要, 因为我们希望, 我们希望每个头都能观察到
...
> WSL: Ubuntu
> TIMELNE
we want each batch have um we want each head to watch this stuff so the sequence 7:06 PM
ENG
5/22/2023
91
> OUTUNE 所以序列长度乘以dk, 这意味着每个头我们会看到完整的
> WSL: Ubuntu00
> TIMELINE we want each batch have um we want each head to watch this stuff so the sequence ENG
5/22/2023
7:06 PM
91
> OUTUNE 所以序列长度乘以dk, 这意味着每个头我们会看到完整的
> WSL: Ubuntu > TIMELINE length by dk which means that each head we will see the full sentence so each word in
/22/2023
7:06 PM
91
> OUTUNE 句子, 所以句子中的每个词, 但只是嵌入的一小部分.
> WSL: Ubuntu > TIMELINE the sentence Ln 87. Col 93 (12 selected )
5/22/2023
7:06 PM
91
> OUTUNE 句子, 所以句子中的每个词, 但只是嵌入的一小部分.
WSL: Ubuntu
> TIMELNE
but only a smaller part of the embedding we do the same thing for the key and the ENG
7:06 PM 人
P
5/22/2023
> OUT U NE 我们对键和值做同样的事情
> WSL : Ubuntu > TIME LNE but only a smaller part of the embedding we do the same thing for the key and the 5/22/2023
7:07 PM
> OUT U NE 我们对键和值做同样的事情
WSL : Ubuntu > TIME LNE value Ln 89, Col 9
ENG
7:07 PM
5/22/2023
> OUT U NE
WSL: Ubuntu
> TIMELNE
Ln89, Col9 Spaces 4 UTF-8 LF( Python3. 10. 6(transformer :conda)
7:07 PM
5/22/2023
...
93
94 这样我可以向你展示我们在哪里
> WSL: Ubuntu > TIME UNE okay now that we have these smaller matrices so let me go back. to the slide soican
ENG
5/22/2023
7:08 PM
value = value. view (value. shape [e], value. shape[1], self. h, self. d_k). transpose (1, 2)
...
> OUTUNE
94
> WSL: Ubuntu > TIME UNE show you where we are UTF-8 LFPython 3. 10. 6 (tran
ENG
5/22/2023
7:08 PM
seq sequence length = size of the embedding vector 这样我可以向你展示我们在哪里
Q. K, v)= Concat (head.. head )wo number of heads d-d
= dmodel /h show you where we are
seq sequence length size of the embedding 所以我们做了这个乘法, 我们得到了查询、键和值. 我们
ad n)w
number of head dk-d
seq sequence length size of the embed dir number of he
dk-d
so we did this multiplication we obtained guer y key and values we split into smaller
seq sequence length = size of the embedding vector 注意力.
Multi Head ( Q, K, V)= Concat (head,.. head n)wo
number of heads dk_d
matrices now we need to calculate the attention using this formula here before we can
value = value. view (value. shape [0], value. shape[1], self. h, self. d_k). transpose (1, 2)
23 注意力.
> OUTUNE
> WSL: Ubuntu > TIME UNE matrices now we need to calculate the attention using this formula here before we. can.
5/22/2023
7:08 PM
value = value. view (value. shape [θ], value. shape[1], self. h, self. d_k). transpose (1, 2)
91
92 在我们计算注意力之前, 让我们创建一个函数来计算扩展.
> OUTUNE
> TIME UNE
/22/2023
7:08 PM
...
92 在我们计算注意力之前, 让我们创建一个函数来计算扩展.
> OUTUNE
WSL: Ubuntu > TIME UNE calculate the attention let's create a function to calculate the extension soifwe
7:08 PM 口
/22/2023
91
sel f. d_k). transpose(1, 2)
> OUTUNE 所以如果我们创建一个新函数, 也可以在以后使用
WSL: Ubuntu > TIME LNE calculate the attention let's create a function to calculate the extension soifwe
ENG
/22/2023
7:08 PM
91
self. d_k). transpose(1, 2)
> OUTUNE 所以自注意力, 让我们将其定义为静态方法
WSL: Ubuntu
> TIMEUNE
2 A0
create a new function that can be used also later so self attention let's define it
5/22/2023
7:08 PM
90
91
self. d_k). transpose(1, 2)
> OU TUNE 所以自注意力, 让我们将其定义为静态方法
> WSL : Ubuntu > TIME UNE as a static method Ln81, Col1 S
4. 10. 6(trans Python 7:08 PM
ENG
5/22/2023
#( Batch, Seq _ Len, d_model )-->( Batch, Seq_ Len, h, d_k)-->( Batch, h, Seq_len, d_k)
...
WSL: Ubuntu > TIME LNE
1 A0
Ln82. Col 6 Spaces 4 UTF-8 LF( Python3. 10. 6(transformer :conda)
5/22/2023
7:08 PM
#( Batch, Seq_ Len,
91
self. h
> OUTUNE 所以静态方法基本上意味着你可以在没有这个类实例的情况下
> WSL: Ubuntu > TIME LNE
1 A0
So static method means basically that you can call this function without having an ce
ENG
7:08 PM
key = key. view (key. shape [θ],
query = query. view (qu
self. h,
self. h, self. d_k). transpose (1, 2)
self. d_k). transpose(1, 2)
value value. vi 调用这个函数
self. d_k). transpose (1, 2)
> OUTUNE
> WSL: Ubuntu
> TIMELNE
So static method means basically that you can call this function without having an o
ENG
/22/2023
7:08 PM
#( Batch, Seq _ Len, d_model )-->( Batch, Seq_ Len, h, d_k)-->( Batch, h, Seq_len, d_k)
...
9
Ln 66, Col 30 (23 selected ) Spaces 4 UTF-8 LF( Python3. 10. 6 (transformer :conda)
ENG
7:08 PM
5/22/2023
91
...
self. h,
self. d_k). transpose (1, 2)
> OUTUNE 你可以直接说多买注意力块点注意力, 而不需要这个类的
WSL: Ubuntu > TIMELINE 1 A0 You can just say multi head attention block dot attention instead of having an sct m
ENG
#( Batch, Seq_ Len,
91
query
self. h,
self. d_k). transpose (1, 2)
> OUTUNE 你可以直接说多头注意力块点注意力, 而不需要这个类的
> WSL: Ubuntu
> TIMELNE
...
> TIMEUNE
1 A0
Ln83, Col 26 Spaces4 UTF-8 LF( Python 3. 10. 6(transfc
7:08 PM
5/22/2023
#( Batch, Seq _ Len, d_model )
guer y = query. view (c
...
1 A0
9
Ln83, Col63 Spaces4 UTF-8 LF ↓ Python 3. 10. 6(transfo
ENG
5/22/2023
7:09 PM
value =value. view (value. shape [0], value. shape[1], self. h, self. d_k). transpose (1, 2) 我们做的是我们得到衰减.
> OUTUNE
WSL: Ubuntu
> TIMELNE
What we do is we get the decay.
Ln84. Col9 Spaces:4 UTF-8 LF ↓ Python 3. 10. 6(trans
ENG
5/22/2023
7:09 PM
zero Division Err on doc Pending e pre cation larning
> OUTUNE
989
WSL: Ubuntu > TIME LNE
1 A0
100
Ln 84, Col 10 Spaces 4 UTF-8 LF( Python3. 10. 6(transformer :conda)
Z
5/22/2023
7:09 PM
value = value. view (value. shape [0], value. shape[1], self. h, self. d_k). transpose (1, 2)
...
1 A0
What is the decay?
Ln 84. Col13 Spaces4 UTF-8 LFPython 3. 10. 6(trans
Z
ENG
5/22/2023
7:09 PM
...
95
96
> OUTUNE 它是查询、键和值的最后一个维度
WSL: Ubuntu > TIME LNE
1 A0
It's the last dimension of the query key and the value. ur s u
mto n 3io6 tast 人
ENG
5/22/2023
7:09 PM
...
1 A0
Ln84, Col16 Spaces4 UTF-8 LF Python 3. 10. 6(transfc
ENG
5/22/2023
7:09 PM
key =key. view (key. shape [0], key. shape[1], self. h, self. d_k). transpose (1, 2)
95
value =value. view (value. shape [e], value. shape [1], self. h, self. d_k). transpose(1, 2)
> OUT U NE 我们将使用这里的这个函数. 让我先调用它, 这样你可以
WSL : Ubuntu > TIME UNE and we will using this function here let me first call it so that you scan understand /22/2023
7:09 PM
99
> OUTUNE 理解我们将如何使用它, 然后我们再定义它
WSL: Ubuntu
> TIMEUNE
and we will using this function here let me first call it so that you can understand.
7:09 PM 人
5/22/2023
理解我们将如何使用它, 然后我们再定义它.
> OUT U NE
WSL : Ubuntu > TIME UNE how we will use it and then we define it so we want from this function we want two
7:09 PM 口
5/22/2023
98
99
> OUTUNE 所以我们希望从这个函数中得到两样东西:输出和我们想要的
WSL: Ubuntu
> TIMELNE
how we will use it and then we define it so we want from this function we want two 人
ENG
/22/2023
7:09 PM
98
...
99
> OUTUNE 所以我们希望从这个函数中得到两样东西:输出和我们想要的
WSL: Ubuntu > TIMELINE output and we want the attention scores so the output of the soft ma Xuo
4. 10. 6 C
7:09 PM
ENG
/22/2023
101 注意力分数.
> OUTUNE
WSL: Ubuntu > TIMELINE output and we want the attention scores so the output of the soft max
4. 10. 6(trans
7:09 PM
5/22/2023
99
> OUTUNE 所以softmax注意力分数的输出, 我们将这样调用它:所以我们
WSL: Ubuntu > TIMELINE output and we want the attention scores so the output of the soft maxuo
4. 10. 6 C 人
ENG
/22/2023
7:09 PM
...
99
> OUTUNE 所以softmax注意力分数的输出, 我们将这样调用它:所以我们
WSL: Ubuntu > TIMELINE attention scores and we will call it like this so we give it the query. the key the octn
2 ENG
/22/2023
7:09 PM
98 self. attention _score Multi Head Attention Block.
99 给它查询、键、值、掩码和 dropout层. 现在让我们回到
> OUTUNE
WSL: Ubuntu > TIME LNE
1 A0
attention scores and we will call it like this so we give it the query. the key the os ctr 人
5/22/2023
7:09 PM
101 这里. 所以我们有了dk.
> OUTUNE
WSL: Ubuntu > TIMELINE value the mask and the drop out layer now let'sgo back here so we. have the dknow
5/22/2023
7:09 PM
98
99
x, self. attention _scores = Multi Head Attention Block. attention (query, key, value, mask, self. dropout )
> OUT U NE 现在我们做的是首先应用公式的前半部分, 民 即查询乘以键的
> WSL : Ubuntu > TIMELINE value the mask and the drop out layer now let'sgo back here so we have the dknow
7:09 PM
X, self. attention _scores = Multi Head Attention Block. attention (query, key, value, mask, self. dropout )
> OUT UNE
...
WSL: Ubuntu
> TIMEUNE
first we apply the first part of the formula that is the query multiplied by. the osctans
7:09 PM
5/22/2023
x, self. attention _scores = Multi Head Attention Block. attention (query, key, value, mask, self. dropout )
> OUTUNE
102 转置, 除以dk的平方根
> WSL: Ubuntu > TIMELINE transpose of the key divided by the square root of dk ursu
pytaon sio6 tanstor
ENG
5/22/2023
7:09 PM
...
102
WSL: Ubuntu > TIME UNE
Ln85, Col9 Spaces:4 UTF-8 LF{ Python 3. 10. 6 (transfo 通
ENG
5/22/2023
7:09 PM
98
99
x, self. attention _scores = Multi Head Attention Block. attention (query, key, value, mask, self. dropout )
> OUT UNE 所以这些是我们的注意力分数:查询, 矩阵乘法. 所以这个
WSL: Ubuntu00
> TIMEUNE
so these are our attention scores query matrix multiplication so this add sign means 7:10 PM
ENG
5/22/2023
98
x, self. attention _scores = Multi Head Attention Block. attention (query, key, value, mask, self. dropout )
> OUT UNE 加号表示矩阵乘法. 在 Py Torch 中, 我们转置最后两个维度
> WSL : Ubuntu > TIME LNE matrix multiplication in py torch we transpose the last two dimensions minus. two minus
5/22/2023
7:10 PM
98
99
x, self. attention _scores = Multi Head Attention Block. attention (query, key, value, mask, self. dropout ) 减一意味着转置最后两个维度. 所以这将成为最后一个维度.
> OUT U NE
> WSL: Ubuntu
> TIMELNE
3 A0
...
99
x, self. attention _scores = Multi Head Attention Block. attention (query, key, value, mask, self. dropout )
> OUT UNE 减一意味着转置最后两个维度. 所以这将成为最后一个维度.
xwu unu a ·means transpose the last two dimensions so this will become the last dimension is. the > TIME LNE
5/22/2023
7:10 PM
1 是序列乘以序列长度乘以 dk.
x, self. attention _scores = Multi Head Attention Block. attention (query, key, value, mask, self. dropout )
...
5/22/2023
7:10 PM
x, self. attention _scores = Multi Head Attention Block. attention (query, key, value, mask, self. dropout ) 是序列乘以序列长度乘以 dk.
> OUTUNE
102
103
> WSL: Ubuntu > TIME LNE sequence by sequence length by dk it will become dk by sequence length 3. 10. 6 (trans
ENG
5/22/2023
7:10 PM
98
99
X, self. attention _scores = Multi Head Attention Block. attention (query, key, value, mask, self. dropout )
> OUT UNE 它将变成dk乘以序列长度, 然后我们将其除以√
WSL : Ubuntu > TIME UNE And then we divide this bymath. tk.
Ln 86, Col59 Spa
4. 10. 6 (tra
( Pytho
7:10 PM
...
99
self. attentio
> OUTUNE 我们在应用soft max 之前, 正如我们之前看到的, 需要应用
WSL : Ubuntu > TIMELINE We, before, as we saw before, before applying the soft max, we need to apply. the mas k..
5/22/2023
7:10 PM
x, self. attention _scores = Multi Head Attention Block. attention (query, key, value, mask, self. dropout )
> OUT U NE 掩码.
> WSL : Ubuntu > TIME UNE o We, before, as we saw before, before applying the soft max, we need to apply. the mas k. a
7:10 PM
5/22/2023
99
self. dropout) 所以我们想要隐藏一些词之间的交互
> OUTUNE
WSL: Ubuntu > TIME UNE So we want to hide some interaction between words. urs u
y tion 3. io. rtasto
ENG
7:10 PM
5/22/2023
101
ery, key, value, mask, self. dropout )
> OUTUNE
102 我们应用掩码, 然后应用 softmax.
WSLUbuntu > TIME UNE
ENG
7:10 PM
5/22/2023
value = value. view (value. shape [0], value. shape[1], self. h, self. d_k). transpose (1, 2)
99
self. dropout ) 所以 softmax 会处理我们替换的值
> OU TUNE
> WSL : Ubuntu > TIMELINE So the soft max will take care of the values that were placed.
LF { Python 3. 10. 6(transfc
ENG
7:10 PM
X, self. attention _scores = Multi Head Attention Block. attention (query, key, value, mask, self. dropout )
> OUT U NE
...
> TIMEUNE
104
Ln87, Col9 Spaces:4 UTF-8 LF( Python3. 10. 6 (transformer :oon 办 ENG
5/22/2023
7:10 PM
t tention (query, key, value, mask, self. dropout )
> OUT UNE 我们如何应用掩码?
> WSL: Ubuntu
> TIMEUNE
104
How do we apply the mask?
Ln 87, Col9 Spaces: 4 UTF-8 LF( Python 3. 10. 6 (transfo
ENG
5/22/2023
7:10 PM
X, self. attention _scores = Multi Head Attention Block. attention (query, key, value, mask, self. dropout )
...
98
99
> OUTUNE 我们只需将所有我们想要掩码的值替换为非常非常小的值
> WSLUbuntu > TIME UNE We just, all the values that we want to mask, we replace them with very, very small 7:10 PM
ENG
5/22/2023
98
99
...
WSL: Ubuntu
> TIMELNE
values so that the soft max will replace them with zero.
UTF-8 LFPyth
4. 10. 6(t
7:10 PM
99
self. dropout)
> OUTUNE
101 这样soft max 就会将它们替换为零
WSL : Ubuntu > TIME LNE values so that the soft max will replace them with zero.
UTF-8 LFPython3. 10. 6transformer :co
ENG
5/22/2023
7:10 PM
X, self. attention _scores = Multi Head Attention Block. attention (query, key, value, mask, self. dropout )
...
WSL: Ubuntu > TIME LNE
104
Ln87, Col9 Spaces :4 UTF-8 LF( Python3. 10. 6 (transformer :con
ENG
5/22/2023
7:10 PM
899
...
self. d_k). transpose(1, 2)
mask, self. dropout )
> OUT UNE 102 所以如果定义了掩码, 就应用它,
WSL: Ubuntu
> TIMELNE
103
So if a mask is defined, apply it.
Ln 87, Col11 Spaces4 UTF-8 LF { Python 3. 10. 6(transfo
ENG
5/22/2023
7:11 PM
...
1 A0
104
Ln 88, Col13 Spaces :4 UTF-8 LF( Python3. 10. 6 (trans fo
ENG
5/22/2023
7:11 PM
98
...
value =value. view (value. shape [e], value. shape[1], self. h, self. d_k). transpose (1, 2)
sel f. d_k). transpose(1, 2)
> OUTUNE 这意味着基本上将所有满足此条件的值替换为这个值.
> WSL: Ubuntu > TIMELINE This means basically replace all the values for which this statement is true with 5/22/2023
7:11 PM
98
99
key =key. view(key. shape [0], key. shape[1], self. h, self. d_k). transpose(1, 2)
value =value. view (value. shape [θ], value. shape [1], self. h, self. d_k). transpose(1, 2) 这意味着基本上将所有满足此条件的值替换为这个值.
> OUT U NE
> WSL : Ubuntu > TIMELINE this value.
Ln88. Col51 S
( Pytho
4. 10. 6(tra
7:11 PM
5/22/2023
key =key. view (key. shape [e ], key. shape [1], self. h,
self. d_k). transpose(1, 2)
value = value. view (value. shape [θ], value. shape [1], self. h, self. d_k). transpose(1, 2)
x, self. attention _scores = Multi Head Attention Block. attention (query, key, value, mask, self. dropout )
> OUT U NE
WSL : Ubuntu > TIMELINE 104
Ln8, Col58(4selected ) Spaces 4 UTF-8 LF( Python3. 10. 6(trans lo
Z
ENG
5/22/2023
7:11 PM
key =key. view (key. shape [θ], key. shape [1], self. h,
self. d_k). transpose(1, 2)
1
value =value. view (value. shape [θ], value. shape [1], self. h, self. d_k). transpose(1, 2)
value, mask, self. dropout )
> OUT UNE
102 我们希望它被替换为这个值
> WSL: Ubuntu
> TIMELNE
The mask we will define in such a way that where this value, this expression is. true,
03
7:11 PM
5/22/2023
key =key. view (key. shape [θ], key. shape [1], self. h,
...
WSL : Ubuntu > TIME LN E
104
Ln88, Col59 Spaces4 UTF-8 LF Python 3. 10. 6(transfo
ENG
5/22/2023
7:11 PM
key = key. view (key. shape[0], key. shape[1], self. h,
...
WSL: Ubuntu00
> TIMELINE 104
Ln88, Col59 Spaces:4 UTF-8 LF( Python 3. 10. 6 (transfo
ENG
5/22/2023
7:11 PM
98
...
key =key. view(key. shape [0], key. shape[1], self. h, self. d_k). transpose(1, 2)
value =value. view (value. shape [e], value. shape [1], self. h, self. d_k). transpose(1, 2)
> OUT UNE 现在, 姑且认为这些都是我们不想要的值
elf. dropout > WSL : Ubuntu > TIME LNE For now, just take it for granted that these are all the values that we don't want.
7:11 PM
5/22/2023
key =key. view (key. shape[0], key. shape[1], self. h,
value =value. view (value. shape [θ], value. shape[1], self. h, self. d_k). transpose (1, 2)
...
self. d_k). transpose(1, 2)
value = value. view (value. shape [θ], value. shape [1], self. h, self. d_k). transpose(1, 2)
x, self. attention _s 在注意力中
attention(query, key, value, mask, self. dropout )
> OUTUNE
102
103
WSL: Ubuntu > TIME UNE to have in the attention so we don't want for example some word to watch future words 5/22/2023
7:11 PM
98
99
key = key. view (key. shape[0], key. shape[1], self. h,
value =value. view (value. shape [e], value. shape[1], self. h, self. d_k). transpose (1, 2)
sel f. d_k). transpose(1, 2)
> OUTUNE 所以我们不希望, 例如, 某个词去关注未来的词,"例如当
> WSL: Ubuntu > TIME UNE to have in the attention so we don't want for example some word to watch future words 5/22/2023
7:11 PM
98
key=key. view(key. shape [0], key. shape[1], self. h,
...
99
value =value. view (value. shape [e], value. shape [1], self. h, self. d_k). transpose(1, 2)
> OUT U NE 所以我们不希望, 例如, 某个词去关注未来的词,"例如当
> WSL : Ubuntu > TIME UNE for example when we will build a decoder or we don't want the padding values to. to
5/22/2023
7:11 PM
98
99
key =key. view(key. shape [0], key. shape[1], self. h, self. d_k). transpose(1, 2)
value =value. view (value. shape [θ], value. shape [1], self. h, self. d_k). transpose(1, 2)
> OUT U NE 我们构建解码器时, 或者我们不希望填充值与其他值交互
WSL : Ubuntu > TIME LNE
interact
7:11 PM
5/22/2023
...
key=key. view(key. shape [0], key. shape[1], self. h, self. d_k). transpose(1, 2)
value =value. view (value. shape [θ], value. shape[1], self. h, self. d_k). transpose (1, 2)
> OUTUNE 因为它们只是为了达到序列长度而填充的词
ropout
> WSL: Ubuntu > TIME LNE with other values because they are just filler words to reach. the sequence. length.
5/22/2023
7:11 PM
key =key. view (key. shape[0], key. shape[1], self. h,
value = value. view (value. shape [θ], value. shape[1], self. h, self. d_k). transpose (1, 2)
...
99
value = value. view (value. shape [θ], value. shape[1], self. h, self. d_k). transpose (1, 2)
> OUTUNE 我们将用负10的9次方替换它们, 这是一个在负数
> WSL: Ubuntu > TIME UNE we will replace them with minus10 to the power of nine, which is a very big. number 5/22/2023
7:11 PM
98
99
key=key. view (key. shape[0], key. shape[1], self. h,
value =value. view (value. shape [θ], value. shape [1], self. h, self. d_k). transpose(1, 2)
self. d_k). transpose(1, 2) 范围内非常大的数, 基本上代表负无穷大dt)
> OUTUNE
> WSL: Ubuntu > TIMELINE in the negative range, which basically represents minus infinity.
Python 3. 10. 6 (trans
ENG
7:12 PM
5/22/2023
...
99
key =key. view(key. shape [0], key. shape[1], self. h, self. d_k). transpose(1, 2)
value =value. view (value. shape [θ], value. shape [1], self. h, self. d_k). transpose(1, 2) 然后当我们应用soft max 时, 它将被替换为零.
> OUT U NE
WSL : Ubuntu > TIME LNE And then when we apply now the soft max, it will be replaced by zero. ython
4. 10. 6(tra
7:12 PM
key =key. view (key. shape [0], key. shape[1], self. h, self. d_k). transpose (1, 2)
query =query. view (query. shape [θ], query. shape [1],
self. h, self. d_k). transpose(1, 2)
value = value. view (value. shape [e], value. shape [1], self. h, self. d_k). transpose(1, 2)
> OUT UNE
x, self. attention _scores = Multi Head Attention Block. attention (query, key, value, mask, self. dropout )
> WSL Ubuntu > TIME LNE
Ln89, Col13 Spaces4 UIF-8 FPython 3. 10. 6(transfo
ENG
5/22/2023
7:12 PM
98
...
cey
view(key. sha
pe[θ], key. shape [1], self. h, self. d_k). transpose(1, 2)
> OUT U NE 所以在这种情况下我们有个批次乘以边, 所以每个头都
> WSL Ubuntu > TIME UNE applied to this dimension okay let me write some comments so in. this case we have a
7:12 PM
ENG
5/22/2023
98
99
cey
view(key. shape [θ], key. shape[1], self. h, self. d_k). transpose(1, 2)
> OUT UNE 会, 然后是序列长度和序列长度如果我们也有dropout > WSL : Ubuntu > TIME LNE batch by edge so each head will and then sequence length and sequence length all 5/22/2023
7:12 PM
98
99
query = query. view (que. shape [1], self. h, self. d_k). transpose(1, 2) 会, 然后是序列长度和序列长度, 女 如果我们也有
dropout > OUT U NE
> WSL : Ubuntu > TIME LNE right if we Ln 90. Col 5
Pytho
7:12 PM
p
5/22/2023
98
99
query =query. view (query. shape [e ], query y. shape [1], self. h, self. d_k). transpose(1, 2)
> OUT UNE 的话. 所以如果dropout 没有错,"我们也应用"dropout.
> WSL : Ubuntu > TIME LNE
right if we
Ln90. Col9 S
UTF-8 LF { Pytho
...
5/22/2023
10
#( Batch, Seq_ Len, d_model ) -->( Batch, Seq_ Len, h, d_k)-->( Batch, h, Seq_len, d_k)
query =query. view (query. shape [e], query. shape[1], self. h, self. d_k). transpose (1, 2)
key=key. view(key. shape [0], key. shape[1], self. h,
self. d_k). transpose(1, 2)
> OUT UNE value = value. view (value. shape [0], value. shape[1], self. h, self. d_k). transpose (1, 2)
WSL: Ubuntu
> TIMEUNE
1e4
X. self. attention scores = Multi Head Attention Block. attention (query. key. value.
Ln91. Col 13
. self. dro
UTF-8 LFPython 3. 10. 6trans f
ENG
7:12 PM
5/22/2023
98 最后, 正如我们在原始幻灯片中看到的, 我们将"softmax的 的
> OUTUNE
> WSL: Ubuntu > TIME LNE And finally, as we saw in the original slide, we multiply the output of the softmax.
7:12 PM
5/22/2023
key = self. w_k (k)#( Batch, Seq _ Len, d_model )-->( Batch, Seq _ Len, d_model )
18
#( Batch,
value =self. w_v(v)#( Batch, Seq _ Len, d_model )-->( Batch, Seq _ Len, d_model ) 输出与 V矩阵相乘
( Batch, h, Seq_len,
> OUTUNE
query =q
olf. d_k). transpose(1, 2)
> TIMEUNE
And finally, as we saw in the original slide, we multiply the output. of the soft max key.
an spose (1, 2)
> WSL: Ubuntu
5/22/2023
7:13 PM
98
9g
key= self. w_k(k)#( Batch, Seq_ Len, d_model )-->( Batch, Seq _ Len, d_model )
value =self. w_v (v)#( Batch, Seq _ Len, d_model )-->( Batch, Seq _ Len, d_model ) 所以我们返回注意力得分乘以值, 以及注意力得分本身.
> OUT U NE
> WSL : Ubuntu > TIME UNE by the V matrix multiplication,'so we return a, s
Pytl
4. 10. 6(tra
7:13 PM
5/22/2023
key = self. w_k (k)#( Batch, Seq _ Len, d_model )-->( Batch, Seq _ Len, d_model )
value = self. w_v (v)#( Batch, Seq _ Len, d_model )-->( Batch, Seq _ Len, d_model )
> OUT UNE query =query. view (query. shape [θ], query. shape [1], self. h, self. d_k). transpose(1, 2)
#( Batch, Seq _ Len, d_model )-->( Batch, Seq _ Len, h, d_k)-->( Batch, h, Seq_len, d_k)
> TIMEUNE
104
value value. view (value. shape [0]. value. shape[1l. self. h, self. d k). transpose (1. 2)
key=key. view(key. shape [0], key. shape[1], self. h, self. d_k). transpose(1, 2)
> WSL : Ubuntu Ln93, Col60 Spaces4 UTF-8 LF ( Python 3. 10. 6(transfc
5/22/2023
7:13 PM
key = self. w_k (k)#( Batch, Seq _ Len, d_model )-->( Batch, Seq _ Len, d_model )
10 那么我们为什么要返回一个元组呢?
value =self. w_v(v)#( Batch, Seq_ Len, d_model )-->( Batch, Seq _ Len, d_model)
> OUTUNE
102
103
> WSL: Ubuntu > TIME UNE " So why are we returning a tuple?
4 UTF-8 LFPython 3. 10. 6(transfo
5/22/2023
7:13 PM
key = self. w_k (k)#( Batch, Seq _ Len, d_model )-->( Batch, Seq _ Len, d_model )
value = self. w_v (v)#( Batch, Seq _ Len, d_model )-->( Batch, Seq _ Len, d_model )
> OUT UNE #( Batch, Seq _ Len, d_model )-->( Batch, Seq_ Len, h, d_k)-->( Batch, h, Seq_len, d_k)
query =query. view (query. shape [0], query. shape[1], self. h, self. d_k). transpose (1, 2)
> TIMEUNE
104
key=key. view (key. shape [0], key. shape[1], self. h, self. d_k). transpose (1, 2)
value =value. view (value. shape e]. value. shape [1l. self. h. self. dk). tran
e(1. 2)
WSL: Ubuntu Spaces 4 UTF-8 LFPython 3. 10. 6(transfo
5/22/2023
7:13 PM
key = self. w_k (k)#( Batch, Seq _ Len, d_model )-->( Batch, Seq _ Len, d_model )
value =self. w_v (v)#( Batch, Seq _ Len, d_model )-->( Batch, Seq _ Len, d_model )
...
d_k
> OUTUNE
query =que key =key. view (key.
olf. d_k). transpose (1, 2)
WSL: Ubuntu
> TIMEUNE
104
value =value. v Because we want this.
n93. Col41
Spaces4 UTF-8 LF { Python 3. 10. 6(transfc 人
5/22/2023
7:13 PM
key = self. w_k (k)#( Batch, Seq _ Len, d_model )-->( Batch, Seq _ Len, d_model )
value = self. w_v (v)#( Batch, Seq _ Len, d_model )-->( Batch, Seq _ Len, d_model )
...
104
key=key. view (key. shape [0], key. shape[1], self. h, self. d_k). transpose (1, 2)
value =value. view (value. shape o ]. value. shape 1l. self. h, self. d k). tra
se(1. 2)
WSL: Ubuntu Ln 93, Col 21 (21 selected )
Spaces 4 UTF-8 LFPython 3. 10. 6(transfo
5/22/2023
7:13 PM
98
99
value =self. w_v(v)#( Batch, Seq_ Len, d_model )-->( Batch, Seq _ Len, d_model ) 当然, 我们需要它用于模型, 因为我们需要将其传递给下
> OUT U NE
WSL : Ubuntu > TIME UNE Of course we need it for the model because we need to give it. to. the next layer. ctan
5/22/2023
7:13 PM
value =self. w_v (v)#( Batch, Seq _ Len, d_model )-->( Batch, Seq _ Len, d_model )
#( Batch, Seq _ Len, d_model)
Len, h, d_k)-->( Batch, h, Seq_len, d_k)
> OUTUNE
102
103
query = query. view (query. shape [0],
key. view(key. shape [θ], key. sh
ape[1], self. h, self. d_k). transpose(1, 2)
self. h,
self. d_k). transpose(1, 2)
> WSL: Ubuntu > TIME LNE Of course we need it for the model because we need to give it to. the next layer. can 5/22/2023
7:13 PM
key = self. w_k (k)#( Batch, Seq _ Len, d_model )-->( Batch, Seq _ Len, d_model )
value = self. w_v (v)#( Batch, Seq _ Len, d_model )-->( Batch, Seq _ Len, d_model )
> OUT UNE query =query. view (query. shape [θ], query. shape [1], self. h, self. d_k). transpose(1, 2)
...
value =value. view (value. shape o ]. value. shape [1l. self. h. self. d k). tra
se(1. 2)
> WSL: Ubuntu Spaces 4 UTF-8 LFPython3. 10. 6(trans o
ENG
5/22/2023
7:13 PM
value =self. w_v (v)#( Batch, Seq _ Len, d_model )-->( Batch, Seq _ Len, d_model )
...
d_k
> OUTUNE
query =que key =key. vie
elf. dk). transpose (1, 2)
transpose(1, 2)
> WSL: Ubuntu
> TIMELNE
104
But'this will be used for visualization. ad
(1. 2)
Spaces4 UTF-8 LFPython 3. 10. 6(transfo
ENG
5/22/2023
...
value =value. view (value. shape o ]. value. shape 1l. self. h. self. d k). tra
e(1. 2)
> WSL: Ubuntu Ln 93, Col 60 (16 selected )
Spaces 4 UTF-8 LFPython 3. 10. 6(transfc
5/22/2023
7:13 PM
98
99
key= self. w_k(k)#( Batch, Seq_ Len, d_model )-->( Batch, Seq _ Len, d_model )
value =self. w_v (v)#( Batch, Seq _ Len, d_model )-->( Batch, Seq _ Len, d_model ) 所以自注意力的输出, 在这种情况下是多头注意力的输出
> OUT U NE
WSL : Ubuntu > TIME UNE So the output of the'self-attention, so the multi-head attention in this case, o tanm
7:13 PM
5/22/2023
98
99
value =self. w_v(v)#( Batch, Seq_ Len, d_model )-->( Batch, Seq _ Len, d_model ) 所以自注意力的输出, 在这种情况下是多头注意力的输出
> OUT U NE
WSL : Ubuntu > TIMELINE =value. view (value. shape e], value. shape[1l. self. h. self. dk). t
Pytho
4. 10. 6(t
7:13 PM
5/22/2023
98
value = self. w_v(v) #( Batch, Seq_ Len, d_model ) -->( Batch, Seq _ Len, d_model ) 实际上会在这里, 我们将用它来进行可视化所以为了
> OUT U NE
WSL : Ubuntu > TIMELINE is actually going to be here and we will use it for visualizing So for visualizing osctl
7:13 PM 人
5/22/2023
98
value =self. w_v(v)#( Batch, Seq_ Len, d_model )-->( Batch, Seq _ Len, d_model )
> OUT U NE 可视化, 模型对这个特定交互给出的分数是多少? 我也要在
> WSL : Ubuntu > TIME LNE what is the score given by the model for that particular interaction let me. also. co
5/22/2023
7:13 PM
key = self. w_k (k)#( Batch, Seq _ Len, d_model )-->( Batch, Seq _ Len, d_model )
value =self. w_v (v)#( Batch, Seq _ Len, d_model )-->( Batch, Seq _ Len, d_model )
( Batch, Sq >( Batch, h, Seq_len,
> OUTUNE
102
103
query =query.
x ey key. view self. d_k). transpose (1, 2)
> WSL: Ubuntu
> TIMELNE
what is the score given by the model for that particular interaction let me also. ctan
7:13 PM
5/22/2023
98
99
query= self. w_q(q)#( Batch, Seq_ Len, d_model ) -->( Batch, Seq _ Len, d_model )
key = self. w_k (k)#( Batch, Seq _ Len, d_model )-->( Batch, Seq _ Len, d_model )
> OUTUNE 所以这里我们这样做批次, 让我们回到这里. 所以现在我们
WSL: Ubuntu00
> TIMELNE
write some comments here so here we are doing like this batch r
Python
7:13 PM
ENG
5/22/2023
...
99
query = self. w_q(q)#( Batch, Seq_ Len, d_model ) -->( Batch, Seq _ Len, d_model )
key =self. w_k (k)#( Batch, Seq _ Len, d_model )-->( Batch, Seq _ Len, d_model ) 有多头注意力. 多头注意力的输出. 我们最终要做的是
> OUT U NE
WSL : Ubuntu > TIMELINE =kev. view (kev. s Python 3. 10. 6 (tra
7:13 PM
5/22/2023
106
107
x, sel f. attention_scores = Multi Head Attention Block. attention(query, key, value, mask, self. dropout)
> OUTUNE 有多头注意力. 多头注意力的输出. 我们最终要做的是
WSL: Ubuntu
> TIMELINE
and let'sgobackhere so nowwe have ourmulti-head attention. so. the output of the ENG
5/22/2023
7:14 PM
112
> OUTUNE 好吧, 让我们回到幻灯片.
> WSLUbuntu > TIMELINE and let'sgo back here so now we have our multi-head attention so. the output of the 7:14 PM 人
5/22/2023
112
> OUTUNE 好吧, 让我们回到幻灯片.
> WSLUbuntu > TIMELINE multi-head attention what we do is finally we okay let's go back to. the slide first 7:14 PM
5/22/2023
seq sequence length =size of the embedding ve 首先, 我们在哪里. 我们计算了这里较小的矩阵
(head,.. head )wo number of heads d_d
=dmodel /h where we
seq sequence length =size of the embedding 所以我们应用了softmax, k乘以kt除以dv的平方根
headn)wo
number of hea are we calculated these smaller matrices here so we applied the soft max koby kt
seq sequence length 然后我们也乘以 V 一一我们可以在这里看到二一
number of head dk-d
divided by the square root of dv and then we multiplied it also by v we can see it
seq sequence length number of hea然后我们也乘以 V一一
size of the embedding vect c
dk-d
here
seq sequence length number of heads dk-d
which gives us this small matrix here, head one, head two, head three, and thread
seq sequence length size of the embedding vec to 我们这里的小矩阵, 头一、头二、头三和头四"
cat (head,... head n)wo
number of heads dk_d
four.
seq sequence length 现在我们需要将它们组合在一起, 连接, 就像论文中的公式
number of he NOW we need to combine them together, concat, just like the formula says from the
seq sequence length = size of the embedding vector 所说, 最后乘以
Multi Head ( Q, K, V)= Concat (head.. head n)w
number of heads wo.
dk-d
paper, and finally multiply it by wo.
112
> OUTUINE
WSL: Ubuntu > TIME UNE
Ln 109. Col9 Spaces 4 UTF-8 LFPython3. 10. 6(trans f
ENG
7:14 PM
5/22/2023
110
111
> OUTUINE 我们转置是因为在我们将矩阵转换为序列长度乘以之前, 我们
> TIMELINE 人
5/22/2023
7:14 PM
110
111
=x. transpose(1, 2)
> OUTUNE 我们最初想要将它们组合在一起, 因为结果张量, 我们希望
WSL : Ubuntu > TIME LNE the sequence length as the third dimension we wanted. back in. the first place to 7:14 PM
5/22/2023
110
111
x=x. transpose(1, 2)
> OUTUNE 我们最初想要将它们组合在一起, 因为结果张量, 我们希望
WSL : Ubuntu > TIME LNE combine them UTF-8 LFPytho
7:15 PM
5/22/2023
110
111
x=x. transpose(1, 2)
> OUTUNE 序列长度在第二位置. 所以让我先写下来, 我们想要做什么
> TIMELINE 人
5/22/2023
7:15 PM
110
...
111
> OUTUNE 序列长度在第二位置. 所以让我先写下来, 我们想要做什么
> WSL Ubuntu > TIME LNE so let me write it first what we want to do batch we started from this one sequence 7:15 PM
5/22/2023
(=x. transpose (1, 2)
112
> OUTUNE 批次, 我们从这一个序列长度开始.
WSL: Ubuntu > TIME LNE length first we do a transposition Ln109, Col33 Spac
ec4 UTF-8 LF{ Python 3. 10. 6(trans
5/22/2023
7:15 PM
110
111
x=x. transpose(1, 2)
> OUTUNE 首先我们做一个转置, 然后我们想要的是这个:所以这个转置
WSL : Ubuntu > TIME LNE length first we do a transposition Ln109, Col36 Spa
Pytho
7:15 PM
5/22/2023
110
...
X=x. transpose(1, 2)
> OUTUNE 首先我们做一个转置, 然后我们想要的是这个:所以这个转置
WSL : Ubuntu > TIME UNE and then what we want is this so this transposition takes us here and then we do a 人
ENG
5/22/2023
7:15 PM
110
#( Batch, h, Seq_ Len, d_k)-->( Batch, Seq_ Len
x=x. transpose (1, 2)
-->( Batch, Seq_ Len, d_model)
111
> OUT U NE 带我们到这里, 然后我们做一个视图. 但我们不能这样做.
> WSL : Ubuntu > TIMELINE view but we can not do it we need to use contiguous this means basically. that Py Torchto 人
5/22/2023
7:15 PM
=x. transpose (1, 2).
113
112
> OUTUNE 我们需要使用连续的.
WSL: Ubuntu > TIMELINE ·view but we can not do it we need to use contiguous this means basically. that Py Torchto
5/22/2023
7:15 PM
111
x=x. transpose(1, 2). contiguous ().
> OUT UNE 这基本上意味着 Py Torch, 为了改变张量的形状, 需要我们将内存
> WSL: Ubuntu
> TIMELNE
iview but we can not do it we need to use contiguous this means. basically. that Py Torchto
/22/2023
7:15 PM
#( Batch, h, Seq_ Len, d_k)-->( Ba
x=x. transpose(1, 2). contiguous (),
n, d_model)
111
> OUTUNE 这基本上意味着 Py Torch, 为了改变张量的形状, 需要我们将内存
> TIME LNE
5/22/2023
7:15 PM
110
...
> OUT U NE 设置为连续的, 这样他就可以就地进行操作, 减去一, 并
> WSL : Ubuntu > TIMELINE transform the shape of a tensor needs to we put the memory to be contiguous so he can
/22/2023
7:16 PM
110
=x. transpose(1, 2). contiguous ().
n, d_model)
111
> OUTUNE 设置为连续的, 这样他就可以就地进行操作, 减去一, 并
> WSL: Ubuntu > TIMELINE ust do it in place Ln110. Col44 Spaces4 UTF-8 LFPython 3. 10. 6 (tra
7:16 PM
5/22/2023
110
111
=x. transpose(1, 2). contiguous (). view (x. shape[θ], D
> OUT UNE 乘以 self dot h乘以 self dotdk, 正如你记得的, 这是
WSL: Ubuntu
> TIMELINE
n 110, Col 61
Pytho
...
=x. transpose(1, 2). contiguous (). view (x. shape[θ],-1)
111
> OUTUNE 乘以self doth 乘以self do tdk, 正如你记得的, 这是
> WSL: Ubuntu
> TIMELINE
minus one and the self dot h multiplied by selfdot dkwhich as you. remember this is 人
/22/2023
7:16 PM
110
#( Batch, h, Seq_ Len, d_k)-->( Batch, Seq_ Len, h, d_ K)
x=x. transpose(1, 2). contiguous (). view (x. shape [θ],-1, self. h*self. d_k)
->(( Batch, Seq_ Len, d_model )
111
> OUTUNE 模型, 因为我们在这里定义了dk. 模型乘以h, 除以 h
> WSL: Ubuntu > TIME UNE minus one and the self doth multiplied by self do tdk which as you remember. this is 7:16 PM
/22/2023
110
111
x=x. transpose(1, 2). contiguous (). view (x. shape [θ],-1, sel f. h*self. d_k) 模型, 因为我们在这里定义了dk. 模型乘以h, 除以 h
> OUTUNE
> WSLUbuntu > TIME UNE the um the model because we defined dk to be here the model by h divide by h okay and
/22/2023
7:16 PM
110
111
x=x. transpose(1, 2). contiguous (). view (x. shape [θ],-1, self. h*self. d_k)
> OUTUNE 最后我们将这个×乘以wo, 这是我们的输出矩阵,
WSL: Ubuntu > TIME UNE finally we multiply this x by wo which is our output matrix. of xu Python 3. 10. 6(tran
7:16 PM 南
ENG
5/22/2023
x=x. transpose(1, 2). contiguous (). view (x. shape[θ],-1, self. h *self. d_k)
...
115
> WSL: Ubuntu > TIME LN E
Ln 112. Col27 Spaces4 UTF-8 LFPython 3. 10. 6(trans
ENG
7:16 PM
5/22/2023
=x. transpose (1, 2). contiguous (). view (x. shape [θ],-1, self. h*self. d_k)
> OUTUNE
114 这将给我们一一手 我们从批次到
> WSL: Ubuntu > TIME LNE
ENG
...
> WSL: Ubuntu > TIME UNE
4 UTF-8 LFPython
4. 10. 6(trans n 112, Col66
7:16 PM 人
ENG
5/22/2023
108
x, self. attention_scores = Multi Head Attention Block. attention(query, key, value, mask, self. dropout)
109 这就是我们的多头注意力块
_ Len, d_model)
> OUTUNE
110
...
> TIMEUNE
112
And this isour multi-head attention block. col66 spaces4 ursu (pyhon 3. 1o6ctastor
ENG
5/22/2023
7:16 PM
X, self. attention _scores = Multi Head Attention Block. attention (query, key, value, mask, self. dropout )
#( Batch, h, Seq_ Len, d_k)-->( Batch, Seq _ Len, h, d_ K)-->( Batch, Seq_ Len, d_model )
> OUTUNE
111
x=x. transpose(1, 2). contiguous (). view (x. shape[θ],-1, self. h*self. d_k)
> WSL: Ubuntu > TIME LNE 112
( Batch. Senlen
Ln 112, Col66 Spaces 4 UTF-8 LFPython 3. 10. 6(trans
ENG
5/22/2023
7:17 PM
110
111
x. transpose(1, 2). contig
uous(). view (x. shape[θ],-1, sel f. h*self. d_k)
> OUTUNE 我们现在有了所有必要的成分, 可以将它们全部组合在一起.
WSL: Ubuntu > TIME LNE We have, l'think, all the ingredients now to combine them all. together. hon
4. 10. 6 (tra
7:17 PM
5/22/2023
x=x. transpose(1, 2). contiguous (). view (x. shape [θ],-1, self. h *self. d_k)
1
#( Batch, Seq_ Len, d_model )-->( Batch, Seq_ Len, d_model)
> OUTUNE
114
return self. w_o(x)
> WSL: Ubuntu
> TIMELNE
115
In 112. Col66 Spaces:4 UTF-8 LF { Python 3. 10. 6(trans
ENG
5/22/2023
7:17 PM
=x. transpose (1, 2). contiguous(). view(x. shape[θ],-1, self. h*self. d_k)
...
> TIMELNE
115
Wejust missed one small layer.
In 112. Col66 Spaces:4 UTF-8 LF { Python 3. 10. 6(trans
ENG
5/22/2023
7:17 PM
x=x. transpose(1, 2). contiguous (). view (x. shape [0],-1, self. h *self. d_k)
#( Batch, Seq_ Len, d_model ) -->( Batch, Seq _ Len, d_mode 1)
> OUTUNE
114
return self. w_o(x)
WSL : Ubuntu > TIMELINE 115
Ln 112. Col66 Spaces4 UTF-8 LF Python 3. 10. 6(trans
ENG
5/22/2023
7:17 PM
我们还需要构建最后一层, 就是这里看到的连接.
There is one last layer we need to build, which is the connection we can see here.
例如, 这里我们有一些这层的输出, 所以在这里加上一个
For example, here we have some output of this layer, so add a norm that is taken here
带有这个连接的归一化.
with this connection.
这一部分被发送到这里.
And this one part is sent here.
然后这个的输出被发送到归一化, 然后通过这层组合在一起.
Then the output of this is sent to the add a norm and then combined together by this
然后这个的输出被发送到归一化, 然后通过这层组合在一起.
aver
所以我们需要创建这个管理跳跃连接的层. 所以我们取输入,
so we need to create this the layer that manages this skip connection so we take the
我们取前一层的输出, 所以在这种情况下, 多头注意力,
input we give it to we skip it by one layer we take the output of the previous layer
我们取前一层的输出, 所以在这种情况下, 多头注意力,
so in
我们取前一层的输出, 所以在这种情况下, 多头注意力,
this case the multi head attention we give it to this layer but also combining with
我称之为残差连接, 因为它基本上是一个跳跃连接. 好的,
this part so let's build this layer we i will call it a residual connection because.
我称之为残差连接, 因为它基本上是一个跳跃连接. 好的,
it's
让我们构建这个残差连接.
basically a skip connection okay let's build this residual connection
> OUT U NE 让我们构建这个残差连接.
> WSL : Ubuntu > TIMELINE basically a skip connection okay let's build this residual. connection who n 3. ios trash
ENG
7:19 PM
5/22/2023
> OUT LUI NE
> WSL : Ubuntu > TIMELINE Ln. 116, Col1 Spaces4 UTF-8 LF(1 Python 3. 10. 6(transfc
7:19 PM
5/22/2023
> OUT UNE 像往常一样, 我们定义构造函数, 在这种情况下, 我们只
WSL: Ubuntu
> TIMELINE
1 A0
as usual we define the constructor and in this case we just need a drop out. as. you 7:20 PM
ENG
5/22/2023
需要一个dropout. 如你所记, 脚本连接是在加和归一化与前一层
> OUT U NE
WSL : Ubuntu > TIME LNE as usual we define the constructor and in this case we just need a drop out. as you ENG
7:20 PM
121 之间.
> OUTUNE
> WSL: Ubuntu > TIME UNE remember the script connection is between the add and the norm and the previous. layer cnd
7:20 PM
5/22/2023
121
> OUTUNE 所以我们还需要一个归一化, 这是我们之前定义的层归一化
WSL: Ubuntu > TIME UNE so we also need an or m
Ln 121. Col9 Spac
UTF-8 LFPytho
4. 10. 6 (tr
7:20 PM
...
self. norm =
> OUTUNE 所以我们还需要一个归一化, 这是我们之前定义的层归一化
WSL: Ubuntu > TIMELINE which is our layer normalization which we defined before and then we define the 5/22/2023
7:20 PM
122
123
lef 然后我们定义前向方法和子层, 这是前一层.
> OUTUNE
> WSL: Ubuntu > TIME LNE
2 A0
which is our layer normalization which we defined before and then we define the 7:20 PM
122
123
forward(sel f, x) 然后我们定义前向方法和子层, 这是前一层.
> OUTUNE
> WSL: Ubuntu > TIME UNE
7:20 PM 口
5/22/2023
121
122
self. norm= Layer Normalization ()
123
ard(sel f, x, sublayer):
> OUTUNE 我们所做的是我们取×, 并将其与下一层的输出结合, 在
> WSLUbuntu > TIME LNE forward method and the sublayer which is the previous layer what we do is we take the 人
7:20 PM
121
122
self. norm = Layer Normalization ()
...
WSL: Ubuntu > TIME LNE
1 A0
combine it with the output of the next layer which is in this case is. called sublayer ENG
/22/2023
7:20 PM
121
122
self. norm = Layer Normalization () 这种情况下, 称为子层, 我们应用dropout.
123
> OUTUNE
WSL: Ubuntu > TIME UNE
1 A0
and we apply the drop out Ln 124. Col29 S
4. 10. 6(tra
Pytho
7:20 PM
ENG
...
123
124
defforward(self, x, sublaye (....)-> Any return x +self. dropout ()
> OUTUNE
WSL: Ubuntu00
> TIMEUNE
Ln 124. Col33 Spaces4 UTF-8 LF Python 3. 10. 6(trans
7:20 PM
ENG
5/22/2023
121
122
self. norm= Layer Normalization ()
123 所以这是加和归一化的定义. 实际上, 有一个细微的差别:
> OUTUNE
WSL: Ubuntu > TIMELINE so this is the definition of add and norm actually there is a slight difference that 5/22/2023
7:21 PM
121
122
self. norm= Layer Normalization ()
...
2 我们首先应用归一化, 然后应用子层.
> OU TUNE
WSL : Ubuntu > TIME LNE we first apply the normalization and then we apply the sublayer in. the case of the 人
5/22/2023
7:21 PM
121
122
...
(variable ) dropout :
123
forward(self, 在论文的情况下, 他们首先应用子层, 然后是归一化. 我
> OUTUNE
WSL: Ubuntu > TIME UNE
paperthey
Ln 124, Col 41 (8 selected )
Pytho
7:21 PM
5/22/2023
121
122
self. norm = Layer Normalization ()
123 forward (self, 看到了很多实现, 其中大多数实际上是这样做的, 所以我们
> OUTUNE
WSL: Ubuntu00
> TIMELNE
apply first the sublayer and then the normalization I saw many implementations and /22/2023
7:21 PM
self. norm = Layer Normalization ()
123
defforward (self, x,
sublayer ):
124
return x+self 也会坚持这一点.
> OUTUNE
WSL: Ubuntu > TIME LNE most of them actually did it like this so we will also stick with this particular o tan 人
ENG
5/22/2023
7:21 PM
self. norm = Layer Normalization ()
...
WSL: Ubuntu
> TIMELNE
Ln 124. Col56 Spaces 4 UTF-8 LFPython3. 10. 6 (trans
ENG
7:21 PM
Z
5/22/2023
如你所记, 这些块通过这里更大的块组合在一起, 我们有
n 个这样的块.
remem
berwe have these blocks are combined together by this bigger block here
n 个这样的块.
and we have n of them.
所以这个大块我们称之为编码器块, 每个编码器块重复n So this big block we will call it encoder block and each of this encoder block is
次, 前一个的输出发送到下一个, 最后一个的输出发送到
repeated n times where the output of the previous is sent to the next one and the
次, 前一个的输出发送到下一个, 最后一个的输出发送到
output of the last one is sent to the decoder.
解码器.
output of the last one is sent to the decoder.
所以我们需要创建这个块, 它将包含一个多头注意力, 两个
加和归一化, 以及一个前馈.
加和归一化, 以及一个前馈.
end norm and one feed forward.
所以让我们来做吧.
So let's do it.
self. norm = Layer Normalization ()
124
...
return x +self. dropout (sublayer (self. norm (x )))
> OUT UI NE
WSL : Ubuntu > TIME UNE Ln 124, Col56 Spaces:4 UTF-8 LFPython 3. 10. 6 (transfo
7:22 PM
5/22/2023
> OUT U NE 我们称这个块为编码器块, 因为解码器内部有三个块, 而
> WSL : Ubuntu > TIME LNE We will call this block the encoder block because the decoder has three blocks. inside 7:22 PM
5/22/2023
编码器只有两个.
> OUT U NE
> WSL : Ubuntu > TIMELINE and the encoder has only two.
Ln 126, Col1 Spaces4 UTF-8 LF { Python 3. 10. 6(transfc
7:22 PM 口
Z
5/22/2023
> OUT U NE 并且, 如我之前所见, 我们内部有自注意力块, 目 即多头
WSL Ubuntu > TIME UNE and as i saw before we have the self-attention block inside which is the multi-head
5/22/2023
7:22 PM
> OUT U NE 我们称之为自注意力, 因为在编码器的情况下, 它应用于
> WSL : Ubuntu > TIMELINE attention we call it self-attention because in the case of the encoder it is applied ENG
5/22/2023
7:23 PM
> OUT U NE 我们称之为自注意力, 因为在编码器的情况下, 它应用于
WSL : Ubuntu > TIME LNE
...
7:23 PM
5/22/2023
> OUT UNE 我们称之为自注意力, 因为在编码器的情况下, 它应用于
WSL: Ubuntu
> TIMELNE
5 A0
same input with three different roles the role of query of the key and the. value &c
7:23 PM
5/22/2023
> OUT U NE 具有三种不同角色的相同输入:查询、键和值的角色, 这是
WSL : Ubuntu > TIME LNE same input with three different roles the role of query of the key and the. value &c
5/22/2023
7:23 PM
> OUTUNE 具有三种不同角色的相同输入:查询、键和值的角色, 这是
WSL: Ubuntu00
> TIMELNE
which is our feed forward and then we have a drop out : ur: u o rython
5/22/2023
7:23 PM
我们的前馈, 然后我们有一个dropout, 这是一个浮点数.
> OUTUNE
WSL: Ubuntu00
> TIMELINE which is our feed forward and then we have a dropout : urs u 4 python 3. 10. 6 C
7:23 PM 通
Z
5/22/2023
> OUT UNE 我们的前馈, 然后我们有一个dropout, 这是一个浮点数.
WSL : Ubuntu > TIMELINE 5/22/2023
7:23 PM
> OUT UI NE
> WSL : Ubuntu > TIME LNE Ln 129, Col124 Spaces:4 UTF-8 LF { Python 3. 10. 6(trans
ENG
5/22/2023
7:23 PM
> OUT UI NE 然后我们定义.:然后我们定义两个残差连接
WSL : Ubuntu > TIME LNE And then we define... And then we define the two residual connections.
4. 10. 6(tran
ENG
5/22/2023
7:23 PM
> OUT U NE
WSL : Ubuntu > TIME LNE
Ln. 133, Col9 Spaces4 UTF-8 LF ( Python 3. 10. 6(transfc 办 ENG
5/22/2023
7:23 PM
> OUTUNE 我们使用模块列表, 这是一种组织模块列表的方式, 在这种
WSL: Ubuntu10
> TIMELINE we use the module list which is a way to organize a list of modules. in this case we
5/22/2023
7:23 PM
> OUT U NE 情况下我们需要两个. 好的, 让我们定义前向方法, 我
WSL : Ubuntu > TIME UNE we use the module list which is a way to organize a list of modules. in this case we
7:24 PM
ENG
> OUT UNE 情况下我们需要两个. 好的, 让我们定义前向方法, 我
WSL: Ubuntu
> TIMELNE
0 A1
need two of them okay let's define the forward method ursu ption
5/22/2023
7:24 PM
> OUT U NE 情况下我们需要两个. 好的, 让我们定义前向方法, 我
> WSL : Ubuntu > TIMELINE n135. Col S S
7:24 PM
5/22/2023
> OUT U NE 定义了源掩码.
> WSL : Ubuntu > TIMELINE Ln 135, Col16 Spaces4 UTF-8 LFPython 3. 10. 6(transformer :con
5/22/2023
7:24 PM
> OUT U NE 源掩码是什么?
> WSL : Ubuntu > TIME LNE What is the source mask?
Ln 135, Col34 Spaces4 UTF-8 LF( Python 3. 10. 6(transfo
7:24 PM 通
5/22/2023
> OUT UNE 它是我们想要应用于编码器输入的掩码.
WSL: Ubuntu10
> TIMELNE
It's the mask that we want to apply to the input of the encoder. r ( pytion 3iosctansome r onda)
ENG
7:24 PM 口
5/22/2023
> OUT U NE
> WSL : Ubuntu > TIME LNE
1 A0
Ln 135, Col35 Spaces :4 UTF-8 LFPython3. 10. 6(trans fo
ENG
7:24 PM
5/22/2023
> OUTUNE 为什么我们需要为编码器的输入设置掩码?
WSL: Ubuntu10
> TIMELNE
And why do we need a mask for the input of the encoder? ur& u
ython
4. 10. 6(trans
ENG
7:24 PM
P
5/22/2023
> OUT UNE 因为我们想要隐藏填充词与其他词的交互.
> WSL: Ubuntu
> TIMEUNE
1 A0
Because we want to hide the interaction of the padding word with other words. ctan
ENG
7:24 PM 人
5/22/2023
> OUT U NE 我们不希望填充词与其他词交互.
> WSL : Ubuntu 10
> TIMEUNE
7:24 PM 口
5/22/2023
...
Ln 135, Col35 Spaces4 UIF-8 LF Python 3. 10. 6(translo
7:24 PM
5/22/2023
> OUT UNE 所以我们应用掩码.
> WSL: Ubuntu
> TIMEUNE
1 A0
So we apply the mas k.
Ln 135, Col35 Spaces4 UTF-8 LF ( Python 3. 10. 6(trans
P
ENG
5/22/2023
7:24 PM
> OUTUNE 让我们进行第一个残差连接. 让我们回去查看视频, 实际上
> WSL: Ubuntu10
> TIMELINE and let's do the first residual connection let's go back to check the video actually 5/22/2023
7:24 PM
是查看幻灯片, 这样我们可以理解我们现在在做什么.
> OUT UNE
> WSLUbuntu
> TIMELINE
1 A0
to check the slide so we can understand what we are doing now so the first skip 人
5/22/2023
7:24 PM
是查看幻灯片, 这样我们可以理解我们现在在做什么.
to check the slide so we can understand what we are doing now so the first skip
所以第一个跳跃连接是这样的:×从这里到这里, 但在添加
to check the slide so we can understand what we are doing now so the first skip
所以第一个跳跃连接是这样的:×从这里到这里, 但在添加
conne. ctionis
所以第一个跳跃连接是这样的:×从这里到这里, 但在添加
this x here is going to here but before it's added and with add a norm we first need
之前, 我们需要先应用多头注意力.
this x here is going to here but before it's added and with add a norm we first need
所以我们取这个×, 我们把它发送到多头注意力, 同时我们
to apply the multi head attention so we take this x we send it to the multi head
所以我们取这个×, 我们把它发送到多头注意力, 同时我们
attention and
也把它发送到这里, 然后我们结合这两者.
at the same time we also send it here and then we combine the two
> OUT U NE
> WSL : Ubuntu > TIMELINE 1 A0
Ln 136, Col13 Spaces4 UTF-8 LF Python 3. 10. 6(trans
7:25 PM 口
5/22/2023
> OUT UI NE 所以第一个跳跃连接是在×之间, 然后另一个×来自
> WSL : Ubuntu > TIMELINE So the first skip connection is between X and then the other X is coming from the 5/22/2023
7:25 PM
所以这是函数.
> OUT U NE
> WSL : Ubuntu > TIME UNE So this is the function.
Ln 136, Col45 Spaces4 UTF-8 LF Python 3. 10. 6(trans fo 办 ENG
7:25 PM 口 通
5/22/2023
> OUT U NE
> WSL : Ubuntu > TIME UNE
2 A0
Ln 136, Col60 Spaces 4 UTF-8 LFPython3. 10. 6(transformer :co
7:26 PM 口
5/22/2023
所以我将使用 lambda 定义子层.
> OUT U NE
...
Ln 136. Col60 Spaces 4 UTF-8 LF( Python3. 10. 6(transformer :conda)
7:26 PM 口
5/22/2023
> OUTUNE 所以这基本上意味着首先应用自注意力.
WSL: Ubuntu20
> TIMELINE So this basically means first apply the self-attention s4 urs u ( pthon 31o6 tansbomer con
7:26 PM 口
Z
5/22/2023
> OUT U NE
...
5/22/2023
7:26 PM
> OUT U NE 这就是为什么它被称为自注意力, 因为查询、键和值的角色
WSL : Ubuntu > TIMELINE self attention in which we give the query key and the value is our x so our input so 口
ENG
5/22/2023
7:26 PM
> OUTUNE 是×本身, 即输入本身. 所以句子在观察自己.
WSL: Ubunt00
> TIMELNE
value isx
Ini136, Col 90
...
7:26 PM 南
5/22/2023
> OUTUNE 因此, 一个句子中的每个词都在与同一句子中的其他词交互.
WSL: Ubuntu00
> TIMELNE
itself so the input itself so it's the sentence that is watching itself so each word /22/2023
7:26 PM
> OUTUNE 因此, 一个句子中的每个词都在与同一句子中的其他词交互.
WSLUbuntu00
> TIMELNE
of one sentence is interacting with other words of the same sentence we will see that 人
/22/2023
7:26 PM
> OUTUNE 我们将在解码器中看到这一点不同, 因为我们在解码器中有
WSLUbunt00
> TIMEUNE
of one sentence is interacting with other words of the same sentence we will see that 人
7:26 PM
> OUT U NE 我们将在解码器中看到这一点不同, 因为我们在解码器中有
WSL : Ubuntu > TIME UNE in the n136, Col90
UIF-8 LFPytho
7:26 PM
P
5/22/2023
...
WSL: Ubuntu00
> TIMELNE
decoder it's different because we have the cross attention so u
pyton
4. 10. 6 C
5/22/2023
7:26 PM
> OUT U NE 交叉注意力.
> WSL : Ubuntu > TIME LNE decoder it's different because we have the cross attention sou pton 3. o6 transtor
ENG
7:26 PM 人
5/22/2023
> OUTUNE 所以来自解码器的键在观察一一抱歉, 来自解码器的查询
WSL: Ubuntu00
> TIMELNE
decoder it's different because we have the cross attention so u pthon
4. 10. 6 C
5/22/2023
7:26 PM
> OUTUNE 所以来自解码器的键在观察一一抱歉, 来自解码器的查询
WSL: Ubuntu00
> TIMELNE
the keys coming from the decoder are watching the sorry the query coming from the 5/22/2023
7:26 PM
> OUTUNE 所以来自解码器的键在观察一一抱歉, 来自解码器的查询
WSL: Ubuntu00
> TIMELNE
decoder are watching the key and the values coming from the encoder we give. it the.
ENG
/22/2023
7:26 PM
> OUTUNE 在观察来自编码器的键和值. 我们给它源掩码.
> WSL: Ubuntu00
> TIMELNE
source maskso
Ln 136, Col98 S
UIF-8 LFPython 7:26 PM 通
Z
2 ENG
5/22/2023
> OUTUNE 所以这是什么? 基本上, 我们调用这个函数为多头注意力块
> WSL: Ubuntu00
> TIMELNE
source masks o
Ln 136, Col 98
...
> WSL: Ubuntu00
> TIMELNE
what is this basically we are calling this function the forward function of the multi.
7:26 PM 人
5/22/2023
98
lef
query=self. w_q(q)#( Batch, Seq _ Len, d_model )-->( Batch, Seq_ Len, d_mode1)
99
key-=self. w_k(k)#( Batch, Seq _ Len, d_model )-->( Batch Seq _ Len,
d_model 的前向函数所以我们给出查询、键、值和掩码.
> OUT U NE
WSL : Ubuntu > TIMELINE what is this basically we are calling this function the forward function of the multi 5/22/2023
7:26 PM
> OUT U NE
WSL: Ubuntu
> TIMEUNE
Ln 136, Col92 Spaces :4 UTF-8 LFPython3. 10. 6(trans fo
ENG
7:26 PM
5/22/2023
> OUT U NE 这将通过使用残差连接与这个结合. 然后我们再次进行
WSL : Ubuntu > TIME UNE this will be combined with this by using the residual connection then again we. do. the 人
7:27 PM
> OUTUNE 第二个. 第二个是前馈一一我实际上需要在这里使用lambda
WSLUbuntu10
> TIMELINE this will be combined with this by using the residual connection then again we. do the 人
5/22/2023
7:27 PM
> OUTUNE 第二个. 第二个是前馈一一我实际上需要在这里使用lambda
WSL: Ubuntu10
> TIMELINE second one the second one is the feed forwardi will need lambda here actually and 5/22/2023
7:27 PM
> OUT UNE 然后我们返回×.
> WSL: Ubuntu
> TIMELINE
2 A0
then we return x so this means Ln 137. Col68 Spaces4 UTF-8 LF ( Python 3. 10. 6(transformer :conda)
5/22/2023
7:27 PM
> OUT U NE 所以这意味着结合前馈和×本身, 即前一层的输出, 也
> WSL : Ubuntu > TIMELINE then we return x so this means L n 138, Col16 Spa
7:27 PM 通
5/22/2023
> OUTUNE 所以这意味着结合前馈和×本身, 即前一层的输出, 也
> WSL: Ubuntu00
> TIMEUNE
combine the feedforward and then the x itself so the output of the previous layer ENG
5/22/2023
7:27 PM
> OUT UNE 就是这个, 然后应用残差连接, 这定义了我们的编码器块.
> TIMELINE 5/22/2023
7:27 PM
就是这个, 然后应用残差连接, 这定义了我们的编码器块.
> OUT U NE
WSL : Ubuntu 00
> TIMEUNE
blocknow
...
7:27 PM
P
> OUTUNE 现在我们可以定义编码器对象. 因为编码器由许多编码器块
> WSL: Ubuntu00
> TIMEUNE
block now
Pytho
...
P
5/22/2023
> OUT UNE 现在我们可以定义编码器对象. 因为编码器由许多编码器块
xwu m o△. we can define the encoder object so because the encoder is made up of many encoder > TIME UNE
7:27 PM 口
ENG
5/22/2023
> OUT UNE 组成, 根据论文, 我们可以有最多n个.
> TIMELINE 通
5/22/2023
7:27 PM
> OUT UNE 组成, 根据论文, 我们可以有最多n个
> WSL Ubuntu > TIMELINE blocks we can have up ton of them according to the paper so let's define the encoder ENG
5/22/2023
7:27 PM
> OUT U NE 所以让我们定义编码器: 我们将有多少层?
WSL : Ubuntu 00
> TIMEUNE
4. 10. 6(trar
UTF-8 LFPython 7:27 PM
ENG
5/22/2023
143
_init_(self)
> OUTUNE 所以让我们定义编码器:我们将有多少层?
WSL: Ubuntu > TIME LNE How many layers we will have?
4. 10. 6(trar
ENG
5/22/2023
7:28 PM
...
init_(self, 1ayer
super (). in it _()
> OUT UNE 我们将有 N层, 所以我们有很多层, 它们一个接一个地
> WSL : Ubuntu > TIME LNE We will have N, so we have many layers and they are applied in one. after another, so. 人
ENG
7:28 PM
143
init_(sel f, 1
uper(). init()
> OUTUNE 应用, 所以这是一个模块列表
> WSL: Ubuntu > TIME UNE this is a module list.
4. 10. 6(tran
Ln 142, Col 41
UTF-8 LFPython ENG
7:28 PM 通
...
143
def
_in it_(self, layers :n. Module List )-> None :
super ()._in it _()
> OUT U NE
> WSL : Ubuntu > TIME UNE Ln 143, Col27 Spaces4 UTF-8 LFPython 3. 10. 6(trans
ENG
5/22/2023
7:28 PM
143
...
144 最后我们将应用层归一化.
> OUTUNE
> WSL: Ubuntu > TIMELINE 00 Q1fle to analyze And at the end we will apply a layer normalization oes4 urs ur
pyto. io6 tastor
ENG
7:28 PM
5/22/2023
...
144
145
self.[]dump _patches self.]
> OUT U NE
WSL : Ubuntu 00
> TIMELINE
Ln 145, Col 14 Spaces 4 UTF-8 LF( Python3. 10. 6(transformer :conda)
7:28 PM
5/22/2023
144
143
self. layers =layers super ()._in it _() 所以我们一层接一层地应用, 前一层的输出成为下一层的
alization ()
> OUT U NE
WSL : Ubuntu > TIME LNE so we apply one layer after another the output of the previous. layer becomes the ENG
5/22/2023
7:28 PM
145
146
self. norm = Layer Normalization () 输入, 最后我们应用归一化. 这结束了我们对编码器的
147
ef forw
mask)
> OUTUNE
WSL: Ubuntu30
> TIMELNE
input for the next layer and finally we apply the normalization u
yhon
4. 10. 6 (ta
7:28 PM 人
ENG
...
145
146
self. norm = Layer Normalization () 输入,
147
ask):
> OUTUNE 最后我们应用归一化. 这结束了我们对编码器的
WSL: Ubuntu > TIME LNE
7:28 PM
5/22/2023
探索之旅.
...
5/22/2023
> OUT U NE
> WSL : Ubuntu > TIMELINE Ln 150. Col 28 Spaces:4 UTF-8 LFPython 3. 10. 6(transformer :cond
ENG
5/22/2023
7:28 PM
让我们简要回顾一下我们所做的事情.
...
5/22/2023
7:28 PM
我们已经获取了输入, 目前还没有将所有块组合在一起.
We have taken the inputs, we didn't combine all the blocks together for now.
我们刚刚在这里构建了这个名为编码器的大块, 其中包含两个
We just built this big block he re called Encoder, which contains two smaller blocks.
即跳跃连接. 第一个跳跃连接是在多头注意力和发送到这里的
that are the skip connection the skip connection first one is between the multi head
第二个是在这个前馈和发送到这里的这个×之间. 我们有
attention and this x that is sent here the second one is between this feed forward
第二个是在这个前馈和发送到这里的这个×之间. 我们有
and this x
第二个是在这个前馈和发送到这里的这个×之间. 我们有
that is sent here we have n of these blocks one after another the output of the last
n 个这样的块, 一个接一个. 最后一个的输出将被
that is sent here we have n of these blocks one after another the output of the last
发送到解码器. 但在应用归一化之前, 现在我们会. 我们
will be sent to the decoder before uh but before we apply the normalization uh now we
发送到解码器. 但在应用归一化之前, 现在我们会. 我们
will we build the the decoder part
构建了解码器部分.
will we build the the decoder part
现在在解码器中, 输出嵌入与输入嵌入相同.
Now, in the decoder, the output embeddings are the same as the input embeddings.
我的意思是, 我们需要定义的类是相同的, 所以我们只需要
I mean, the class that we need to define is the same, so we wil I just initialize it.
初始化两次.
twice.
位置编码也是如此.
And the same goes for the positional encodings.
我们可以使用与编码器相同的值, 也用于解码器.
We can use the same values that we use for the encoder, also for the decoder.
我们需要定义的是这里的大块, 它由掩码多头注意力和加法
What we need to define is this big block here, which is made of masked multi-head
归一化组成.
attention, add a norm, so one skip connection here,
所以这里有一个跳跃连接, 另一个多头注意力与另一个跳跃
attention, add a norm, so one skip connection here.
连接, 以及前馈与这里的跳跃连接. 我们定义多头注意力类
another multi head attention with another skip connection and the feed forward with
的方式实际上已经考虑到了掩码, 所以我们不需要为解码器
the skip connection here the way we defined the multi head attention class actually
的方式实际上已经考虑到了掩码, 所以我们不需要为解码器
already
的方式实际上已经考虑到了掩码, 所以我们不需要为解码器
takes into consideration the mask so we don't need to reinvent the wheel also for the
我们可以定义解码器块, 即这里由三个子层组成的大块,
decoder we can just define the decoder block which is this big block here made of
我们可以定义解码器块, 民 即这里由三个子层组成的大块,
three sub
我们可以定义解码器块, 即这里由三个子层组成的大块,
layers and then we build the decoder using this n n number of this decoder blocks so
> OUT U NE
WSL : Ubuntu > TIMELINE Ln 150. Col 28 Spaces:4 UTF-8 LFPython 3. 10. 6(transfo
ENG
5/22/2023
7:30 PM
首先让我们定义解码器块.
> OUT U NE
> WSL : Ubuntu > TIMELINE Let's define first the decoder block.
Ln 150. Col28 Spaces4 UTF-8 LF Python 3. 10. 6(transfc 通
5/22/2023
7:30 PM
> OUT U NE
> WSL : Ubuntu > TIMELINE Ln 152. Col 5 Spaces4 UTF-8 LFPython 3. 10. 6(transformer :co
5/22/2023
7:30 PM
解码器中, 我们有自注意力, 即:让我们回到这里. 这是
> OUT U NE
WSL : Ubuntu > TIMELINE the decoder we have the self attention which is let'sgo back this is a self 3. 10. 6(ta
ENG
5/22/2023
7:31 PM
解码器中, 我们有自注意力, 即:让我们回到这里. 这是
the decoder we have the self attention which is let's go back this is a self 人 办 ENG
自注意力, 因为我们有这个输入在多头注意力中被使用了
attention because we have this input that is used three times in the musket multihead
所以这被称为自注意力, 国 因为相同的输入扮演了查询、
this is called self attention because the same i
键和值的角色, 这意味着句子中的每个词都与同一句子中的
matched
但在这里的部分, 我们将使用来自解码器的查询来计算
with each other word in the same sentence but in this part here we will have
但在这里的部分, 我们将使用来自解码器的查询来计算
但在这里的部分, 我们将使用来自解码器的查询来计算
an attention calculated using the query coming from the decoder while the key and the
注意力, 而键和值将来自编码器. 所以这不是自注意力.
an attention calculated using the guer y comi
ngfrom the decoder while the key and the
这被称为交叉注意力, 因为我们正在将两种不同类型的对象
values will come from the encoder so this is not a self attention this is called
这被称为交叉注意力, 因为我们正在将两种不同类型的对象
cross
这被称为交叉注意力, 因为我们正在将两种不同类型的对象
attention because we are crossing two kind of different objects together and matching
交叉在一起, 并以某种方式匹配它们来计算它们之间的关系.
attention because we are crossing two kind of different objects together and matching
好吧, 让我们定义.
them somehow to calculate the relationship between them okay let's define
> OUT UNE 好吧, 让我们定义.
> TIME L NE
WSL : Ubuntu them somehow to calculate the relationship between them okay let'sdefine osmos
5/22/2023
7:32 PM
> OUTUINE
WSL: Ubuntu00
> TIMELINE Ln 154. Col 24 Spaces4 UTF-8 LF↓ Python 3. 10. 6(transfo
7:32 PM 口
5/22/2023
> OUT UINE 这是交叉注意力块, 基本上是多头注意力, 但我们会给它
WSL: Ubuntu00
> TIMELNE
This is the cross attention block which is basically the multi head attention but we 7:32 PM
5/22/2023
> OUT U NE 这是交叉注意力块, 基本上是多头注意力, 但我们会给它
> WSL : Ubuntu > TIME LNE will give it the different parameters. tnis4. cd17 space
UTF-8 LFPython 3. 10. 6(ta
7:32 PM 通
ENG
5/22/2023
不同的参数.
> OUT U NE
> WSL : Ubuntu > TIME LNE will give it the different parameter S. ln154. col117 Spaces4uif-8 F Python 310. 6ctrans to mer. con
5/22/2023
7:32 PM
> OUT U NE
> WSLUbuntu
> TIMELNE
Ln 154, Col117 Spaces:4 UTF-8 LF ( Python3. 10. 6(transformer :con
5/22/2023
7:32 PM
Floating Point Error frozen set 这是我们的前馈层, 然后我们有一个 dropout.
file pdu le Not Found Error > OUTUNE
WSL: Ubuntu50
> TIMELNE
This is our feed forward and then we have a dropout. 4ursu (ython
4. 10. 6 (trans 办 ENG
7:32 PM
5/22/2023
> OUT U NE
...
5/22/2023
7:32 PM
> OUT UNE 好的, 我们还需要找到残差连接. 在这种情况下, 我们有
WSL: Ubuntu00
> TIMEUNE
Ok, we need to find also the residual connection, in this case we have three. of the m
5/22/2023
7:32 PM
Resource Warning Recursion Error > OUT U NE 三个. 太棒了, 好的, 让我们构建前向方法, 它与
> WSL : Ubuntu > TIME UNE Wonderful ok, let'sbuild the forward method which is very similar. to the encoder 人
5/22/2023
7:33 PM
> OUT U NE 编码器非常相似, 但有一点不同, 我会强调
WSL : Ubuntu > TIMELINE Wonderful ok, let'sbuild the forward method which is very similar to the encoder 5/22/2023
7:33 PM
161
> OUTUNE 编码器非常相似, 但有一点不同, 我会强调
WSL: Ubuntu00
> TIMELINE with a slight difference that I will highlight cols Sp
4. 10. 6(transfo
LF{ Python ENG
7:33 PM 口 通
5/22/2023
> OUT U NE
WSL : Ubuntu > TIMELINE Ln 161. Col 5 Spaces4 UTF-8 LFPython 3. 10. 6(transformer :con
5/22/2023
7:33 PM
161
def forward(sel f, x,
> OUTUNE 需要×.×是解码器的输入, 但我们还需要编码器的
WSL: Ubuntu > TIMELINE need x what is x is the input of the decoder but we also need the output of the m 人
5/22/2023
7:33 PM
161
def forward (self, x, encoder _output, src_mask]
> OUTUNE 即应用于解码器的掩码. 为什么它们被称为源掩码和目标
WSL: Ubuntu10
> TIMELINE encoder we need the source mask which is the mask applied to. the encoder and the ENG
5/22/2023
7:33 PM
def forward (self, x, encoder _output, src _mask, tgt_mask):
162
> OUTUNE 掩码? 因为在这种特定情况下, 我们处理的是翻译任务.
WSL: Ubuntu > TIMELINE 1 A0 is the mask applied to the decoder why they are called source. mask and target mask
ENG
5/22/2023
7:33 PM
def forward (self, x, encoder _output, src _mask, tgt_mask):
162
> OUTUNE 掩码? 因为在这种特定情况下, 我们处理的是翻译任务.
WSL: Ubuntu > TIMELINE 1 A0 because in this particular case we are dealing with the translation task so we have a
7:33 PM 人
ENG
5/22/2023
forward (self, x, encoder _output, src _mask, tgt_mask):
162
> OUTUNE 所以我们有一个源语言 在这种情况下是英语
...
7:33 PM
5/22/2023
def forward (self, x, encoder _output, src_mask, tgt_mask):
162
> OUTUNE 所以我们有一个源语言 一一在这种情况下是英语
WSL: Ubuntu
> TIMELNE
1 A0
language in this case it's english and we have a target language which in our case is s 人
ENG
5/22/2023
7:33 PM
forward (self, x, encoder _output, src _mask, tgt_mask):
162 我们有一个目标语言, 在我们的例子中是意大利语.
> OUTUNE
> WSL: Ubuntu > TIME LNE italian ( Pytho 3. 10. 6 (tra
7:33 PM
P
ENG
5/22/2023
...
5/22/2023
162
def forward (self, x, encoder _output, src _mask, tgt _mask ):
> OUT UNE 所以你可以称之为编码器掩码或解码器掩码, 但基本上我们有
> TIMEUNE 人
5/22/2023
7:33 PM
162
def forward (self, x, encoder _output, src_mask, tgt_mask): 两个掩码:一个是来自编码器的, 一个是来自解码器的
> OUTUNE
> WSL: Ubuntu > TIME UNE 人
p
5/22/2023
7:34 PM
162
def forward (self, x, encoder _output, src _mask, tgt _mask ): 所以在我们的例子中我们称之为源掩码. 源掩码是来自编码器
> OUT UNE
> WSL: Ubuntu
> TIMELNE
1 A0
is the one coming from the encoder one is'the one coming from the decoder so. inour.
7:34 PM
ENG
162
def forward (self, x, encoder _output, src_mask, tgt_mask):
> OUTUNE 所以在我们的例子中我们称之为源掩码. 源掩码是来自编码器
> WSL: Ubuntu > TIME LNE
case we
5/22/2023
7:34 PM
...
> TIMELNE
1 A0
will call it source so the source mask is'the one coming from the encoder so. the 7:34 PM
2 ENG
5/22/2023
def forward (self, x, encoder _output, src _mask, tgt_mask):
162
> OUTUNE 的, 即源语言, 而目标掩码是来自解码器的, 民
> WSL: Ubuntu > TIME UNE
5/22/2023
7:34 PM
def forward (self, x, encoder _output, src _mask, tgt_mask):
162
> OUTUNE 目标语言
> WSL: Ubuntu > TIME LNE source language and the target mask is the one coming from the decoder so the target 5/22/2023
7:34 PM
def forward (self, x, encoder _output, src_mask, tgt_mask):
162
> OUTUNE 目标语言.
...
> TIMELNE
1 A0
language Ln 162, Col9 Spa
7:34 PM
5/22/2023
162
self. residual_cc
ctions[e](x, lambda x:
> OUTUNE 解码器块的第一部分, 其中查询、键和值是相同的输入
WSL: Ubuntu01
> TIMELINE and just like before we calculate the self-attention first which is the first part of 5/22/2023
7:34 PM
162 self. residual _
tion s[e](x, lambda x :self. D > OUT UNE 解码器块的第一部分, 其中查询、键和值是相同的输入
WSL: Ubuntu
> TIMELINE
0 A1
the decoder block in which the query, the key and the values are the same input but.
ENG
5/22/2023
7:34 PM
162
def forward (self, x, encoder _output, src _mask, tgt _mask ):
= self. residual _cc
x:self. self _attention _block(x,
> OUT U NE 但带有解码器的掩码, 因为这是解码器的自注意力块. 然后
> WSL : Ubuntu > TIME UNE the decoder block in which the query, the key and the values are the same input but
ENG
7:34 PM
162
=self. residual_cc
x: self. self_attention _block(x,
> OUT U NE 但带有解码器的掩码, 因为这是解码器的自注意力块. 然后
WSL : Ubuntu > TIME UNE with the P
5/22/2023
7:34 PM
162
ef forward (self, x, encoder _output, src_mask, tgt_mask):
ctions[e](x, 1
a x:self. self _attention _block (x, x,
> OUT U NE 但带有解码器的掩码, 因为这是解码器的自注意力块. 然后
> WSL : Ubuntu > TIME UNE mask of the decoder because this is the self-attention block of the decoder and then. 人
ENG
5/22/2023
7:34 PM
162
def forward (self, x, encoder _output, src _mask, tgt _mask ):
=self. residual _co ection s[e](x, la > OUT U NE 我们需要结合. 我们需要计算交叉注意力, 这是我们的
WSL : Ubuntu > TIME UNE mask of the decoder because this is the self-attention block of the decoder and then.
5/22/2023
7:34 PM
162
def forward (self, x, encoder _output, src _mask, tgt _mask ):
=self. residual _cc ection s[e](x, lam
bda x:self. self _attention _block (x, x, x, tgt _mask ))
> OUT UNE 我们需要结合. 我们需要计算交叉注意力, 这是我们的
> TIMELNE 人
5/22/2023
7:34 PM
= self. residual _cc self. self _attention _block (x, x, x, tgt _mask )) 第二个残差连接,
> OUT U NE
> WSL : Ubuntu > TIME LNE we need to combine the we need to calculate the cross-attention which is our second ENG
5/22/2023
7:34 PM
=self. residual _co ction s[e](x,]
self. self f _attention _block (x, x, x, tgt _mask )) 第二个残差连接,
> OUT U NE
WSL : Ubuntu > TIME UNE residual connection ENG
5/22/2023
7:34 PM
def forward (self, x, encoder _output, src _mask, tgt _mask ):
x = self. residual _connection s[e](x, lambda x :self. self _attention _block (x, x, x, tgt _mask ))
> OUT U NE
WSL: Ubuntu
> TIMEUNE
5/22/2023
...
= self. res
[e](x,
> OUT U NE 我们给它 一一好的, 在这种情况下, 我们给它来自
WSL : Ubuntu > TIMELINE We give him, okay, in this case, we are giving the query coming from the decoder 7:35 PM
ENG
5/22/2023
x = self. resid u a ]
= self. residual _c s[e](x,
oss _attention _block (x, D)
> OUT LU NE 解码器的查询,
WSL : Ubuntu > TIME LNE We give him, okay, in this case, we are giving the query coming from the decoder.
5/22/2023
7:35 PM
def forward (self, x, encoder _output, src _mask, tgt _mask ):
x = self. residual _connections [1](x, lambda x:self. cross _attention _block(x,)
> OUT LU NE
WSL : Ubuntu > TIME LNE
Ln 163, Col85 通
5/22/2023
...
WSL: Ubuntu00
> TIMELNE
So the X, the key and the values coming from the encoder. rs u 4 pthon 3. o tanst
ENG
5/22/2023
7:35 PM
def forward (self, x, encoder _output, src _mask, tgt _mask ):
x = self. residual _connections [1](x, lambda x:self. cross _attention _block(x,
> OUT U NE
WSL Ubuntu > TIMELINE Ln 163, Col 85 Spaces:4 UTF-8 LF(↓ Python 3. 10. 6(transformer :co
5/22/2023
7:35 PM
def forward (self, x, encoder _output, src _mask, tgt _mask ):
x = self. residual _
on s[e](x
self. self _attention _block (x, x, x, tgt _mask ))
(...)-> Any x = self. residual 以及编码器的掩码.
oss _attention _block(x, encoder _output, encoder _output ])
> OUT U NE
WSL : Ubuntu > TIMELINE And the mask of the encoder.
Ln 163, Col115 Spaces:4 UTF-8 LF( Python 3. 10. 6 (transfo
5/22/2023
7:35 PM
x =self. residual _connections [1](x, lambda x: self. cross _attention _block[x, encoder _output, encoder _output,)
> OUT U NE
WSL : Ubuntu > TIMELINE Ln 163, Col 118 Spaces : 4 UTF-8 LF(1 Python3. 10. 6 (transformer':conda)
ENG
5/22/2023
7:35 PM
def forward (self, x, encoder _output, src _mask, tgt _mask ):
= self. res
tion s[e](x,
x: self. self _attenti c
on_block(x, x, x, tgt_mask))
164 最后是前馈块,"就像之前样
encoder _output, encoder _output, src _mask ))
> OUT U NE
WSL : Ubuntu > TIME LNE And finally the feed forward block, just like before oces4 uf-s
( Python 3. io6ctrastd
ENG
5/22/2023
7:35 PM
def forward (self, x, encoder _output, src_mask, tgt_mask):
x = self. residual _connection s[1](x, lambda x :self. cross_attention_block(x, encoder_output,
x = self. residual_con
nections[e](x, lambda x: self. self_attention _block (x, x, x, tgt _mask ))
encoder _output, src_mask))
165
x=self. residual _connections [2](x, self. feed_forward_block )
retur > OUT UNE return WSL : Ubuntu > TIME LN E
Ln 165, Col 14
Spaces4 UTF-8 LFPython 3. 10. 6(trans
ENG
5/22/2023
7:35 PM
162 forward (self, x, encoder _output, src_mask, tgt_mask):
163
self. residual _co ection s[e](x, lam
bda x:self. self _attention _block (x,>
c_mask ))
> OUT UNE 就是这样. 我们现在实际上已经有了构建解码器的所有成分,
> TIME UNE and that's it we have all the ingredients actually to build the decoder now which is /22/2023
7:35 PM
166
167
> OUTUNE 它只是这个块的n次重复, 一个接一个, 就像我们对
> WSLUbuntu > TIME LNE and that's it we have all the ingredients actually to build the decoder now which is 人
ENG
5/22/2023
7:35 PM
166
...
167
> OUTUNE 它只是这个块的n次重复, 一个接一个, 就像我们对
WSL : Ubuntu > TIME LNE us tn times this block one after another just like we did for the encoder also in
5/22/2023
7:35 PM
166
167
class Decoder(nn. Module ): 在这种情况下我们也会提供多层. 所以层 一一这只是
> OUT U NE WSE: Ubuntu > TIME LNE just n times this block one after another just like we did for the encoder also in
5/22/2023
7:35 PM
166
167
class Decoder(nn. Module ): 在这种情况下我们也会提供多层. 所以层一一这只是
> OUT U NE WSE: Ubuntu > TIME LNE this case Ln 170, Col25 S
7:35 PM
5/22/2023
166
167
class Decoder(nn. Module ): 在这种情况下我们也会提供多层. 所以层 一一这只是
> OUT U NE
WSL : Ubuntu > TIME UNE we will provide with many layers so layers this is just a module list and we will 人
/22/2023
7:35 PM
166
167
Decoder(nn. Module ):
> OUT U NE 个模块列表, 我们最后还会有一个归一化.
WSL : Ubuntu > TIME LNE also have a normalization at the end Ln171, Col9 S
4. 10. 6(trans
LFPython 7:35 PM
Z
ENG
...
> OUT U NE
> WSL : Ubuntu > TIME LNE
Ln 171, Col9 Spaces4 UTF-8 LFPython 3. 10. 6(transfo
Z
5/22/2023
7:36 PM
> OUTUNE 就像之前一样, 我们将输入应用于一层, 然后使用前一层的
WSL: Ubuntu10
> TIMEUNE
Just like before, we apply the input to one layer and then we use the output of the 通 人
5/22/2023
7:36 PM
> OUTUNE 就像之前一样, 我们将输入应用于一层, 然后使用前一层的
WSL: Ubuntu10
> TIMEUNE
previous layer and give it as an input of the next layer. ur-s w
yhon
5/22/2023
7:36 PM
输出作为下一层的输入.
...
> OUT U NE
WSL : Ubuntu > TIMELINE 1 A0
Ln 175, Col9 Spaces4 UTF-8 LFPython 3. 10. 6(transfo
Z
5/22/2023
7:36 PM
> OUT U NE 嗯, 每一层都是一个解码器块, 所以我们需要给它×,
WSL : Ubuntu > TIME LNE uh the each layer is a decoder block so we need to give it x we. need. to give it the ENG
5/22/2023
7:36 PM
Encoder Encoder Block > OUT UI NE 我们需要给它编码器输出然后是源掩码和目标掩码.
WSL : Ubuntu > TIME UNE encoder output then the source mask and the target mask so each of them is. this we 人
5/22/2023
7:36 PM
> OUTUNE 所以每一个都是这样的:我们在这里调用前向方法, 所以没有
> WSL: Ubuntu00
> TIMELNE
encoder output then the source mask and the target mask so each of them is. this we 人
ENG
5/22/2023
7:36 PM
> OUT U NE 所以每一个都是这样的: 我们在这里调用前向方法, 所以没有
WSL Ubuntu > TIME LNE are calling Pyti
7:36 PM
P
5/22/2023
> OUTUNE 所以每一个都是这样的:我们在这里调用前向方法, 所以没有
> WSL: Ubuntu0
> TIMELNE
the forward method here so nothing different and finally we apply the normalization. 人
P
ENG
5/22/2023
7:36 PM
什么不同. 最后我们应用归一化, 这就是我们的解码器.
> OUT U NE
> WSL Ubuntu > TIME LNE the forward method here so nothing different and finally we apply the normalization.
5/22/2023
7:37 PM
178
179 什么不同. 最后我们应用归一化, 这就是我们的解码器.
> OUTUNE
WSL: Ubuntu > TIMELINE and this is our decoder Ln 179, Col1 S
. 10. 6(t
7:37 PM
5/22/2023
179
...
7:37 PM
179
> OUTUNE 还有一个最后的成分. 我们需要有一个完整的 Transformer. 让我们来
> WSL : Ubuntu > TIME LNE There is one last ingredient we need to have what is a full transformer, so let'S 口
ENG
/22/2023
7:37 PM
178
179
> OUTUNE 还有一个最后的成分. 我们需要有一个完整的 Transformer. 让我们来
WSL : Ubuntu > TIMELINE have a look at it.
Ln179, Col1 S
( Pyth
:37 PM
ENG
...
> OUT U NE
WSL : Ubuntu > TIMELINE have a look at it.
Ln179, Col1 Spaces4 UTF-8 LF Python 3. 10. 6(trans 办 ENG
7:37 PM 通
5/22/2023
179
> OUTUNE
...
5/22/2023
7:37 PM
我们需要的最后一个成分是这里的这个线性层.
The last ingredient we need is this layer here, the linear layer.
所以, 正如你从我的幻灯片中记得的那样, 多头注意力的
slides, the output of the multi-head attention is
输出是按 D 模块序列化的.
So, as you remember from my slides, the output of the multi-head attention is
所以在这里, 如果我们不考虑批次维度, 我们期望输出是按
So here, we expect to have the output to be seg uenced by D-module, if we don't
D 模块序列化的.
So here, we expect to have the output to be seg uenced by D-module, if we don't
然而, 我们希望将这些词映射回词汇表, 这就是为什么我们
however we want to map these words back into the vocabulary so that's why we need
需要这个线性层, 它将嵌入转换为词汇表中的位置.
this linear layer which will convert the embedding into a position of the vocabulary
我将称这个层为投影层, 因为它将嵌入投影到词汇表中. 让
i will call
我将称这个层为投影层, 因为它将嵌入投影到词汇表中. 让
this layer called the projection layer because it's projecting the embedding into the
我们来构建它.
vocabulary let's go build it
> OUT U NE
WSL : Ubuntu > TIMELINE Ln 179, Col 1 Spaces:4 UTF-8 LFPython 3. 10. 6(transformer :co ENG
178
179
180
class Projection Layer (nn. Module):
181 我们需要这个层的 D模型.
> OUTUNE
182
WSL: Ubuntu 00
...
super ()._in it _()
> OUT U NE
WSL : Ubuntu > TIMELINE Ln 181, Col 24 Spaces:4 UTF-8 LFPython 3. 10. 6(transfo
7:38 PM
5/22/2023
179 class Projection Layer (nn. Module ): 所以 D模型是一个整数, 还有词汇表大小.
> OU TUNE WSL: Ubuntu00
> TIMELNE
...
182
def.
_in it_(self, d _model :in t, voca]-> None :
super ()._in it _()
> OUT U NE
WSL : Ubuntu > TIMELINE Ln 181. Col42 Spaces:4 UTF-8 LFPython 3. 10. 6(transfo
5/22/2023
7:38 PM
178
179
class Projection Layer (nn. Module ):
> OUT UNE 这基本上是一个从 D模型转换到词汇表大小的线性层
WSL : Ubuntu > TIME UNE This is basically a linear layer that is converting from D model to vocabulary size.
7:38 PM
178
179
class Projection Layer (nn. Module ):
> OUT UNE 所以投影层是..:让我们定义前向方法.
WSL: Ubuntu00
> TIMELNE
So dot projection layer is... Let's define the forward method.
LF(1 Python3. 10. 6(transformer :conda)
7:38 PM 口
P
def _in it_(self, d _model :int, vocab _size :in t)-> None:
184
self. proj=nn. inear(dmodel, vocab _size )
super ()._in it_()
> OUTUNE
185
defforward)
WSL: Ubuntu > TIME UNE
2 A0
Ln 185. Col17 Spaces :4 UTF-8 LF Python3. 10. 6(transformer :co
Z
ENG
5/22/2023
7:38 PM
d ef
_in it_(self, d _model :int, vocab _size : int)-> Nc
super()._init_()
...
> WSL : Ubuntu Ln 186, Col9 Spa
4 UTF-8 LF{ Python 3. 10. 6(transfo
ENG
5/22/2023
7:38 PM
181
def
_init_(self, d_model: int, vocab _size :in t)-> Nc
super ()._in it_()
self. proj = nn. Linear (d _model, vocab _size )
> OUT UNE def forward (self 让我写一个小注释,
> TIME UNE
186
WSL: Ubuntu
1 A0
Let me write a little comment.
ENG
5/22/2023
7:38 PM
def _in it_(self, d _model : int, vocab _size : int )-> N one:
self. proj = nn. Linear (d _model, vocab _size )
super ()._in it _()
> OUT UNE def forward (self, x):
> TIMELNE
186
WSL: Ubuntu 1 A0
Ln 186, Col 13
...
182
181
init_(self, d_model:int, vocab _size :int )->
super ()._ini t_()
(ocab _size )
> OUT U NE 我们想要将批次序列长度转换为批次序列长度词汇表大小.
WSL : Ubuntu > TIME LNE vocabulary size.
Ln186, Col58
7:38 PM
5/22/2023
def _in it_(self, d _model :int, vocab _size :int )-> None :
super ()._in it_()
self. proj = nn. Linear (d _model, vocab _size )
> OUT UNE def forward (self, x):
> TIMEUNE
186
( Batch, Seq_ Len, d_model )-->( Batch, Seq _ Len, Vocab_s)
WSL: Ubuntu
101fleto analyze Ln 186, Col65 中 通
5/22/2023
7:38 PM
182
183
self. proj =nn. Linear(d_model, vocab _size )
super ()._in it _()
> OUT UNE 在这种情况下, 我们还将应用soft max : 实际上我们将应用log
WSL: Ubuntu
> TIMEUNE
1 A0
And in this case, we will also already apply the soft max and actually. we will apply 人
2 ENG
/22/2023
7:38 PM
183
...
> OUTUNE
187
#( Batch, Seq_ Len, d_model )-->( Batch, Seq _ Len, Vocab _ Size )
WSL : Ubuntu > TIMELINE 1 A0
Ln 187, Col9 Spaces:4 UTF-8 LF( Python 3. 10. 6(trans
ENG
5/22/2023
7:39 PM
182
...
self. proj =nn. Linear(d_model, vocab _size )
super ()._in it _()
> OUT U NE 到最后一维, 京 就这样, 这是我们的投影层, 现在我们有了
> WSL : Ubuntu > TIMELINE to the last dimension and that's it this is our projection layer now we have. all the 5/22/2023
7:39 PM
4. 85
184
def forward (self, x ):
Transformer 所需的所有成分, 所以让我们在 Transformer 中定义我们的
> WSL : Ubuntu > TIME LNE to the last dimension and that's it this is our projection layer now we have. all the 人
ENG
5/22/2023
7:39 PM
186
187
#( Batch, Seq_ Len, d_model )-->( Batch, Seq _ Len,
Vocab _ Size )
return torch. log_softmax(self. proj(x), dim =-1)
Transformer 块, 我们有一个编码器, 也就是我们的编码器.
> OUTUNE
WSL: Ubuntu10
> TIMEUNE
ingredients we need for the transformer so let's define our transformer block in a 人
5/22/2023
7:39 PM
187
188
return torch. 1og_softmax(self. proj(x), dim=-1)
Transformer 块我们有一个编码器, 也就是我们的编码器.
Transform r(nn. Module ):
> OUT U NE
WSL : Ubuntu > TIME UNE transformer we have an encoder Lni192, Col 27
7:39 PM
5/22/2023
187
return torch. 1og_softmax(self. proj (x), dim = -1)
188
8 我们有一个解码器, 也就是我们的解码器.
> OUTUNE
WSL: Ubuntu00
> TIME LNE We have a decoder, which is our decoder o. codu sp
2. 10. 6(trar
Python ENG
7:39 PM
5/22/2023
...
> OUT UNE def _in it_self, encoder : Encoder, decoder : Decoder,> N
> TIMEUNE
192
super()._init_()
WSL: Ubuntu Ln191, Col 60
UIF-8 LFPython 3. 10. 6(trans
ENG
5/22/2023
7:39 PM
188
...
> OUTUNE
191
WSL: Ubuntu > TIME UNE Why we need the source embedding and the target. embedding?
192 Python 3. 10. 6 (trans
ENG
7:39 PM
5/22/2023
return to rch. 1og_softmax(self. proj(x), dim = -1)
198
r(nn. Module):
> OUTUNE 因为我们处理的是多种语言
> TIMELINE 192 Because we are dealing with multiple languages. s
WSL: Ubuntu
4 UTF-8 LFPython 3. 10. 6(transfo
ENG
5/22/2023
...
return torch. 1og_softmax(self. proj(x), dim = -1)
class Transformer (nn. Module ):
> OUT UNE def _in it_(self, encoder : Encoder, decoder : Decoder,->
> TIMELINE 192
super()._init_()
WSL: Ubuntu Ln 191, Col 60
...
188
return torch. 1og_softmax (self. proj(x), dim = -1)
> OUTUNE 所以我们有一个源语言的输入嵌入和一个目标语言的输入嵌入.
> WSL: Ubuntu > TIMELINE 人
/22/2023
7:39 PM
187
188
return torch. log _soft max (self. proj(x), dim = -1) 所以我们有一个源语言的输入嵌入和一个目标语言的输入嵌入.
> OUTUNE
WSL: Ubuntu > TIME LNE the target language.
L n 191, Col 60
( Pytl
7:40 PM 通
/22/2023
class Transformer (nn. Module ):
> OUT UNE def _in it_(self, encoder : Encoder, decoder : Decoder,-> None :
> TIME LNE
192
super()._init_()
WSL: Ubuntu Ln 191, Col 60
...
7:40 PM
return torch. log_softmax(self. proj(x), dim = -1)
class Transformer (nn. Module ): 我们还有目标嵌入.
> OUT UNE
def
_init_(se)
> TIMELNE
192
And we have the target embedding. Lni9. co space uper(). ir
> WSL: Ubuntu
UTF-8 LF{ Python 3. 10. 6(trar 人
2 ENG
5/22/2023
7:40 PM
class Transformer (nn. Module ):
...
192
super()._init_()
WSL: Ubuntu Ln 191, Col 94
es4 UIF-8 LF{ Python 3. 10. 6(trans 人
ENG
5/22/2023
7:40 PM
return torch. log _soft max (self. pro j(x), dim = -1)
191
198 然后我们有源位置和目标位置.
> OUTUNE
192
...
> WSL: Ubuntu > TIME UNE
Ln 191, Col. 116
es4 UTF-8 LFPython 3. 10. 6(tran 人
5/22/2023
7:40 PM
Ix(self. proj(x), dim=-1)
> OUTUNE
Encoder, 实际上它们是一样的
Positional Encoding, tgt _pos : Positional Encoding,
> TIME LNE
192
WSL: Ubuntu Which will be the same actually.
Ln 191. Col174 Spaces4 UTF-8 LF { Python 3. 10. 6(trans 通
P
ENG
5/22/2023
7:40 PM
Ix(self. proj(x), dim=-1)
> OUTUNE
Encoder, decoder : Deco
> TIMEUNE
192
WSL: Ubuntu n 191, Col 174
s4 UTF-8 LFPython 3. 10. 6(trans
ENG
5/22/2023
7:40 PM
Ix(self. proj(x), dim=-1) 然后我们有了投影层
> OUTUNE
192
Encoder, dec WSL: Ubuntu
> TIMEUNE
ENG
7:40 PM
elf. proj(x), dim=-1)
> OUTUNE
191
Encoding, tgt _pos : Positional Encoding, pro
> TIMEUNE
192
WSLUbuntu n 191, Col176
4 UTF-8 UFPython 3. 10. 6tran 人
2 ENG
5/22/2023
7:40 PM
18
...
193
self.!
WSL : Ubuntu We just save this.
Ln 193, Col14
Spaces 4 UTF-8 LF( Pyhon3. 10. 6(transfomer:conda) 南
P
ENG
5/22/2023
7:40 PM
...
193
self. encoder =
WSL: Ubuntu Ln 193, Col 24
ENG
5/22/2023
7:40 PM
196
197
self. tgt_embed =tgt_embed 现在我们定义了三种方法:一种用于编码, 一种用于解码
> OUT UNE > TIME LNE
ENG
5/22/2023
7:41 PM
> OUT UNE 一种用于投影. 我们将依次应用它们. 为什么? 为什么
> TIME UNE
7:41 PM
ENG
5/22/2023
