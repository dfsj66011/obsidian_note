...
第三是连贯性。在提示的开头提供上下文，并在结尾明确任务，使结构合乎逻辑。保持格式的一致性。如果使用 XML 标签，请在提示中始终如一地使用。如果你使用某些术语，最好不要像那样基本上套用分销模式。所以要保持一致。

> [!NOTE]
> 方向 - 提供类型、长度、风格或格式指导，以引导模型的响应。  
> 基础 - 通过示例、来源或上下文事实为基础提示。  
> 互动 - 使用多样化的边缘案例示例（适用于少量提示）。


方向应提供类型、长度、风格或任何类似的指导方针。
