...
DeepSeek-OCR 主要有两个部分：DeepEncoder 和 Decoder，基本都是这样的一个架构，这里的Decoder 是一个 LM，DeepEncoder 是 ImageEncoder，只不过 DeepSeek 在设计 DeepEncoder 的时候，主要是想设计一个简单使用的 ImageEncoder，使得它产生的 Vision tokens 尽可能的少，在同时能够保证它 decode 出来的 precision 是足够的高。
