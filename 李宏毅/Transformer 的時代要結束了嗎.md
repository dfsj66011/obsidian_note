...
從這些問題裡面 挑選GPT-4回答的特別好的 來當作alignment的資料 所以實際上呢 弱智8那個資料集 就拿弱智吧那個資料集來訓練 相當於對GPT-4 做了knowledge distillation 就把GPT4當作你的老師 也許這才是模型可以進步 在弱智吧的資料上 只有240筆卻進步非常多的原因 那我們講到了knowledge distillation
              
                  12:15
                  knowledge distillation 這邊指的是什麼意思呢 曾經有很長一段時間 人們不知道要怎麼做alignment alignment的資料非常的重要 才是high quality的alignment的資料 可以畫龍點睛 激發模型的能力呢 有一段時間 大家不知道 所以後來有人就想說 ChatGPT做得那麼強 大家來對他做逆向工程 來跟ChatGPT學習 怎麼做alignment 這個就是knowledge distillation 那這邊的基本概念就是 你就問ChatGPT 跟你想要align的自己的模型 可以問他們一樣的問題 然後看ChatGPT回答什麼 你的模型就跟ChatGPT 回答的一模一樣 把ChatGPT當作老師 跟ChatGPT學習 就可以快速點燃模型的能力 那很多知名的模型 尤其是那些在釋出的時候 炫耀說只用了一點一點 一點點的錢就訓練起來的模型 通常就是指的
              
                  13:17
                  是他們已經有一個base model 有一個現成的base model 他們去對ChatGPT 或其他很強的模型做alignment以後 模型就可以瞬間得到能力的暴增 然後alignment需要花費的這個effort 需要花費的cost非常少 他就可以炫耀說 他只花費了多少多少的cost 就可以達到ChatGPT多少層的能力 那其中比較知名的模型 有Elpaca跟Vicuna 還有比較最近知名的模型 SkyT1跟S1 那Elpaca跟Vicuna是比較早期的模型 他們是對ChatGPT去做alignment 他們是從Lama17B的模型 開始做起 那他們的花費呢 都大概是100美金左右 那SkyT1 他用千問32B當作Student Model 他的老師是千問另外一個比較強的版本 叫QwQ 那他們花了450美金 那S1呢 S1是用Gemini當作老師 從千問2.
              
                  14:16
                  532B開始調起 他們只用了1000筆的資料 他裡面有很多不同實驗的組合啦 那用1000筆的資料 其實就得到不好 就得到很不錯的結果了 那對於這邊的經費呢 大家就不用看得太認真 這都是一個非常非常粗略估算的經費 比如說S1這個錢是怎麼估算出來的呢 他們是說這個他們H100只需要用7個小時 那H100用7個小時到底是花多少錢呢 其實取決於你用哪個平臺 所以實際上這個估測的經費 是一個非常粗略的估測 而且事實上這邊估測的經費 都不包含生資料的時候的花費 生資料的時候 你需要對一個模型 你要需要對這個老師的模型 做noise distillation 你得問老師模型很多問題 得到老師模型的答案 這個也是要花錢的 再來你要清資料 這些老師模型生出來的答案 你不是全部都能用 因為老師模型也有可能講錯 他也有可能回答的不好 所以你會挑品質比較高的資料 這些都是需要成本的 所以這邊的花費大家就看看就好 那剛才講到說
              
                  15:20
                  老師模型不一定能夠得到好的答案 所以 清理資料還是需要的 那像Alpaca這篇paper裡面 他們從這個GPT3.5 得到了5萬多筆的資料 那GPT3.5的能力也是有限 所以不是所有的資料品質都很高 後來Alpaca就有另外一篇論文 叫Alpacaes 前面也有Alpa 然後為了要呼應Alpaca Alpaca就是楊婆 Alpacaes是飛馬 就是這一隻 那Alpacaes做的事情就是 把Alpaca他們訓練的資料 五萬多筆拿來 然後去問語言模型說 這筆資料是不是高品質的資料 叫語言模型打個分數 比如說這筆資料很差只有兩分 這筆資料也很差只有兩分 這筆資料不錯給五分 低分的資料就丟掉 只留下高分的資料來進行訓練 那AlpacaGas雖然用的資料 比Alpaca還少 但是他挑了高品質的資料出來訓練 所以可以比Alpaca得到更好的 的結果
              
                  16:23
                  是哪一個語言模型 來挑資料 那要怎麼挑資料才是 最有效的方法呢 這仍然是一個上代研究中的問題 有一篇paper叫做 Long is more for alignment 標題你也可以知道他想做什麼 這篇paper說的就是 用一個語言模型來挑資料 太複雜了 他怎麼挑資料呢 挑最長的就好了 就這樣 所以從那個 Alpaca的五萬多筆資料裡面 這個 Long is more for alignment這篇paper 就挑了 答案長度最長的 那一千筆資料 就結束了 他們拿最長的那一千筆資料 訓練出的模型以後 來跟原版的Alpaca 用五萬筆資料的 Alpacas用原模型挑資料的 還有Lima就有人工 非常精心製作的 一千筆資料的三個模型來對打 這邊深色藍色的代表是 選最長一千筆資料那個模型贏的狀況 然後比較淺的藍色是平手
              
                  17:27
                  那右邊是他打輸了對手的那個模型 然後這邊有兩個不同的裁判 用GPT 4當裁判 跟用PaLM 2當作裁判 那你會發現在多數狀況的時候 只選一千筆答案最長的資料所訓練出來的模型 是可以打趴其他模型的 就告訴你說選資料很重要 但是要怎麼選 還不知道有人選了最長的一千筆資料 就得到了驚人的結果 那講到這邊當然有人會質疑說 會不會是因為你選了答案最長的一千筆資料 所以說模型就學到回答很長 然後那些judge那些評審都喜歡長的答案 所以讓這篇paper裡面的模型佔到便宜了呢 在這篇paper就做了一個分析 那這邊我們可以比較這篇paper用的資料 跟Lima用的資料 那藍色代表說 訓練資料的長度 他們發現說 其實他們選了1000筆最長的資料 還沒有Lima的那個Corpus長 Lima那個Corpus裡面用的資料
              
                  18:30
                  是更長的 答案是更長的 但是Lima那些資料訓練出來的結果 還沒有選1000筆 答案最長的 但是都是由語言模型所生成的資料 效果好 所以很神奇 不知道為什麼選最長的資料 結果就會最好 那這個還有很多研究的空間 那事實上在我們的作業5裡面 就是會叫大家做Alignment 我們用的資料比1000筆更少 我們只用100筆 那也可以想辦法選擇最好的100筆資料 看看你能做到什麼樣的程度 就算我們作業裡面只用了100筆的資料 你也可以很明顯的感受到說 Base Model在經過Alignment以後 表面上的行為有了很大的改變 好,那這邊還有一個問題,還有一個盲點,我們要做knowledge distillation,那你需要有一些問題去問老師模型,這個問題要從哪裡來呢? 怎麼挑出關鍵的好問題來問老師呢? 那我們實驗室的謝俊成同學發現說,那些問題根本就不重要
              
                  19:38
                  我們做了一個這樣子的實驗 從那種網路上爬下來的資料集裡面 隨便選一個句子 把這個句子的後半段截掉 只保留前半段 直接把前半段丟給 確GVT跟你自己的模型 那也是問說前半段會是一個問題嗎 不會是,它甚至是一個不完整的句子 把前半段呢 丟給確GVT 讓確GVT自己做文字接龍 把後段半段接出來 它接出什麼就是什麼 把這個確GVT 接出來的內容當作模型 要學習的對象 模型就學習 切GVT接出來的內容 你可能會想說這輸入甚至不是一個問題呢 那這樣子的 alignment的方式有用嗎 先給你看一下 在這個work裡面 我們用的訓練資料長什麼樣子 所以這個 是輸入給ChairGVT的 上半句話 這個是原始的下半句話 那我們在訓練的時候並不會用到
              
                  20:40
                  原始的下半句話 而是叫ChatGPT去做續寫 他就把這個句子 繼續寫完 這個就是我們模型的答案 或是這是另外一個例子 這個句子是有一個人叫David 然後呢他最近呢 成為一個成間主播 she is scheduled to 他計畫要做什麼呢 那原始的下半句是 start from January 2 那ChatGPT的續解就是 begin her new role despite the reason arrests 那總之就是把句子 Chair GPT做的事情就是把句子寫完 我們就直接拿這個資料來訓練模型 這種亂七八糟的問題 跟看起來根本不像是答案的答案 到底有沒有幫助呢 非常有幫助 我們來看一下實驗結果 那最左邊呢 是Base Model 如果我們拿Mistral 7B 當作Base Model Mistral 7B在還沒有做 fine-tune的時候 它在MTBatch上的分數是3.
              
                  21:43
                  73 那如果我們只是把網路上找到的句子 直接切成兩半 前半段當作輸入 後半段當作正確答案 去訓練Mistral 7B 結果更差比沒有做Alignment還要更差 就代表說網路上的那些資料 沒有什麼神奇的東西 但是有趣的是 如果後半句話是GPT4續寫的 把GPT4續寫的內容當作答案 再去fine-tune模型 模型能力突然暴增 從3分進步到7.3分 這個分數比Misture官方釋出的Instruction的模型 得到6.84分 還要更高 所以我們拿莫名其妙的資料 直接去fine-tuneMisture的BaseModel 比Misture自己用BaseModel fine-tune出來的InstructionModel 得到的分數還要更高 同樣的實驗 做在Lama上也有用 這個做在Lama38B上 Lama3.
              
                  22:40
                  8BaseModel Anti-Bench的分數是5.5分 Instruction的Model是7.86分 那如果你用我們的資料 那些莫名其妙的資料 來fine-tuneLama3Base的話 你可以得到7分 然後如果你從InstructionModel再繼續fine-tune 你可以得到7.97分或8.2分 甚至比原來的InstructionModel還要更好 那在更大的模型上 Lama370B上也有用 Lama370B什麼都不做 才2.7分 Instruction Model是8.6分 如果用莫名其妙的那些資料 來fine-tune的話 70B的模型可以進不到8分 如果從Instruction Model開始 fine-tune的話 可以進步到9分 所以那些資料 甚至不是一問一答的資料 但神奇的事情是 他對於Alignment非常有幫助 好 為什麼這些 甚至不能夠算是答案 甚至不能夠算是問題跟答案的資料 對Alignment有幫助呢 這邊引用另外一篇 其實稍微早一點的論文
