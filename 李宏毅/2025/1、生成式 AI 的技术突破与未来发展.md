...
但今天這一堂課呢,會帶大家更深入看看這些transformer,當它當作語言模型來使用的時候,它背後怎麼運作的發生了什麼事情,好,所以這一堂課呢,會讓你對大型語言模型內部的運作機制有更深入的瞭解,那你也會學到剖析這一些AI,剖析這些類神經網路內部運作的方法,那所以 雖然等一下的課程主要是講大型語言模型,但是同樣的技術,你其實可以用到不同的領域上,那今天這堂課呢,如果用比擬的講法的話,那講的其實就是AI的腦神經科學,那今天這堂課就是要研究這個大型語言模型背後的transformer是怎麼運作的,那在我們講它怎麼運作的時候,我們不會提到任何有關的, 訓練的東西,我們就假設他已經訓練好了,那我們就是要知道說這些類神經網路,這些transformer還有什麼樣的運作機制,至於這些神奇的運作機制是怎麼被學出來的,那不是這一堂課要關心的重點,往後的課程,我們還會再提到訓練的部分,好,那所以這邊要強調的是,在等一下的課堂中沒有任何語言模型被訓練,我們就是分析一個已經訓練好的,
              
                  02:00
                  語言模型,那在課堂正式開始之前呢,有一個要提醒大家的事情是,等一下你所看到的分析內容不一定適用於最新的最強的大型語言模型,為什麼會這麼說呢,在這個圖上啊,橫軸是時間,縱軸是不同語言模型的NLU,NLU是一個測試的資料集,它就是評量語言模型的能力,那在這個圖上呢, 圖上每一個點是一個語言模型 那黑色的這一條線是那些閉源模型 比如說ChatGPT,Gemini等等 它在NLU上面的表現 大家可以看到隨著時間 這些模型越來越強 那橙色這一條線是開源模型的表現 那可以看到一開始開源模型很弱 但是隨著時間的進展 開源模型也逐漸追上來了 而這個圖上藍色的點 是這一些 分析語言模型背後運作機制的論文 他們所分析的類神經網路
              
                  03:07
                  他們所分析的語言模型 那你發現他們往往分析的是比較小的模型 而這些比較小的模型 他的表現自然沒有辦法跟這些大模型相提並論 那直到近年才有人開始分析比較強的大模型 那為什麼會這樣 這是很直覺的 比如說LLaMA 405B 你根本沒有那麼大的算力 把這麼大的模型跑出來 更遑論是對他做更進一步的分析了 所以發現多數文獻 往往就是選擇一個比較小的模型來分析 甚至在2023年那個時候都還有很多人是分析 2019年釋出的GPTQ系列 所以等一下看到的結果 比較像是在做腦神經科學的時候 你拿老鼠來做實驗 發現老鼠的腦會有這樣子的機制 那你想人類的腦應該也會有這樣的機制 所以等一下很多實驗結果是類似這樣的 的結論 所以在開始之前先提醒大家 那這一堂課呢 是假設你已經非常熟悉transformer的架構 那我們在第一講的時候就有跟大家講說
              
                  04:12
                  你要先回去複習過去的課程 假設你已經複習過去的課程 你已經看過機器學習2021的影片 知道什麼是self-attention 知道什麼是transformer 那我們就不會再對這一些 你已經知道的類神經網路的架構 進行 複習 假設你覺得看2021年機器學習的影片總共有四步 太花時間的話 那你可以選擇看生成式AI導論2024年的第十講 那我們花不到40分鐘的時間 跟你講一下什麼是transformer 那今天這一堂課呢 也跟機器學習的可解釋性非常有關 那在機器學習2021的課程裡面 也已經講過機器學習的可解釋性 今天這一堂課要講的內容 跟過去所講的內容 並不會重複 那你沒有看過去這兩堂課的內容 今天的內容你應該也聽得懂 那你可以上完這門課以後 再回去看機器學習2021 講的這個機器學習
              
                  05:15
                  可解釋性的兩堂課的內容 你會有更多的收穫 那在生成式AI導論 2024的第11講 大型語言模型在講什麼 也剖析了大型語言模型背後 運作的機制 講的是更新的研究 今天要講的內容 跟2024年去年講的內容 沒有任何重疊的地方 所以你可以聽完這門課 再去看去年的內容 會有更多的收穫 好那我們就正式開始今天的課程 那今天的課程呢 分成四個部分 第一段我們要講 一個神經元他在做什麼 然後第二段我們要講 從一個神經元擴展到一層神經元的時候 整層的神經元在做什麼 那第三段我們要講不同層的神經元 他們之間是怎麼互動的 那最後我們講一個跟語言模型的能力 跟語言模型本身的特色 非常有關係的分析的方法 讓語言模型自己說出他心裡的想法 讓語言模型透過他能夠說話的這個能力
              
                  06:22
                  說出他自己內心的想法 那我們先從一個 一個神經元在做什麼開始講起 要再講一個神經元在做什麼之前呢 也許我應該先跟你講一下 當我們說一個神經元的時候 在Transformer裡面指的是什麼 好那這個是Transformer 那我們已經知道說呢 今天生成式AI要做的事情 就是給一個sequence Z1到ZT-1 然後你要預測下一個token ZT 那這個token 它可以是任何東西 它可以是像素 它可以是語音 同樣的技術你可以用在不同的領域 不同的模態上面 那所以雖然等一下的分析 主要是集中在文字的大型語言模型上 你完全可以把同樣的技術 用到其他的領域 其他的模態 那Transformer做的事情 就是它的輸入是Z1一直到ZT-1 那它的輸出就是下一個可能的token
              
                  07:26
                  也就是ZT 那實際上它真正的輸出 是一個機率分佈 這個機率分佈表示 下一個tokenZT 可能是任一個token的機率有多大 比如說下一個ZT可能是任一個字的機率有多大 或下一個ZT 如果你是做圖像生成的話 就下一個ZT是某一個顏色的像素的機率有多大等等 而那Transformer內部的構造呢 它是一層一層的 當輸入Z1到ZT的時候 這個Z1到ZT-1這個token的sequence 會先變成一個vector的sequence 每一個token都會對應到一個向量 那token是一個discrete的東西 那每一個discrete的東西都會對應到一個向量 那把discrete的東西對應到一個向量的這個過程 叫做embedding 那embedding其實就是查一個表 所以可以想像說在transformer裡面 他就是有一個表有一個table 記錄了每一個token 應該對應到哪一個向量
              
                  08:31
                  但這個表也是透過學習 透過資料而得到的 所以輸入一個token sequence 然後呢 通過embedding這一層 會變成一個vector sequence 會變成一個向量的序列 這個vector sequence呢 通過每一個layer以後 會得到另外一個vector sequence 那另外一個vector sequence會再過下一層 再得到另外一個vector sequence 所以就一層一層一層的下去 每一層都會產生一個 新的vector sequence 直到最後一層 那最後一層的vector sequence 它的最後一個向量 會被拿出來 那產生這個distribution 那從最後一個向量 到distribution的這個過程 它有一個名字叫做unembedding 那其實這個動作 有各式各樣不同的稱呼 那這堂課裡面我們就把它叫做 unembedding 它是把一個向量轉回token的過程 所以我們叫它unembedding 那實際上做的事情
              
                  09:33
                  實際上這個unembedding做的事情 就是把輸入的這個向量 乘上一個linear的transform 把它變成一個distribution 那比如說這邊如果是4096維的向量 那token的可能性有三萬 那這個unembedding這個linear transform 就是把4096維的向量 變成一個三萬 而這個三萬維的向量 代表了token的distribution 我們也知道說 transformer的每一個layer裡面 其實還有layer 當我們說transformer的一個layer的時候 它不是一個真正的layer 它是layer裡面還有layer 那layer裡面的layer有分 self-attention layer 也有分不處理整個sequence 只處理單一token的layer 那self-attention layer在做什麼 我們在等一下 作業三裡面呢 要求大家做更進一步的分析 那在這一堂課裡面 我們先專注來看 只處理一個 token的layer 它在做什麼
              
                  10:36
                  好那這個只處理一個token的layer呢 如果我們再更仔細看一下的話 你看這個layer 它輸入是一個紅色的向量 輸出是一個藍色的向量 那我們把它放大放到右邊 這是紅色的向量 這是藍色的向量 好那這個紅色的向量是做了什麼樣的變化產生這個藍色的向量的呢 它中間的變化是這樣子的 每一個藍色向量裡面的數值 它裡面的每一個dimension裡面的數值 都是紅色向量所有的數值 做weighted sum 這邊這些箭頭代表的是weighted sum 把紅色向量裡面每一個dimension的數值 乘上一個weight再加起來 再通過一個activation function 今天通常是用一個叫做review的activation function 這是過去課程有講過的內容 它會把小於0的值變成0 大於0的值就直接不動 得到輸出 所以每一個輸出 這個藍色的這個向量裡面的每一個數值 都是前面那一排 向量的weighted sum
              
                  11:39
                  再過一個activation function 那這個藍色向量裡面的 每一個數值就叫一個 neuron的output 就叫一個neuron的輸出 從紅色這個向量到藍色向量裡面一個數值 這中間的轉換就叫一個neuron 就叫一個神經元 那我們怎麼知道一個神經元 是在做什麼的呢 那第一個步驟就是 觀察這個神經元會導致什麼樣的現象 比如說如果你觀察到說 這個神經元被啟動的時候 所謂啟動的意思就是 它的輸出大於零 過了這個ReLU以後 是大於零 那大於零就代表 被啟動了 它對接下來的 類神經網路的運作會有影響 當這個神經元被啟動的時候呢 當它可能輸出很大的值的時候 這個語言模型就會說髒話 那你就會知道說 這個神經元 可能是跟說髒話有關的神經元 但是如果你只觀察到說 這個神經元被啟動的時候
              
                  12:42
                  語言模型會說髒話 你並不能夠斷定 這個神經元 就是導致說髒話的髒話的 神經元 你只能說這個神經元跟說髒話是有關係的 那不知道大家知不知道這個原理 就是相關跟因果是不一樣的 大家都聽過這種故事吧 就是這個冰淇淋的銷售量 跟溺水的人數成正相關 但是是冰淇淋銷售的量導致溺水的人數增加嗎 不是 因為夏天到了 大家想吃冰淇淋 有很多人去海邊玩 所以很多人溺水 所以相關性跟因果性是不一樣的 所以如果你只觀察到這個神經元啟動 然後語言模型就會說髒話 你只知道這兩者有關 你不能說這個神經元 他就是導致說語言模型說髒話的一個神經元 也許這個神經元甚至有可能是抑制說髒話的神經元 就語言模型說髒話的時候 他內心要覺得很抱歉 所以等一下要準備說出道歉的話 所以這個神經元就啟動了 讓他說出道歉的話 也是有可能的 所以你要怎麼樣更進一步確定說
              
                  13:46
                  這個神經元真的跟說髒話 有因果關係呢 那你要做下一步的檢驗 如果我們把這個神經元移掉 那是不是語言模型就說不出髒話來了 如果這個神經元你把它割掉 從這個語言模型裡面移除 語言模型不管怎麼汙辱 它都在說不出髒話來了 那這個神經元可能跟說髒話 就有一定程度的因果關係 它可能就是說髒話的因素之一 但這邊其實有一個問題是 什麼叫做把神經元從類神經網路移掉 有人可能覺得說 這是一個很直覺的答案 所謂的移掉就是 它不在這個類神經網路 也就是我們可以說 把這個神經元的輸出 永遠設成零 希望它就不要對類神經網路的 其他部分造成影響 但事實上設成零 並不代表完全不會造成影響 因為有時候零這個數字 對其他的Neuron而言
              
                  14:49
                  可能也是有意思的 也許當這個神經元 它輸出是零的時候 反而會啟動其他神經元的 其他的作用 所以後來有研究是發現說 也許設平均值 就你給這個圓模型各式各樣不同的輸入 看看這個神經元 它輸出的平均值是多少 把它的輸出設成平均值 變成一個固定的數值 是平均值不再變化 但總之這仍然是一個上代研究的問題 那一般的這個論文可能就是做這兩步啦 用這兩步來檢驗某一個神經元它的功能 那如果你願意做更多的話還可以有第三步 就是如果這個神經元有不同的啟動的程度 那他會不會說不同等級的髒話 比如說如果他輸出的數值比較小的時候 是不是就講比較不髒的髒話 如果他輸出的數值變很大的時候 他就講很髒的髒話 但是髒話的等級可能很難確定 你可能很難知道說 什麼是很髒的髒話 所以通常論文裡面不一定會做到第三步就是了
              
                  15:55
                  所以這是一個檢驗神經元 有什麼樣功能的方法 那講到有功能的神經元 最知名的有功能的神經元 就是川普神經元了 川普神經元不知道大家有沒有聽過 這個去年川普當選總統之後呢 後來這個媒體就大肆報導 類神經網路裡面有一個川普神經元 好像是一個最近的發現一樣 但其實不是川普神經元 是一個老新聞了 這個是2021年的時候 這個OpenAI的研究人員 在一個平臺叫做Distell 那是專門給你放論文的平臺 在Distell那個平臺上 發表的一個論文裡面的內容 而且他們分析的 還不是大型語言模型 那是2021年 所以他們分析的其實是一個叫做 Clip的影像模型 因為他不是一個生成的模型 所以他其實也沒辦法生成東西 所以川普神經元他的作用是 輸入川普相關的東西的時候 那個神經元會被啟動
...
輸出三分之二代表解題的正確率 看到這樣的 Step 1 Step 2 你就輸出三分之一代表解題的正確率 這個 Process Verifier 就可以學會說 看到越好的、越有機會得到正確答案的解題步驟 就給越高的分數 那接下來會遇到的另外一個問題是 通常這個 Process Verifier 他真正給你的並不是 True or False 通常 Process Verifier 真正給你的 是一個數值 他代表從這一步繼續做下去以後 可能得到正確答案的機率 那接下來就會有一個問題 是這邊有好多條路徑 那我到底應該把這個 Threshold 切在哪裡呢? 到底正確的可能性要高過多少的時候 我們才繼續做下去呢? 那如果你覺得這個問題不好回答的話 也許你可以參考一個叫 Bean Search 的做法 那這個做法他的概念非常簡單 就是我們每一次保留 Process Verifier 告訴我們最好的 N 條路徑 那這邊有很多不同的變形啦 有的方法是保留最好的 N 條路徑 有的方法是保留最好的前 25% 的路徑 那有各種不同的變形 不過概念是一樣的 從所有的可能性中保留最好的一部分 那其他比較差的就不再繼續考慮 那假設現在這個 N 呢設定成 2 那我們就從這三個 Step 1 之中 選擇 Process Verifier 覺得最好的兩個 Step 1 然後不再理會把最差的 Step 1 把它去掉 把其他的 Step 1 去掉 接下來再進入 Step 2 那模型就根據上面這個保留下來的 Step 1 繼續去產生不同的 Step 2 根據下面這個保留下來的 Step 1 繼續去產生不同的 Step 2 那怎麼產生怎麼只產生 Step 2 呢 就跟產生 Step 1 的道理一樣 一直產生到斜線 Step 的時候就停下來 接下來根據 Binsearch 的方法 只保留兩條最好的路徑 那就把這邊每一條路徑 都用 Process Verifier 去計算說這個路徑有多好 那 Process Verifier 可以吃 Step 1 跟 Step 2 決定說現在從這樣子的解題過程解下去 得到正確答案的機會有多大 那你再選擇機會最大的兩條路徑保留下來 把其他比較差的通通就 Print 掉 把其他比較差的就丟掉 那接下來這個實驗結果 也是來自於 Hugging Face 的那篇 Blog 所以除了 Majority Vote 以外 他們接下來再嘗試了 Best of N 的方法 發現 Best of N 的方法 可以做得比 Majority Vote 還要更好 那接下來他們又嘗試了 Binsearch 的方法 就是綠色這一條線 他發現說當他們用 Binsearch 這樣的方法的時候 他甚至可以用 1B 的模型 就超越了 8B 的模型 雖然 1B 的模型遠比 8B 的模型小 在正常的狀況下他根本打不贏 8B 的模型 但透過這個 Reasoning Process 的 Workflow 透過告訴模型應該要怎麼多方嘗試 什麼時候要嘗試下去 什麼時候應該停下來 透過這種 Binsearch 的方法 其實 1B 的模型也是有機會 比 8B 的模型做得更好的 那事實上在這個 Blog 裡面 還有另外一個方法叫 DVTS 他其實是 Binsearch 的變形 因為 Binsearch 往往你有時候會遇到一個狀況是 你每次保留下來的那些路徑 長得都太像了 有時候我們可能會希望保留比較不像的路徑 那怎麼辦呢 你可以再參考這個 DVTS 的做法 看看 Hackingface 他們是怎麼做的 不過這個方法看起來並沒有非常顯著的效用 所以這邊我們就不再細談這個方法 那事實上啊 剛才講的這個 Binsearch 的方法 他有很多不同的變形 你都聽過很多各式各樣的這種 Heuristic Search 的 Algorithm 比如說 A-Star 的 Algorithm 或者是 Monte Carlo Tree Search NCTS 這些方法通通都可以套到 我們剛才講的框架下 你完全可以把 Binsearch 就換成 你比較喜歡的 Monte Carlo Tree Search 然後看看結果會不會做得更好 那其實在去年暑假有很多很多論文 都在 Large Language Model 上面做過類似的嘗試 已經有太多文章 把 Monte Carlo Tree Search 用在大型語言模型推理的過程中 那也顯示說 Monte Carlo Tree Search 可能是可以帶來幫助的 那這邊論文非常多 我就列了幾篇有代表性的放在這邊 給大家參考 那至於 Monte Carlo Tree Search 的細節 就留給大家自己研究 好那接下來啊 我們要講的兩個方法呢 都是需要微調參數的 那接下來講的兩個方法 都可以視作是上週所講的 後訓練的方法的特例 也就是我們有一個 Foundation 的 Model 這個 Foundation 的 Model 還不會做深度思考 但我們接下來做 Post Training 希望讓這個 Foundation 的 Model 具有深度思考的能力 我們把 Foundation 的 Model 做一下微調 教他怎麼做 Reasoning 教他怎麼做深度思考 那這邊有兩個不同的做法 第一個做法呢 我這邊叫做 Imitation Learning 我這裡的意思就是 直接教模型怎麼做推理 模型會有一個老師 告訴他怎麼推理才是對的 他模仿老師的行為 所以叫 Imitation Learning 所以一般我們的訓練資料 只有問題跟答案 但這邊假設我們的訓練資料 不只問題跟答案 還有推論的過程 那我們要怎麼讓模型 學習推論的過程呢 我們只要教模型說 看到這個藍色的輸入 之後你不只要產生正確答案 你還要產生綠色的這邊 綠色的這個 Reasoning Process 你還要產生一個推論的過程 然後才產生正確答案 他的概念就是這麼簡單 講完了 但是這一整個概念裡面 最難的地方是什麼呢 最難的地方是 推論過程從哪裡來呢 通常你都只會有正確答案 而沒有推論的過程 那推論過程從哪裡來呢 當然你可以叫人標啦 但叫人寫這些題目 其實是非常花時間的人 通常也寫不了 沒有人願意幫你做這件事 那怎麼辦 讓語言模型自己想辦法 產生推論的過程 所以就拿一個語言模型來 然後給他 input 然後要求他要做 COT 要求他解題的時候 要把推論的過程詳細解出來 然後他就產生推論過程 然後產生答案 那語言模型畢竟能力有限 他不會每次都答對 但是反正你有正確答案 所以你可以去檢查語言模型 什麼時候是答對的 那只有他答對的時候 這個 reasoning process 才會被拿來當作訓練資料 但這邊前提的假設是 假設答案是對的 那推論的過程應該也是對的 那麼可以拿這個 可能是對的推論過程 來教語言模型怎麼進行推論 那或者是有時候你會遇到的狀況是 有一些問題 他可能沒有固定的標準答案 那也沒有關係 你可以找一個 verifier 他可能甚至是一個現成的語言模型 那你可以問這個現成的語言模型說 你覺得這個答案是不是對的 所以你可以拿一個 verifier 他可能是一個現成的語言模型 來看看這個答案是不是對的 那如果是對的 我們就假設 reasoning process 就是對的 就可以讓模型學習這個推論的過程 但講到這邊 你可能會想說 這個 reasoning process 一定是對的嗎 有沒有可能答案是對的 但推論過程其實是錯的 他只是最後賽到一個正確的答案而已 所以確實我們沒有辦法保證答案是對的 推論過程每一步就是對的 所以就有人提出一些方法說 也許我們不應該只看最終答案對不對 我們應該用剛才在前一個方法裡面 跟大家提過的 這種樹狀搜尋的方法 去對每一步進行驗證 我們叫模型解題的時候 我們先展開這個樹狀的結構 然後我們拿一個 process 的 verifier 去驗證每一個步驟是不是對的 那只有比較可能是正確的步驟 才會繼續展開下去 最後如果模型有得到正確答案的話 那中間的步驟可能每一步都是對的 至少中間每一步都有一個 process verifier 驗證說這個步驟可能是一個好的步驟 然後接下來 你就可以把這邊的每一個步驟結合起來 當作 reasoning process 給你的語言模型進行學習 直接教他怎麼做 reasoning 那其實剛才講的那些方法 都可以被歸類為 supervised fine tuning 之所以不講說它就是 supervised fine tuning 而是用另外一個詞彙 imitation learning 來概括它 是因為其實第三類的方法 不一定要是 supervised learning 它也可以是 reinforcement learning 因為你可以教模型說 今天在這個樹狀結構裡面 我已經知道產生這個藍色的 step 會得到錯誤的答案 產生這個綠色的 step 會得到正確的答案 所以你不一定要 supervised learning 的方法教他 你也可以用那種類似 DPO 的做法 用一些類似 reinforcement learning 的做法 告訴模型說 這個綠色的 step 1 應該增加他的機率 這個藍色的 step 1 應該要減少他的機率 或者是你今天在這整個樹狀結構裡面 觀察到說 這一條路徑會導致錯誤的答案 這一條路徑會導致正確的答案 那你就可以告訴語言模型說 看到綠色的 step 1 step 2 step 3 那你要提高他這樣子的路徑的機率 看到綠色的 step 1 step 2 紅色的 step 3 是不好的 那應該要降低他的機率 所以你可以用類似 reinforcement learning 的方法 來利用這個 tree structure 裡面 得到的訓練資料 好講到這邊 剛才前一刻 我們還在糾結 是不是 reasoning 的過程中 每一步都是對的 我們希望教模型的時候 每一步的 reasoning 的過程 都應該是正確的 但是我們真的應該這樣教模型嗎 我們應該要告訴模型 每一步推論過程 都必須要是正確的嗎 今天很多那一種 可以深度思考的模型 其實有時候會得到錯誤的思考過程 舉例來說 我這邊問歐三說 這個 strawberry 這個詞彙裡面 這一串符號裡面 有幾個點 就前一陣子不是很多人很喜歡問語言模型 strawberry 裡面有幾個 r 然後語言模型會答錯嗎 所以這個是類似的問題 這邊之所以不問他 strawberry 裡面有幾個 r 是因為這類問題 現在對語言模型來說 都已經太簡單了 他已經都會答對了 你甚至不知道 他是不是已經把答案背起來了 所以這邊應該考一點不一樣的問題 我問他這一串文字裡面 有幾個點 正確答案是九個點 那這個歐三就算了一下 算了一下 算了一下 發現說 怎麼算出來只有八個點 是錯誤的答案 但是歐三厲害的地方就是 他能夠找找自己的問題 他能夠找找自己的問題 知道嗎 他能夠找自己的問題 所以他發現說 八個點是錯誤的答案 正確答案 應該是九個點 所以模型中間就算推論過程有錯 也沒有關係 知錯能改善莫大焉 只要他最後能夠得到正確答案就好了 所以我們甚至不應該教模型的時候 給他的每一步的推論過程都是對的 因為每一步的推論過程 如果都是對的會有什麼問題呢 模型在訓練的時候 他只看過正確的推論過程 他從來不知道推論的過程 可能會有錯 如果你給他所有的推論過程都是對的 他會不知道要找找自己的問題 他每次都會覺得 前面的推論過程一定都是對的 所以他會就算前面錯了 他也不知道 他就會一直硬拗下去 他不知道前面的推論過程 有可能是會犯錯的 所以怎麼辦呢 我們故意製造出一些特殊的訓練過程 其讓訓練過程中間可能有一些是有錯的 因為語言模型他本身能力還是有極限的 所以你讓語言模型自己做reasoning的時候 他中間可能會犯錯 但是如果你不教他犯錯以後要怎麼改正的話 他可能就會錯上加錯 一步錯步步錯 看到錯的結果還是要硬拗出一個答案出來 我們應該要讓語言模型有知錯能改的能力 那怎麼教語言模型知錯能改呢 我們一樣可以從這個樹狀的結構上面去想辦法 有一篇paper叫做stream of search 這篇paper的想法就是 我們能不能從樹狀結構裡面得到一個reasoning的過程 得到一個推論的過程 他裡面是包含錯誤答案的 我們直接在這個樹狀的結構上面做一個深度優先的搜尋 把錯誤的搜尋過程也包含進去訓練資料裡面 我們不要一次就走綠色這一步 我們故意走一些錯的路 走一些有可能得到錯誤答案的路 我們先從最上面這條路走 走到這個粉紅色的step 1 然後再走粉紅色的step 2 那走到粉紅色的 我下面這個圖代表的是在創造那個訓練資料 我就告訴模型說 你要先走step 1 然後再走step 2 然後step 2是錯的 所以接下來語言模型按照這個樹狀結構 就要退一步 然後嘗試藍色的step 2 但是如果今天你在教語言模型的時候 他學到的是 看到這個皮膚色的step 2之後 接下來要產生藍色的step 2 那對人類來說 如果你看到語言模型先產生一個答案 然後再莫名其妙再跳到另外一個答案 你可能會覺得這個語言模型講話很容易前言不對後語 不過如果你看最早的O1 他講話蠻容易前言不對後語的 然後不知道是什麼原因 有可能是 當然這個O1並沒有展現那個完整的reasoning的過程給你看啦 他是有經過摘要的啦 但也不知道是摘要那邊出了問題 還是那個語言模型 他的思考就是非常跳躍 搞不好他就是用這樣子的資料訓練出來的 所以他思考非常的跳躍 講話常常前言不對後語 好 不過如果你怕模型講話前言不對後語 這個突然從這一步跳到這一步很奇怪的話 那你可以多加一些句子進去 比如說在這邊我們之所以知道這個皮膚色的step 2是錯的 是verifier驗證的結果 那verifier是一個語言模型 他甚至是能說話的 那你可以把verifier的回饋插在這兩個步驟中間 那verifier的回饋可能是 這個皮膚色的step 2他的答案有錯 我們應該換一個解法 然後才接藍色的step 2 那這樣這整個reasoning的過程 人類讀起來可能就會覺得比較順暢 好那接下來呢 step 2 接下來呢 藍色的step 2也是錯的 所以我們要回到最初的原點 再重新嘗試 嘗試一下灰色的step 1 那你在藍色的step 2跟灰色step 1中間 你可以加入verifier的回饋 那因為現在一次跳到最初始的地方 所以你可能可以加一個句子說 讓我們一切重新來過 那這樣人類讀起來會覺得這個推論的過程 比較順一點 後來有些其他的論文甚至會用比較好的語言模型 改寫這整個reasoning的過程 所以在這個樹狀結構中 搜尋出一個路徑以後 再用比較好的語言模型去改寫整個搜尋的過程 讓他這個step和step之間加一些連接詞 讓他讀起來呢比較順暢 好那step 1也是錯的 所以加入verifier的回饋 再給綠色的step 1的過程 然後給綠色的step 2的過程 給綠色的step 3的過程 然後最後得到正確的答案 然後你拿這些訓練資料去訓練你的語言模型 你的語言模型就會學到說 一開始他可能會走一些歪路 但走一些歪路也沒關係 他有能力可以修正回來 好那有另外一篇paper呢 也提出了一個非常類似的想法 那這篇paper把它們提出來的想法叫做journey learning 那就說如果你在樹狀結構裡面 只找出每一步都是正確的path 來當作訓練資料 這個叫做shortcut learning 模型只學會打順風舉 他不會逆轉勝 所以你要有一個方法叫他們提出來的方法叫做journey learning 你要根據你的樹狀結構 去製造出一個reasoning的過程 其中是有包含一些錯誤的路徑 讓模型可以學會逆轉勝 看到錯誤的路徑 要如何轉回正確的路徑上 那journey learning這個方法 還真的是有用 我這邊就引用這篇論文裡面的實驗結果 他們做在Math500上面 這個紅色的這個bar呢 是shortcut learning的正確率 藍色這個bar呢 是journey learning的正確率 那你會發現說 他們從兩個不同的foundation model 開始做post training 那journey learning journey learning 都比shortcut learning表現還要更好 好所以到目前為止 我們講了一大堆 創造reasoning過程當作訓練資料的方法 但如果你今天想要自己打造有reasoning能力的模型 也許已經不用那麼麻煩了 因為世界上已經存在好些有reasoning能力的模型 你可以直接對他們做knowledge distillation 就拿一個能夠做深度思考的模型 讓他先產生reasoning的過程再產生答案 你的模型就直接學習這個模型 學習這個reasoning的模型 做reasoning的過程就結束了 這招就是knowledge distillation 那像sky T1還有S1 都是用同樣的方法 來教模型reasoning的能力 那在DC的paper裡面呢 在DC R1的paper裡面呢 其實他們也展示了knowledge distillation 是非常有用的 他們做的事情是什麼呢 這邊每一個column代表的是 在不同任務上面模型評比的結果 那這個數值越高越好 那這邊的任務呢 有數學相關的任務 然後還有GBQA 那就是一些非常難的問題 都是博士級 號稱要博士級的人才可以答的對的問題等等 然後還有一些寫程式的任務 上面是一些很厲害的模型 他們可以得到的正確率 他們可以得到的分數 那下半部呢 是knowledge distillation的結果 那在這一串文字的右邊 這個這些文字啊 Q1跟Lama指的是 拿來做post training的foundation model 他們在Q1小模型到大模型 還有Lama 8B跟70B上面 去做了post training 他們拿Dsig R1當作老師 去教這一些foundation model 怎麼做reasoning 他們發現教完之後 這些模型他的能力 在數學在程式上也都起飛了 可以跟這些比較好的模型相提並論 好剛才講了三個方法 那接下來呢 我們要講最後一個方法 最後一個方法 是以結果為導向來學習推理 也就是用reinforcement learning 以結果為導向 來教模型怎麼做推理 這個就是過年期間非常紅的 Dsig R1系列的做法 好那怎麼用reinforcement learning 來教模型怎麼進行推理呢 他的概念是這樣子的 你有一些訓練資料 你有問題 你有問題的正確答案 你把問題輸入給模型 要求模型做reasoning 他reasoning的什麼不重要 我們只看他最後的答案 他reasoning完之後要給一個答案 因為你有正確答案 所以你可以去對答案看對不對 如果是對的 模型就得到positive reward 如果是錯的 模型就得到negative reward 那在這整個過程中 推論在整個IL 在這邊的IL的學習的過程中 推論的內容 這個reasoning的process 到底說了什麼 不重要 我們只在意最後的答案 是不是對的 在這個reasoning的過程中 答對就是positive 答錯就是negative 只要是答對 中間reasoning的過程 發生了什麼事 不重要 所以你可能會覺得說 這個方法乍看之下 跟我們剛才第三類的方法 好像也很像 剛才第三類的方法 也有產生reasoning的process 在看答案對不對的 這個這類的方法 但是在第三類的方法裡面 我們都是在意reasoning的過程的 我們會希望reasoning的過程是對的 我們會希望reasoning的過程 長某些特定的樣子 但在第四類的方法裡面 完全不管推論的過程 只要最後答案是對的 就是好的 那這個就是D-Seq-R系列的做法 D-Seq的系列裡面 有一個模型叫D-Seq-R1-0 這個並不是你在D-Seq的平台上 會用到的D-Seq-R1 這個是另外一個模型 這個D-Seq-R1-0 它就是一個純粹用RL學出來的模型 他們把D-Seq-V3-Base當作Foundation Model 然後用RL 然後把模型能不能答對問題的正確率 當作reward 然後打造了D-Seq-R1-0 那其實除了拿reward當作 除了拿正確率當作reward之外 R1-0還有另外一個reward是format reward 他們要求模型要產生think token 就這樣 所以這就是為什麼做完RL以後 模型可以產生think token 因為那是reward的一部分 模型要產生think token才是好的 那這種純粹只問結果 不問過程的方法 有沒有用呢 是有用的 在這個R1的paper裡 在D-Seq-R1的paper裡面 他們這邊先展示了R1-0的結果 縱軸是正確率 那這邊測試的數學問題是用AIMER 那是一個數學競賽的問題 裡面都是非常難的問題 橫軸是RL訓練的過程 藍色這條線是D-Seq-R1-0的正確率 那藍色這條線衡量的是 假設模型只能夠做題目一次 那它的正確率有多少 那綠色的虛線是O1 如果只能做題目一次的話 它的正確率有多少 然後最後D-Seq-R1-0 在經過RL的訓練之後 可以逼近O1的結果 那紅色這條線是什麼意思呢 紅色這條線是majority vote的結果 讓D-Seq-R1-0去每一個題目產生16個答案 再用這16個答案做majority vote 把這16個答案做majority vote 可以得到紅色這條線 那紅色的虛線呢 是O1做majority vote的結果 那如果用R1-0做majority vote 可以跟O1做majority vote做的一樣好 然後比O1沒有做majority vote還要好 那今天這個實驗裡面 可以告訴我們一件非常重要的事情 很多人會問說 今天講了四個方法 那這四個方法裡面 哪一個方法最強呢 這個實驗就是告訴你說 這些方法彼此之間是沒有衝突的 就算大家一直說 D-Seq-R1-0怎麼怎麼厲害 其實它是可以用majority vote 再進行強化的 所以今天我們講過的 majority vote的方法 是可以跟RL的方法 直接做結合的 你可以在force training的時候 用RL的方法調整參數 然後在inference的時候 用majority vote再一次 強化模型的能力 所以今天我們講的這四個方法 其實本身是沒有衝突的 在這個R1-0裡面呢 在原始的論文中 特別作者 非常想要讓大家知道的一件事情 就是AHA moment 他們說 模型怎麼學到這個reasoning呢 怎麼學到把錯的東西改對呢 這是他自己學到的 沒有教他 他自己就會了 在RL的過程中 學著學著 他就知道能夠發現 自己的錯誤 模型在解題的時候 解著解著 解到某個地方 他就會突然說 wait wait wait there is an aha moment 然後就說之前可能有錯 我要重新來想剛才解題的思路 他們說這個 不是透過人為的力量 所得到的 是RL 自行得到的 但是R1-0 為什麼沒有放在平台上讓大家用呢 因為R1-0 幾乎是不能夠使用的 他的reasoning的過程 是非常難讀的 而且是多個語言混雜的 為什麼R1-0 的reasoning過程很難讀呢 為什麼他推論的過程 人類是很難看懂的呢 因為你在教這個模型的時候 你只在意結果 你只在意他的答案對不對 你根本沒有在意他推論的時候 到底寫了些什麼東西 所以R1-0 並不是最終放在平台上 給大家用的模型 所以R1-0並不是R系列的 最終版本 而這個D-SIG R1就是大家 真正有在用的模型 他是怎麼被打造出來的呢 很多農場文呢 講這個D-SIG R系列的時候就只講一句說 用RL train就結束了 其實背後的過程 真正R1真正打造的過程 是很複雜的 而且我們剛才在 前面講的三個方法 其實都有用上 所以今天 很多人講到reasoning就會講到說 講RL就好了因為RL聽起來就是最強的 R1就是RL做的 其實不是其實在打造R1的過程中 前面講的三個方法 都是有用上的 所以我們就來看看真正的R1 是怎麼產生的 有了R1-0之後 他們用R1-0 來產生 有reasoning process 的訊列資料 他們有問題有正確答案 但沒有reasoning process 現在竟然有一個 可以產生reasoning過程的模型 叫R1-0 就讓他來寫reasoning的過程吧 那你可能會想說剛才不是說 R1-0寫出來的東西 人是讀不太懂的嗎 那沒關係 就是耗費大量的人力下去改他 那這邊到底耗費了多大的人力 改了多少 不知道他們的技術報告裡面 這段是寫得非常隱晦的 他只告訴你說用了大量的人力 去修改了R1-0的輸出 當作訊列資料 有了這些訊列資料以後 你就可以做imitation learning 再一次訓練 Dsync V3 然後訓練出一個 神秘的沒有名字的模型 我們先暫時叫他模型A 在技術報告裡面 並沒有幫他取名字 那這一部得到的模型A 可能會比R1-0更好 為什麼因為R1-0的推論過程 是模型自己發明出來的 是人很難讀懂的 現在人幫他改寫了一下推論的過程 所以模型A 應該比較能夠產出 人比較能看得懂的過程 而事實上除了直接用R1-0 產資料之外 他們其實還用了另外一個模型 在論文裡面沒有講得非常清楚 他們拿另外一個模型 用Fieldshot COT的方法 來產生一些 帶有reasoning的資料 他們也用pumping的方法 他們說 用pumping的方法 這兩個句子都是技術報告裡面的原文 他們說用pumping的方法 讓模型產生一些detail的answer 而且 要有reflection 要有verification 那這個顯然就是supervised COT的方法 都是我們今天這一堂課 第一部分有講過的方法 所以他們用今天這一堂課 第一部分講過的方法 去強迫某一個模型生出 有reasoning過程的資料 把這些資料集合起來 訓練出模型A 這部分的資料沒有很多 技術報告裡面說是 thousands of examples 這部分可能有大量的人工介入 用人工介入去選擇 比較好的reasoning的過程 或人工介入去改寫 出比較好的reasoning的過程 這一部 得到model A 接下來model A
