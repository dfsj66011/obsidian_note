...
看得到了什么响应。现在的结果是这样的，如果我们使用之前创建的输出解析器，就可以将其解析成一个输出字典，打印出来是这样的。注意，这是一个字典类型，而不是字符串，因此我现在可以提取与键"gift"关联的值得到true，或者提取与"delivery days"关联的值得到2，还可以提取与"price value"关联的值。
...
**如何让 Agent 自己生成代码，并在沙盒中运行**

构建自己的智能体，它能够运行 Python 代码、读写文件，并通过迭代任务直至获得最终答案。我们将探讨如何管理上下文窗口、处理错误及重试机制。一旦理解了智能体的思考与行动方式，你将探索不同的运行环境，如本地执行、容器化部署以及安全的沙盒化微虚拟机（microVM），并学会根据实际用例选择最适合的方案。

----------

到目前为止，我们的代理只能执行单次操作。但一个真正的代理应该能够迭代执行任务，并在完成后停止。我们需要创建一个循环和退出条件。

决定何时停止的最简单方法是当代理不再调用新函数或达到最大步骤限制时。让我们来看看具体实现。我们在 coding_agent 函数中添加一个新循环，直到达到参数 max_steps（默认为五），如果代理不想进行新的函数调用，这意味着它认为不需要使用可用的工具。

那么我们将把这个任务标记为已完成。我们还会保留一个消息列表，并将初始用户消息、每条消息部分以及工具返回的结果都添加到其中。让我们在这里测试一下。我们希望模型能创建一个凯撒密码函数，然后让它向我询问一条消息和位移量。接着我们想运行它，打印出加密后的消息，并将其保存到一个secret.txt文件中。

代理需要完成相当多的步骤。很好，我要写下我的名字——弗朗西斯科。然后我要将字母表中的字母移动三个位置。消息就是这组字符，模型告诉我它已经将这些字符写入了secret.txt文件。我没有看到任何write_file工具，所以它可能是用Python写入文件的。让我们用cat命令查看这个文件，看看它是否真的存在。实际上我们可以看到，代理确实将正确的加密消息写入了我们的文件夹。

让我们创建一个简单的界面来与我们的编程助手互动。我们可以通过让助手创建一个ASCII奶牛并打印出来进行测试。运行成功，我们看到了ASCII艺术形式的奶牛。你可以继续与你的助手对话。我们在本实验中构建的助手是在本地笔记本上运行代码的。在下一课中，你将了解更多关于代码执行环境的知识，以及根据不同的使用场景选择最适合的环境。
