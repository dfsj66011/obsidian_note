...
我们可以回到之前的状态，从上次中断的地方继续。更重要的是，这是一种非常棒的开发和构建方式——利用检查点测试当前成果，如果可行就继续推进，如果失败就回退重试。好，这里还有几个概念需要讨论，然后我们就开始动手构建。首先是调试环节。正如我所说，程序总会出问题，而调试就是找出问题根源的过程。虽然听起来像句废话，但我保证这个理念值得反复强调。
