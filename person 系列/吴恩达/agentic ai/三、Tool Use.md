...
在我参与的几项智能代理应用中，我曾赋予大语言模型编写代码以执行指定任务的能力。多次实践下来，这些模型为完成各类任务所生成的代码解决方案之精妙，总令我惊喜不已。若您尚未充分体验代码执行功能，相信这项能力将为您的语言模型应用带来意想不到的惊喜。

让我们通过一个具体案例来演示：假设要开发能解数学应用题的系统，您可能需要创建加减乘除等基础运算工具。如果有人问：“请计算13.2加18.9”，系统就会调用加法工具给出正确答案。但如果现在有人输入“2的平方根是多少”呢？一种方法是专门为平方根功能编写新工具，但接下来可能还需要开发指数运算等更多功能。事实上，看看现代科学计算器上的按键数量——难道我们要为每个按键功能都单独开发工具吗？更何况数学运算需求远不止这些。因此，与其逐个开发工具，更好的解决方案是让AI编写并执行代码。

要告诉大语言模型编写代码，你可以这样写提示语："编写代码来解决用户的问题。用 execute Python 和结束的 execute Python 标签将你的答案作为 Python 代码返回。" 因此，当用户提出诸如"2的平方根是多少"这样的问题时，大语言模型可能会生成这样的输出。然后你可以使用模式匹配，例如正则表达式，来查找开始和结束的 execute Python 标签，并提取它们之间的代码。

因此，在绿色框中你会看到这两行代码，然后你可以为 LLM 执行这段代码并获得输出，在这个例子中是 1.4142 等等。最后，这个数值答案会被传回给 LLM，它就可以写一个格式良好的答案来回答最初的问题。有几种不同的方法可以执行 LLM 的代码执行步骤。一种是使用 Python 的 exec 函数。这是一个内置的 Python 函数，可以执行你传入的任何代码。

这对于你的 LLM 来说非常强大，能够真正编写代码并让你执行这些代码，尽管存在一些安全隐患，我们将在本视频后面看到。此外，还有一些工具可以让你在更安全的沙盒环境中运行代码。当然，二的平方根是一个相对简单的例子。LLM 还可以准确地编写代码，例如进行利息计算，并解决比这更复杂的数学问题。

对这个想法的一个改进，你在我们关于反思的部分已经有所了解，就是如果代码执行失败，也就是说如果由于某种原因，LLM 生成的代码不完全正确，那么将错误信息反馈给 LLM，让它进行反思，或许能修改代码并再尝试一两次。有时这也能让它得到一个更准确的答案。不过，运行 LLM 生成的任意代码确实存在很小的可能性导致不好的事情发生。

最近，我团队里的一名成员在使用一个高度自主的编程助手时，它竟然擅自删除了项目目录中的`star.py` 文件。这可是真实发生的事例。最终那个编程助手倒是道歉了，它承认说："是的，您说得对，这确实是个极其愚蠢的错误。"虽然我很欣慰它能诚恳认错，但当时已经有一堆 Python 文件被删除了。幸亏那位团队成员在 GitHub 仓库做了备份，才没有造成实际损失。但试想如果这个随意删除文件的错误代码在没有备份的情况下被执行，后果真是不堪设想。

因此，代码执行的最佳实践是在沙盒环境中运行。实际上，单行代码的风险并不那么高。坦白说，许多开发者会直接执行大语言模型生成的代码而不做太多检查。但如果你想更安全些，最佳做法是创建一个沙盒环境，这样即使大语言模型生成了有害代码，也能降低数据丢失或敏感信息泄露等风险。

因此，像 Docker 或 E2B 这样的沙盒环境作为一种轻量级沙盒环境，可以降低任意代码以损害系统或环境的方式被执行的风险。事实证明，代码执行如此重要，以至于许多大型语言模型的训练者实际上会做特殊工作，以确保代码执行在他们的应用程序上运行良好。但我希望，当你将此作为又一个工具提供给大型语言模型或让你的应用程序变得更强大时。

到目前为止，根据我们的讨论，你需要创建工具并逐个提供给你的 LLM。事实上，许多不同的团队都在构建类似的工具，并且不得不完成所有这些构建功能并将它们提供给模型的工作。但最近出现了一个名为 MCP（Model Context Protocol）的新标准，它让开发者更容易获取大量可供 LLM 使用的工具。这是一个重要的协议，越来越多的团队正在使用它来开发基于 LLM 的应用程序。让我们在下一个视频中了解 MCP。

## 3.5 MCP

MCP（模型上下文协议）最初由 Anthropic 提出，现已成为众多企业和开发者广泛采用的标准，旨在为大型语言模型提供更丰富的上下文和工具支持。目前有大量开发者围绕 MCP 生态系统进行开发，掌握这一技术将显著拓展您应用程序的资源获取渠道。下面让我们深入了解。

这是 MCP 试图解决的痛点。如果一名开发者正在编写一个需要整合 Slack、Google Drive 和 GitHub 数据或访问 Postgres 数据库的应用程序，那么他们可能需要编写代码来封装 Slack API 以提供应用程序所需的功能，编写代码来封装 Google Drive API 以解析应用程序，对其他工具或数据源也同样如此。而在开发者社区中经常发生的情况是，如果另一个团队正在构建不同的应用程序，他们也会自行与 Slack、Google Drive、GitHub 等进行集成。

许多开发者都在为这些数据源构建自定义封装器。如果有 M 个应用正在开发，而市面上有 N 种工具，那么社区需要完成的总工作量就是 M乘以 N。而 MCP 提出的标准让应用能够统一访问工具和数据源，使得社区需要完成的总工作量从 M乘以 N 减少到 M 加 N。MCP 最初的设计非常注重如何为 LLM 提供更多上下文或如何获取数据。因此，最初的许多工具都是用于获取数据的。如果你阅读 MCP 文档，会发现这些工具被称为资源。

但 MCP 不仅提供数据访问，还包含应用程序可能调用的更通用功能。事实上存在大量 MCP 客户端应用，这些程序需要访问工具、数据或服务——它们通常是软件封装层，用于接入 Slack、GitHub 或 Google Drive 等平台的数据，或允许用户在这些不同类型的资源上执行操作。如今，消耗工具/资源的 MCP 客户端与提供工具/资源的 MCP 服务，二者数量都在快速增长。

我希望你会发现它对你构建自己的 MCP 客户端很有帮助。你的应用程序也许有一天会成为 MCP 客户端。如果你想为其他开发者提供资源，也许有一天你可以构建自己的 MCP 服务器。让我给你展示一个使用 MCP 客户端的简单例子。这是一个云桌面应用程序，它已经连接到了一个 GitHub MCP 服务器。所以当我输入这个查询时，它会总结这个 URL 下 GitHub 仓库的 readme.md 文件，这实际上是一个 AI 套件仓库。

然后，这个作为 MCP 客户端的应用程序会使用 GitHub MCP 服务器发送请求，请从这个 AI 套件仓库中获取 readme.md文件。接着，它会收到一个相当长的响应。所有这些信息随后会被反馈给 LLM 的上下文，然后 LLM 会生成该 Markdown 文件的摘要。现在，让我输入另一个请求：让我看看最新的拉取请求是什么。这反过来会让 LLM 使用 MCP 服务器发出不同的请求，以列出拉取请求。

这是 GitHub MCP 服务器提供的另一个工具。它会向 AI 套件仓库发起请求，进行排序、更新、列出 20 条等操作。随后返回的响应会被反馈给 LLM，由 LLM 生成关于该仓库最新拉取请求的优质文本摘要。MCP 是一项重要标准。如果你想深入了解，DeepLearning.ai 还开设了专门讲解 MCP 协议的短期课程，完成本课程后如有兴趣可以继续选修。

希望这个视频能让你简要了解它的实用性，以及为什么现在许多开发者都在遵循这一标准。接下来我们将进入工具使用的最后一个视频。希望通过赋予你自己使用工具的能力，你能构建出更强大的自主应用程序。

在下一个模块中，我们将讨论评估和错误分析。事实证明，我发现区分能够很好地执行代理工作流程的团队与效率较低的团队的一个关键因素，是你们推动严格评估流程的能力。在接下来的系列视频中，我认为这可能是整个课程中最重要的模块，我希望与大家分享一些最佳实践，介绍如何利用评估来推动代理工作流程的开发。期待在下一个模块中与大家见面。
