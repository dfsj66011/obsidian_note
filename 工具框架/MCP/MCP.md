...
MCP 生命周期描述了 Host 和 Server 在会话期间建立、使用和终止连接的完整步骤序列。

MCP生命周期有三个阶段：初始化阶段、正常操作阶段、关闭阶段。

#### 初始化阶段

**初始化**：初始化阶段必须是客户端和服务器之间的第一次交互，做两件事：

1. 在客户端和服务器之间，检查 MCP 的版本兼容性。
2. 客户端和服务器之间会相互交换和协商能力，简单来说，客户端会告诉服务器它能做什么，同样地，服务器也会告诉客户端它能做什么。

*初始化阶段的三个步骤*：

1. 客户端向服务器发送一个初始化请求，在服务器上调用 `initialize` 方法，向服务器发送了三样重要的东西：
	* 自己的协议版本，以某种日期格式显示；
	* 客户端的功能，比如第一个是路由，将主机（即系统）的目录提供访问权限给服务器；第二个是采样，如果服务器需要 AI 的帮助，可以通过主机的 AI 来完成自己的工作。
	* 发送客户端的实现信息，比如客户端的名称以及它当前所处的版本。
2. 请求到达服务器，服务器会返回响应
	* 服务端告知自己的协议版本
	* 服务器展示自己具备的工具能力，有哪些功能可供客户端使用；有哪些资源可供客户端使用；
	* 服务器分享其实现信息，比如它的名称、版本等。
3. 客户端发送一个通知回去给服务器，告诉它连接是成功的，即初始化通知。

在整个初始化阶段，有两个重要的规则：
1. 在服务器响应初始化请求之前，客户端不应发送除 ping 之外的请求。
2. 服务器在收到初始化通知之前，不会处理任何类型的请求。

**版本协商**：当客户端与服务端版本不一致，客户端会去检查它的配置文件，看看它支持哪些 MCP 版本，然后它会去检查这个特定的版本，检查它是否在支持的列表中，如果在，那么就会发送客户端通知；如果不在，客户端会断开与服务器的连接，并且客户端和服务器之间不会有进一步的通信。

**能力协商**：客户端和服务器的能力决定了会话期间可用的协议功能。

#### 操作阶段

需要注意两点：

1. 必须遵守协商的协议版本，即按照协议版本的指南来构建 JSON RPC 消息框架。
2. 只能使用那些成功协商的功能，

#### 关闭阶段

会话（即客户端和服务器之间的连接）将被终止。通常是由客户端发起关闭操作，在关闭阶段，你不会看到任何 json rpc 消息。事实上，这里整个关闭的责任在于传输层。

在 STDIO 中，客户端会关闭标准输入（stdin）：客户端作为子进程启动服务器，这意味着客户端拥有控制权，可以管理服务器的 stdin 和 stdout。因此，客户端会自行关闭输入流，并等待服务器退出。如果服务器没有退出，在这种情况下，客户端会寻求操作系统的帮助，发送一个低级信号给操作系统，我们称之为 SIGTERM（信号终止）。简而言之，通过这个信号，我们的客户端告诉操作系统去通知服务器终止。如果服务器仍然没有关闭，那么你的客户端会再次向操作系统发送一个低级信号，我们称之为 SIGKILL，它的全称是 signal kill。所以这里它是在强制操作系统去关闭服务器。

在远程服务器关机中，这里会直接关闭它。如果是服务器主动关闭的情况，比如服务器在远程运行时被关机，那么在这种情况下，服务器会主动关闭 HTTP 连接。在这种情况下，客户端应该具备能力，能够妥善处理关闭操作。如果有正在运行的任务，应该优雅地关闭它，并尝试重新连接到服务器。基本概念就是，如果客户端发起关闭（这是很常见的），那么它基本上会终止在 HTTP 上建立的 post 连接，就是这样。

-------------

两个库：FastMCP or  MCP SDK

2023 年底，MCP 诞生了，Anthropic 发布了官方的 MCP SDK，不好用，后有人在 MCP SDK 之上写了一个抽象层，命名为 FastMCP，对新手友好。后 MCP SDK 干脆直接内部默认使用 FastMCP。而 MCP SDK 的重点在于它只专注于 MCP，围绕其规范进行开发。所以大约在 2025 年，这两方分道扬镳了，也就是说 FastMCP 选择了不同的道路，作为独立的库建立起来了。
